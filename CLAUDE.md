...
- Migrations : Flyway actif (source de vérité), `backend/src/main/resources/db/migration/` contient V1..V15 (dernier : `V15__password_reset_tokens_version.sql`) ; `ddl-auto=validate` (dev + prod). Tout changement de schéma exige une migration V{N} + un mapping entité exact, sinon `SchemaManagementException` au boot. **Prochaine migration : V16.**
