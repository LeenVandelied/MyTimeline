...
## Issue
[BUG] NPE dans Utils.calculateEndDate quand durationUnit est null (BR-EVE-004)

### Contexte
Certaines combinaisons de saisie provoquent une erreur 500 brutale au lieu d'un message métier clair.

### Description
Dans `backend/src/main/java/com/matimeline/eventmanager/utils/Utils.java` (~ligne 11), `switch (eventCreationRequest.getDurationUnit())` ne vérifie pas la nullité avant usage. Un `POST /api/events` avec `type='duration'`, `durationValue` renseigné mais `durationUnit` null → NullPointerException non gérée → 500 au lieu d'un 400 métier clair. Documenté BR-EVE-004 dans br-events.md.

### Critères d'acceptation
- Un appel avec `durationUnit` null renvoie une erreur 400 avec message explicite (pas de 500)
- Tests unitaires `UtilsTest` ajoutés couvrant BR-EVE-003 (calcul correct selon l'unité) ET BR-EVE-004 (unité inconnue ET unité null)

### Piste technique
- `backend/src/main/java/com/matimeline/eventmanager/utils/Utils.java:11` — null-guard explicite avant le `switch`. Le message d'erreur doit remonter en 400 (IllegalArgumentException mappée, ou mécanisme de validation existant — vérifier le ControllerAdvice/handler en place).
- Filet complémentaire DB : issue #128 (contrainte CHECK) — ne PAS la traiter ici.
...
- Vague 1 : lancé en parallèle avec #161 (frontend, fichiers 100% disjoints, aucun build partagé).
- Livré AVANT #162 (upgrade Boot) : ton null-guard trivial survivra à l'upgrade ; la suite complète sera re-validée sous le nouveau Boot par le test-runner final.
- Aucune dépendance amont.

## Designer
Non applicable (correctif backend pur).
...
- Tu partages ce worktree avec l'agent #161 (frontend). Tu ne touches QUE : `backend/src/main/java/com/matimeline/eventmanager/utils/Utils.java` et le fichier de test `UtilsTest` (créer/étendre sous `backend/src/test/...`).
- NE TOUCHE PAS à `backend/pom.xml` (réservé à #162, vague suivante).
- Au commit : `git add` avec les chemins EXACTS de tes 2 fichiers UNIQUEMENT (JAMAIS `git add -A` / `git add .`).

### Travail
- Branche cible : `sprint/14` (déjà checkout).
- Ajouter un null-guard explicite avant le `switch` dans `Utils.calculateEndDate`. `durationUnit` null → erreur métier 400 (pas 500). Regarde comment les autres validations du domaine remontent un 400 (ControllerAdvice/handler existant, cf. br-events.md et cp-backend.md) et reste cohérent avec ce mécanisme.
- Ne PAS changer le comportement nominal (BR-EVE-003 : calcul correct selon l'unité doit rester intact).
- Tests : `UtilsTest` couvrant BR-EVE-003 (calcul par unité) + BR-EVE-004 (unité inconnue ET unité null → exception/400).
- Lancer : `./scripts/test-quiet.sh unit` (ou scope backend ciblé). Si le volume > 500 tests ou > 3 min → signaler RECOMMAND_TEST_RUNNER plutôt que tout lancer.
- Commit : 1 commit logique, gitmoji français (ex: `:bug: null-guard durationUnit dans calculateEndDate — 400 au lieu de 500 (BR-EVE-004, #164)`).

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: <fichiers touchés + mécanisme 400 utilisé + BR couvertes + résultat tests>
- [MEMORY:*] signaux: <si applicable>
- recommandations suite: <RECOMMAND_* ou pitfall ; sinon "Pas de RECOMMAND_X car ...">
