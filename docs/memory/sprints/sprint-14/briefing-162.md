...
## Issue
[SECURITY] Backend : upgrade Spring Boot 3.2.2 → 3.3/3.4 LTS (CVE tomcat-embed, spring-security, postgresql)

### Contexte
Spring Boot 3.2.x est en fin de support (EOL) : les failles découvertes ne sont plus corrigées. Passage à une version LTS (3.3 ou 3.4).

### Description
`trivy` remonte 35 vulnérabilités HIGH/CRITICAL sur `backend/pom.xml` :
- `tomcat-embed-core` 10.1.18 : 4 CRITICAL (dont CVE-2025-24813) + 15+ HIGH
- `spring-security-web` 6.2.1 : CVE-2024-38821 CRITICAL
- `postgresql` (driver JDBC) 42.6.0 : CVE-2024-1597 CRITICAL
- `jackson-databind` 2.15.3, `spring-core` 6.1.3 : failles additionnelles

### Point d'attention documenté — DEC-S3-001
Lors du passage Spring Boot 3.3+/Flyway 10+, il FAUT ajouter le module `flyway-database-postgresql` : le support PostgreSQL quitte `flyway-core` à partir de Flyway 10. Sans ce module, le démarrage Flyway casse.

Monter également `jjwt` 0.11.5 → 0.13.x.

### Critères d'acceptation
- `trivy fs` ne remonte plus aucune vuln CRITICAL sur `backend/`
- `./mvnw verify` passe (tests Testcontainers inclus)
- Démarrage app + migrations Flyway OK sur base VIDE
- Démarrage app + migrations Flyway OK sur base EXISTANTE (non vide)

### Piste technique
- `backend/pom.xml` : upgrade parent Spring Boot 3.2.2 → 3.3.x ou 3.4.x LTS (choisir la LTS la plus stable ; documenter le choix)
- Ajouter `flyway-database-postgresql` (DEC-S3-001)
- Upgrade `jjwt` 0.11.5 → 0.13.x — ⚠ API BREAKING sur signature/parsing JWT (`Jwts.parserBuilder()` → `Jwts.parser()`, `SignatureAlgorithm`/`Keys`, claims). Impacte la révocation jti BR-AUT-011. Adapter le code JWT en conséquence (JwtService).
- Vérifier compatibilité `tomcat-embed-core`, `spring-security-web`, driver `postgresql`, `jackson-databind` après upgrade du parent Boot (héritent normalement des versions gérées par le BOM Boot).
...
issue_0162:
  fichiers_cles:
    - "backend/pom.xml"
    - "backend/src/main/java/.../JwtService (adaptation API jjwt 0.13.x)"
  ...
  risque_regression: "Upgrade majeur Boot 3.3/3.4 : JPA/security/migrations — surface large. flyway-database-postgresql requis (DEC-S3-001). jjwt 0.11.5→0.13.x = API breaking signature/parsing JWT (BR-AUT-011 révocation jti)."
  ordre_ecriture: "infra (pom) → adapter code jjwt → vérif compilation → tests non-régression complets"
  zod_dto_sync: "NON"
  etat_reel_du_code: "Confirmé Boot 3.2.2 dans backend/pom.xml"
```
...
- Vague 1b : lancé SEUL (aucun autre agent n'écrit sur le worktree en même temps) — c'est volontaire : cet upgrade réécrit le build backend, aucune concurrence de build tolérée.
- Les commits de #164 (Utils null-guard) et #161 (frontend) sont DÉJÀ sur `sprint/14` en amont. Ton `./mvnw verify` doit les inclure et rester vert.
- Doit être livré AVANT #168 (validations Bean) et #128 (migration V10), qui présument un build Boot stable.

## Designer
Non applicable (upgrade infra pur).
...
- Vérifier `git rev-parse --abbrev-ref HEAD` == `sprint/14`. Si non → STOP, retourner STATUS PARTIAL + raison.
- Tu es SEUL sur le worktree pour cette vague. Tu peux toucher `backend/pom.xml` et le code backend strictement nécessaire à l'adaptation de l'API jjwt (ex: `JwtService`). Ne modifie PAS de logique fonctionnelle non liée à l'upgrade.
- Au commit : `git add` les chemins EXACTS modifiés (JAMAIS `git add -A` non maîtrisé — vérifie `git status` avant).

### Travail
- Branche cible : `sprint/14` (déjà checkout, contient déjà les commits #161/#164).
- Upgrade parent Boot vers une LTS 3.3.x/3.4.x. Ajouter `flyway-database-postgresql` (DEC-S3-001). Upgrade `jjwt` 0.13.x et adapter le code JWT cassé par l'API breaking.
- Vérifier compilation, puis suite complète : `./scripts/test-quiet.sh backend` (Testcontainers). Comme c'est un upgrade majeur, le volume/temps peut être élevé — c'est attendu ici, lance la suite complète (tu ES la validation de non-régression de cet upgrade).
- Vérifier explicitement le démarrage + migrations Flyway. Si tu ne peux pas tester une "base existante non vide" facilement, décris comment tu l'as approximé (Testcontainers applique toutes les migrations = base construite ; c'est un proxy acceptable, le noter).
- Si `trivy` est disponible localement, vérifier 0 CRITICAL sur `backend/` ; sinon documenter que la vérification trivy reste à faire en CI.
- Commit : 1 commit logique (ou 2 si séparation pom / adaptation jjwt plus lisible), gitmoji français (ex: `:arrow_up: Spring Boot 3.2.2→3.4 LTS + flyway-database-postgresql + jjwt 0.13 (CVE, #162)`).

### Signaux attendus
- Si tu touches la sécurité JWT/auth → signale `RECOMMAND_SECURITY` (audit sécurité post-upgrade).
- Si le comportement Flyway/migrations change → signale `RECOMMAND_DB_EXPERT`.

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: <versions avant→après (Boot, jjwt, flyway) + adaptations code jjwt + résultat mvnw verify + Flyway vide/existante + trivy si dispo>
- [MEMORY:*] signaux: <si applicable — ex [MEMORY:decision] confirmation DEC-S3-001>
- recommandations suite: <RECOMMAND_SECURITY / RECOMMAND_DB_EXPERT / autre, ou négation explicite>
