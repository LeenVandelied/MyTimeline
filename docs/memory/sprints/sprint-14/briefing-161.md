...
## Issue
[SECURITY] Frontend : appliquer les correctifs CVE disponibles dans les ranges semver (axios, next, form-data)

### Contexte
Audit qualité 2026-07-02 : 22 vulnérabilités npm (3 critical, 5 high), trivy 25 HIGH/CRITICAL sur `package-lock.json`. Toutes corrigées par les éditeurs — mise à jour vers versions déjà autorisées par les ranges semver du `package.json`, AUCUN changement de version majeure.

### Cibles
- `axios` 1.8.4 → 1.18.1 attendu (13 CVE HIGH dont CVE-2025-58754)
- `next` 15.2.4 → 15.5.20 attendu (CVE-2025-55182 CRITICAL + 9 HIGH)
- `form-data` 4.0.2 → 4.0.6 attendu (CVE-2025-7783 CRITICAL)
Tout corrigeable par `npm update` ciblé, sans changer les ranges de `package.json`.

### Critères d'acceptation
- `npm audit` ne remonte plus aucune vuln critical/high sur les deps de production
- `npm run build` passe sans erreur
- `npm run test` passe sans régression
- `package-lock.json` mis à jour est commité

### Piste technique
- `frontend/package-lock.json` : mettre à jour axios, next, form-data via `npm update`
- Ne PAS élargir les ranges dans `package.json` sauf si un correctif l'exige (le documenter alors)
...
issue_0161:
  ...
  zod_dto_sync: "NON"
  etat_reel_du_code: "grep 'axios ^1.8.1' = dép courante (FAUX POSITIF). Vérifier les ranges réels avant."
```
...
- Vague 1 : lancé en parallèle avec #164 (backend Utils) et #162 (backend pom.xml). Fichiers 100% disjoints.
- Aucune dépendance amont.

## Designer
Non applicable (aucun changement visuel — bump de dépendances).
...
- Vérifier `git rev-parse --abbrev-ref HEAD` == `sprint/14`. Si non → STOP, ne rien faire, retourner STATUS PARTIAL + raison.
- Tu partages ce worktree avec 2 autres agents. Tu ne touches QUE : `frontend/package.json`, `frontend/package-lock.json`.
- Au commit : `git add frontend/package.json frontend/package-lock.json` UNIQUEMENT (JAMAIS `git add -A` ni `git add .` — tu écraserais le travail en cours des autres agents).

### Travail
- Brancher cible : `sprint/14` (déjà checkout).
- Faire `cd frontend`, puis `npm update axios next form-data` (ou ciblage précis si besoin), sans modifier les ranges de `package.json` sauf nécessité documentée.
- Vérifier : `npm audit --production` (0 critical/high attendu), `npm run build`, `npm run test`.
- Commit : 1 commit logique, message gitmoji français (ex: `:lock: bump axios/next/form-data — correctifs CVE (#161)`).
- Ne PAS toucher au backend.

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: <versions bumpées avant→après + résultat npm audit + build + test>
- [MEMORY:*] signaux: <si applicable>
- recommandations suite: <RECOMMAND_* ou pitfall subtil ; sinon "Pas de RECOMMAND_X car ...">
- STATUS: COMPLETED en dernière ligne du retour (ou STATUS: PARTIAL + BLOQUE_SUR)
