...
## Issue
**[BUG] Au zoom arrière, la frise saute loin d'aujourd'hui et n'affiche plus aucun événement**

Symptôme d'origine : au zoom arrière (bouton toolbar ou raccourci `-`), l'affichage saute au bout
de la période et ne montre plus **aucun** événement, alors que les lanes produit restent visibles.
Mesures relevées à l'époque via `page.evaluate` : `scrollLeft = 31348`, `scrollWidth = 32330`,
`clientWidth = 982` — soit exactement `scrollWidth − clientWidth`, le clamp navigateur au bord
droit. Zéro `data-testid="timeline-event"` monté à cet instant.

## ⚠ ÉTAT RÉEL DU CODE — LIS CECI AVANT DE TOUCHER QUOI QUE CE SOIT (`possibly_done`)

**Le correctif de code décrit par l'issue est DÉJÀ dans `dev`.** Vérifié par le lead en Phase 0.5 :

- Commit `3dcc5ea` — `:bug: fix(timeline): ancre le defilement sur le temps, pas sur les pixels
  (#449)`, 2026-08-31, **ancêtre de `origin/dev`**.
- Il a introduit `anchorDaysRef` / `autoAnchorRef` / `lastDayWidthRef` et un `useLayoutEffect`
  sur `[dayWidth]` (`frontend/src/components/timeline/TimelineView.tsx:895-912`) qui re-projette
  `scrollLeft` au changement d'échelle : `el.scrollTo({ left: anchorDaysRef.current * dayWidth })`,
  puis relit la valeur réellement retenue pour rester fidèle au clamp.
- Il conserve explicitement le repère PISTE de #392 (commentaire en place, `LANE_TRACK_OFFSET_PX`).
- **Le corps de l'issue est PÉRIMÉ** : il décrit `TimelineView.tsx:795-820` avec un `scrollToToday()`
  en `useEffect(..., [])` monté une seule fois. Ce code n'existe plus. Ne te fie pas aux numéros de
  ligne de l'issue, ils ne correspondent à rien.

**Ce qui reste réellement à faire, et c'est TON périmètre : le test de non-régression.**
Aucun test n'épingle le cas mesuré. La spec la plus proche, `frontend/e2e/timeline.spec.ts:977`
(« live-region : contenu réel annoncé »), dézoome puis asserte une pastille — mais elle passe par
`revealSeededLane`, une parade à la virtualisation **VERTICALE** (#467). Elle n'épingle donc PAS
le clamp du défilement **HORIZONTAL** : elle resterait verte si la re-projection d'ancre était
supprimée. Elle ne protège pas ce correctif.

## À faire
1. **Commence par VÉRIFIER mon constat** — ne me crois pas sur parole. Lis
   `TimelineView.tsx:890-960`, et confirme que la re-projection sur `dayWidth` est bien là et bien
   armée. Si tu trouves que le correctif est en réalité incomplet ou faux, **dis-le et corrige-le** :
   remonte-le explicitement dans le done.md. C'est un résultat valable, pas un échec.
2. **Écris le test de non-régression** qui manque. Il doit reproduire le cas MESURÉ :
   un événement proche du début de l'étendue de dates, un zoom arrière, puis l'assertion qu'au
   moins un `data-testid="timeline-event"` est **monté dans le DOM**.
   - Le test doit **échouer si on retire la re-projection d'ancre**. C'est le seul critère qui
     compte. Vérifie-le pour de vrai : neutralise temporairement l'effet `[dayWidth]`, constate le
     rouge, remets-le, constate le vert. Rapporte ces deux observations. Un test qui passe dans les
     deux cas ne prouve RIEN et sera rejeté.
   - ⚠ **PIÈGE MESURÉ, ne tombe pas dedans** : sous jsdom, `scrollLeft` n'est PAS clampé (on écrit
     400, on relit 400). Un test unitaire jsdom sur le défilement **ne prouve rien** — tout le
     mécanisme en cause EST le clamp du navigateur. **Il faut un E2E Playwright.** Si tu ajoutes
     aussi un test unitaire, très bien, mais il ne remplace pas l'E2E.
   - Ne masque pas un échec avec `revealSeededLane` sans comprendre ce que tu masques : c'est une
     parade à la virtualisation verticale. Si tu en as besoin, dis pourquoi, et assure-toi que le
     test reste sensible au défaut horizontal.
3. **Non-régression #392, explicitement** : la pastille du jour doit tomber JUSTE APRÈS l'en-tête
   sticky (repère PISTE, `LANE_TRACK_OFFSET_PX`), pas dessous. Vérifie que ça tient toujours après
   ton test / ton éventuel correctif, et dis comment tu l'as vérifié.
4. **Non-régression raccourcis** : `T`, `[`, `]` continuent de fonctionner.

## Critères d'acceptation (issue)
- [ ] Après un zoom arrière, au moins un événement visible avant le zoom reste visible ou
      atteignable sans manipulation manuelle du scroll
- [ ] Un test reproduit le cas mesuré (événement proche du début de l'étendue, zoom arrière, au
      moins un `data-testid="timeline-event"` monté) — **et échoue si le correctif est retiré**
- [ ] Le comportement volontaire de #392 reste intact — non-régression vérifiée explicitement
- [ ] Les raccourcis `T`, `[`, `]` fonctionnent sans régression

## Triage
Taille: M (probablement réduite à S — le code est là, le test manque)
Modele: opus
Effort: high
...
- Vague 1, en parallèle de #452 (backend pur) et #469 (harnais E2E : `e2e/support/accounts.ts`,
  `playwright.config.ts`, specs `settings-*`).
- **Fichiers que tu NE DOIS PAS toucher** (pris par d'autres subagents en parallèle) :
  `frontend/playwright.config.ts`, `frontend/e2e/support/accounts.ts`, `frontend/package.json`,
  `frontend/e2e/settings-*.spec.ts`, `docs/memory/pitfalls.md`, tout `backend/**`.
- Ton terrain : `frontend/e2e/timeline*.spec.ts`, `frontend/e2e/support/timeline-lanes.ts`,
  `frontend/src/components/timeline/**`.
- Si tu as besoin d'un helper partagé qui appartient à #469, NE LE MODIFIE PAS : signale-le en
  RECOMMAND_FOLLOWUP et contourne dans ton propre fichier.

## Designer
Non applicable (test de non-régression, aucune nouvelle surface visuelle).

## Contraintes
- Branche cible : `claude/sprint-65-start-468415` (déjà checkout — NE PAS créer/changer de branche).
- Commit : 1 seul commit logique, message gitmoji en **français**.
- `git add` CIBLÉ. **JAMAIS `git add -A` ni `git add .`** — working tree partagé avec 2 autres
  subagents qui committent en parallèle.
- Tests E2E : la recette locale EST fonctionnelle (mesurée 49/49 en 38 s). Pièges connus :
  - CORS/Origin sur `:3000` se déguise en « rate-limit » — un `curl` qui réussit ne disculpe PAS
    le CORS (`curl` n'envoie pas d'`Origin`). Lis les statuts instrumentés par la fixture AVANT
    toute hypothèse : 3 diagnostics faux ont été posés en S57 faute de le faire.
  - `workers > 1` fait rougir 4 specs `settings-*` (c'est précisément l'objet de #469, en cours en
    parallèle). `playwright.config.ts` est actuellement à `workers: 1` — **laisse-le tel quel**.
  - Une fixture E2E rouge n'accuse pas forcément le code : vérifie d'abord l'arithmétique de la
    fixture (piège mesuré en S51).
- TypeScript strict, i18n via next-intl (locale principale `fr`), code EN / docs FR.
...
RETOUR :
- commits: [SHA]
- verdict etat du code: <le correctif 3dcc5ea est-il complet OUI/NON + preuve>
- preuve de sensibilite du test: <rouge sans le correctif / vert avec — les 2 observations réelles>
- resume: <fichiers clés + tests réels lancés/passés + comment #392 a été vérifié>
- [MEMORY:*] signaux: <bug|pitfall|pattern|decision si applicable>
