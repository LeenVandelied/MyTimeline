...
## Issue
**[CHORE] Sortir les identités E2E du scope module pour rendre `workers>1` viable**

Pour rendre un run E2E local interprétable, le Sprint 64 a dû limiter Playwright à un seul worker
en local (`frontend/playwright.config.ts`, issue #465). La parade fonctionne mais coûte cher :
un run local complet passe de **4,8 min (2 workers) à 9,0 min (1 worker)**. Cette issue vise à
retrouver la vitesse du parallélisme sans réintroduire les échecs qui ont justifié de le brider.

## Diagnostic — DÉJÀ FAIT ET MESURÉ au Sprint 64, ne le refais pas
À 2 workers, **4 des 5 échecs observés sont exactement `PIT-S47-004`** :
`toHaveValue` attend `sh4148187640411` et reçoit `sh4148087641348`.

Cause : deux process Node chargent `frontend/e2e/support/accounts.ts` **AVANT** que le projet
Playwright `setup` n'ait persisté `.auth/accounts.json`. Chacun fige alors son propre `RUN` dérivé
du `pid`, et la spec compare son identité locale au compte réellement enregistré par l'autre process.

`dependencies: ['setup']` **n'y change rien** : il ordonne l'**exécution**, pas le moment de
l'**import du module**. L'import de `accounts.ts` a déjà lieu, dans chaque worker, avant que
`setup` ait fini d'écrire le fichier.

## À faire
1. **Sortir l'identité `RUN` du scope module** pour qu'elle soit résolue **à l'usage** plutôt qu'à
   l'import — p. ex. en lisant `.auth/accounts.json` au moment où la valeur est effectivement
   utilisée (accès paresseux / getter / fonction), ou par tout autre mécanisme qui élimine la
   dépendance à l'ordre d'import entre workers. Le choix du mécanisme t'appartient : justifie-le.
   - Contrainte à PRÉSERVER : le fichier documente longuement POURQUOI les comptes sont fixes —
     anti rate-limit `register` (`RateLimitingFilter` : `/api/auth/register` = 5 req/min/IP).
     Le job CI `e2e` tourne sur UNE IP. Le nombre total de `register` de la suite doit rester
     **sous le seuil**. Ne régresse pas là-dessus en « simplifiant » l'identité.
   - Contrainte à PRÉSERVER : identités bornées 3..20 caractères (BR-AUT-003 + schéma register).
   - Contrainte à PRÉSERVER : l'isolation inter-process (deux jobs CI sur le même runner peuvent
     partager l'horloge → `Date.now()` seul risque la collision).
2. **Remonter `workers`** au-delà de 1 en local dans `frontend/playwright.config.ts`, une fois le
   correctif tenu. Le fichier porte un long commentaire expliquant la parade #465 : **mets-le à
   jour**, ne le laisse pas mentir sur l'état réel.
3. **Valider par des runs RÉELS** : les 4 specs `settings-*` doivent passer au vert sur **au moins
   2 runs complets consécutifs** à `workers > 1`. C'est un critère d'acceptation explicite parce
   que le défaut est intermittent — un seul run vert ne prouve rien. **Mesure et consigne la durée**
   des runs (l'issue le demande nommément).
4. **Corriger une mention FAUTIVE dans `docs/memory/pitfalls.md`** : l'entrée `PIT-S47-004` déclare
   le défaut « corrigé » par la persistance de `.auth/accounts.json`. **C'est faux**, et le Sprint 64
   l'a mesuré directement (4 échecs sur 5, mot pour mot). Un futur agent qui s'y fierait rouvrirait
   le parallélisme et récolterait 4 rouges par run. Corrige l'entrée pour dire ce qui est vrai.

## Note du lead — attention au faux positif
Il existe un 5e échec (`event-outside-label`) qui n'est **PAS** un problème de workers : il persiste
à 1 worker. Ne le compte pas dans ton verdict et ne cherche pas à le corriger ici (hors périmètre).
Si tu le vois rouge, dis-le, ne le maquille pas.

## Critères d'acceptation (issue)
- [ ] `workers > 1` viable en local : les 4 specs `settings-*` vertes sur **≥ 2 runs complets
      consécutifs**
- [ ] La durée du run local à `workers > 1` est **mesurée et consignée**
- [ ] La mention fautive de `PIT-S47-004` dans `docs/memory/pitfalls.md` est corrigée

## Risque signalé par l'issue
Le correctif touche un mécanisme d'identité **partagé** entre plusieurs specs `settings-*` : une
résolution mal isolée pourrait réintroduire une variante du même défaut sous une autre forme.
D'où l'exigence des 2 runs consécutifs — ne la contourne pas.
...
- Vague 1, en parallèle de #452 (backend pur) et #451 (specs timeline).
- **Tu es le propriétaire exclusif de** : `frontend/e2e/support/accounts.ts`,
  `frontend/playwright.config.ts`, `frontend/e2e/settings-*.spec.ts`, `docs/memory/pitfalls.md`.
- **Fichiers que tu NE DOIS PAS toucher** : `frontend/e2e/timeline*.spec.ts`,
  `frontend/e2e/support/timeline-lanes.ts`, `frontend/src/components/timeline/**` (pris par #451),
  tout `backend/**` (pris par #452), `frontend/package.json` (pris par #470 en vague 2).
- **#470 arrive APRÈS toi** sur `frontend/playwright.config.ts` (drapeau
  `--pass-with-no-tests`). Laisse le fichier dans un état propre et commité ; ne traite PAS #470
  toi-même.

## Designer
Non applicable (harnais de test, aucune surface visuelle).
...
- La recette E2E locale EST fonctionnelle (mesurée 49/49 en 38 s sur la suite ciblée). Pièges :
  - **CORS/Origin sur `:3000` se déguise en « rate-limit »**. Un `curl` qui réussit ne disculpe
    PAS le CORS — `curl` n'envoie pas d'en-tête `Origin`. Lis les statuts instrumentés par la
    fixture AVANT toute hypothèse : 3 diagnostics faux ont été posés en S57 faute de le faire.
    C'est directement pertinent ici, puisque tu vas voir des échecs d'identité/auth.
  - Une fixture E2E rouge n'accuse pas forcément le code : vérifie l'arithmétique de la fixture
    avant d'accuser (piège mesuré en S51).
- TypeScript strict.
- `rtk proxy git diff` (le hook RTK vide la sortie de `git diff` nu).
- Si un run complet dépasse ~3 min ou 500 tests, c'est normal ici (c'est l'objet de l'issue) :
  ne coupe pas court, la mesure EST le livrable.

## Livrable attendu (format strict, MAX 500 tokens, style caveman)
Écris ton rapport ET rends-le. Dernière ligne = STATUS.
RETOUR :
- commits: [SHA]
- mecanisme retenu: <comment `RUN` est désormais résolu, et pourquoi ça élimine l'ordre d'import>
- preuve runs: <workers=N ; run 1 = X passed/Y failed en Zmin ; run 2 = ... — 2 runs CONSÉCUTIFS réels>
- registers comptes: <nombre total de register de la suite, vs seuil 5/min/IP>
- pitfalls.md: <ce qui a été corrigé dans PIT-S47-004>
