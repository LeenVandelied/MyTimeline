[BRIEFING ISSUE #249 — Sprint 50, vague 1]
...
git rev-parse --short HEAD   # DOIT afficher ada1ed9
git branch --show-current    # DOIT afficher claude/sprint-50-start-9b7161
```

Si HEAD ou la branche diffèrent → STOP, retourne `STATUS: PARTIAL` + `BLOQUE_SUR: mauvais worktree`.

Piège connu (mémoire projet) : `git diff` renvoie ~vide sous le hook RTK de ce poste. Utilise
`rtk proxy git diff` / `rtk proxy git log` ou un dump dans un fichier + lecture.

## ⛔ RÈGLE ABSOLUE — SECRETS (prime sur tout le reste de ce briefing)

- **Tu ne dois JAMAIS afficher, coller ou recopier une valeur de secret** — ni dans ton retour, ni
  dans un fichier, ni dans un commit, ni dans un message. **Noms de variables uniquement.**
- Tes commandes d'audit doivent être écrites pour **ne pas** faire sortir la valeur : compte les
  occurrences, donne `commit:fichier:ligne` et la **longueur** de la valeur, jamais la valeur.
  N'utilise pas `git show <rev>:<fichier>` en sortie brute sur un fichier de secrets — filtre.
- **Tu ne rotationnes RIEN.** Pas de `gh secret set`, pas d'appel à un dashboard fournisseur, pas de
  génération de nouvelle valeur, pas de `ALTER ROLE`. C'est une action opérateur, réservée au dev.
- Si une valeur t'échappe accidentellement dans une sortie : signale-le immédiatement et
  explicitement dans ton retour (`[MEMORY:pitfall]` + mention en clair du fait, pas de la valeur).

## Issue #249 — [CHORE] Rotation des secrets exposés dans l'historique git

Labels : `chore`, `epic:devops`, `priority:P1`, `size:S`, `sprint-50`

### Contexte (corps de l'issue)

Le travail d'externalisation des secrets (issue #34, clôturée) a sorti les secrets du code source,
mais n'a PAS purgé les anciennes valeurs en clair qui restent visibles dans l'historique git. Un
runbook de purge de l'historique a été livré en Sprint 29 (issue #112).

Cette issue couvre une action distincte et indépendante de la purge d'historique : la rotation
effective des secrets eux-mêmes. Même après purge de l'historique, les valeurs exposées doivent être
considérées comme compromises et changées, car elles ont pu être copiées avant la purge.

### À faire (corps de l'issue)

- Faire tourner : `DB_PASSWORD`, `JWT_SECRET`.
- Vérifier également si `BREVO_API_KEY` a été exposée dans l'historique et la faire tourner si c'est
  le cas.
- Appliquer la procédure de rotation par service une fois disponible (issue F3 liée, qui crée
  l'inventaire des services externes).
- ⚠️ La rotation de `JWT_SECRET` invalide tous les tokens JWT existants (déconnexion globale
  attendue). À planifier sur une fenêtre de faible usage et à communiquer en amont.

### Critères d'acceptation (corps de l'issue)

- [ ] `DB_PASSWORD` régénéré et redéployé dans tous les environnements concernés
- [ ] `JWT_SECRET` régénéré et redéployé, avec communication préalable sur la déconnexion globale
- [ ] Vérification effectuée sur `BREVO_API_KEY` (exposée ou non dans l'historique) ; rotation si
      nécessaire
- [ ] Aucune valeur de secret n'apparaît en clair dans l'issue, les commits ou les PR liés
- [ ] Rotation confirmée fonctionnelle (login, envoi d'email, connexion DB testés post-rotation)

---

## ⚠ PÉRIMÈTRE RÉVISÉ AU DÉMARRAGE DU SPRINT (décision dev, 2026-07-28) — LIT CECI

Le lead a vérifié l'état réel avant de te briefer. **Trois faits mesurés changent le périmètre :**

**1. Il n'y a AUCUNE cible de rotation.**
- `docs/memory/devops/secret-rotation-runbook.md` le dit lui-même : *« Non exécuté — projet pas
  encore en production (noté 2026-07-12) »*.
- `gh secret list --repo LeenVandelied/MyTimeline` → **vide**.
- `gh api repos/LeenVandelied/MyTimeline/environments` → **vide**.
- Aucun secrets-manager, aucun backend déployé, aucun workflow de déploiement
  (`.github/workflows/` = `ci.yml` seul).

**2. L'exposition historique est en revanche RÉELLE.** Vérifié par le lead (valeurs jamais affichées) :
- commit `53175da`, `backend/src/main/resources/application.properties` :
  `spring.datasource.password` = littéral opaque de **10 caractères** (ligne 3),
  `jwt.secret` = littéral opaque de **128 caractères** (ligne 12) ;
- commit `c6ea19e`, `backend/src/test/resources/application-test.properties:28` :
  `jwt.secret` = littéral opaque de **68 caractères** ;
- ~9 occurrences historiques au total sur 4 fichiers de configuration.
- État **actuel** propre : `application.properties` et `application-prod.properties` n'utilisent que
  `${DB_PASSWORD}` / `${JWT_SECRET}` ; les défauts en dur d'`application-dev.properties` sont des
  placeholders documentés.

**3. `BREVO_API_KEY` n'a JAMAIS été exposée.** Scan du lead sur 75 fichiers de l'historique portant
le motif `brevo` : **aucune valeur littérale opaque**. Les seuls « hits » sont de la prose dans
`docs/memory/sprints/sprint-8/*`, `sprint-29/issue-112-done.md` et le rapport architecte S50.
⚠ **Re-vérifie ce point toi-même** — c'est un critère d'acceptation, et un « non exposée » affirmé
sans preuve ne vaut rien. Si tu trouves le contraire, dis-le : le lead peut s'être trompé.

**4. `docs/memory/devops/external-services-inventory.md` N'EXISTE PAS.** Le rapport architecte
affirme qu'il existe (« procédure §3quater → la dépendance F3 est levée »). C'est faux :
`docs/memory/devops/` ne contient QUE `secret-rotation-runbook.md`. C'est un **chemin fantôme**,
le 3ᵉ sprint consécutif sur ce projet. Le runbook, lui, référence ce fichier « créé par #250 quand
dispo » — #250 n'est pas faite.

### ⇒ TON PÉRIMÈTRE (validé par le dev)

Tu livres **de la documentation et un audit**, PAS une rotation :

1. **Rapport d'audit d'exposition** — `docs/memory/audits/secret-exposure-audit.md` (nouveau).
   Pour chaque secret (`DB_PASSWORD`, `JWT_SECRET`, `BREVO_API_KEY`) : exposé ou non, où
   (`commit:fichier:ligne`), longueur de la valeur, **jamais la valeur**. Méthode reproductible
   (les commandes exactes que tu as lancées, écrites de façon à ne pas faire fuiter de valeur).
   Conclusion par secret : à rotationner au déploiement / sans objet.

2. **Correction de `docs/memory/devops/secret-rotation-runbook.md`** :
   - statut réel et daté (toujours non exécuté, projet non déployé — confirme-le toi-même) ;
   - `BREVO_API_KEY` : conclusion de l'audit intégrée (étape « vérifier l'exposition » → résolue) ;
   - le lien vers `external-services-inventory.md` doit cesser d'être un chemin fantôme (tu crées
     le fichier, cf. point 3) ;
   - ⚠ la section `JWT_SECRET` doit être **mise en cohérence avec #323** (vague 2 de ce sprint) :
     `JWT_SECRET` ne sera pas « rotationné », il sera **supprimé** au profit d'une paire de clés
     RS256, et un secret dédié `EXPORT_TOKEN_SECRET` apparaît pour `ExportTokenService`. Écris cette
     section au futur/conditionnel en renvoyant à #323 — **ne préempte pas** le travail de l'autre
     agent, ne touche à aucun fichier backend.

3. **Création de `docs/memory/devops/external-services-inventory.md`** — le fichier manquant.
   Inventaire des services externes réellement utilisés par ce projet, **constaté dans le code**,
   pas deviné : au minimum PostgreSQL et Brevo (`BrevoEmailService`) ; cherche s'il y en a d'autres
   (`grep` sur les clients HTTP / `@Value` / variables d'environnement). Avec une **§3quater
   « procédure de rotation par service »** — c'est la section que le runbook et la règle globale du
   dev référencent déjà par ce nom exact.
   Le fichier est aussi référencé par la règle de sécurité globale du poste comme *« Procédure de
   rotation par service : docs/memory/devops/external-services-inventory.md §3quater »* — respecte
   ce nom et cette numérotation.

4. **Ne coche PAS les critères d'acceptation opérationnels de #249** et **ne ferme pas l'issue**.
   Elle reste ouverte : la rotation effective revient au dev, au déploiement prod. Dis-le
   explicitement dans ton retour et dans le runbook.

## Plan d'implementation (architect, /sprint plan — ⚠ partiellement périmé, cf. ci-dessus)
...
⚠ Deux erreurs dans ce bloc : (1) `external-services-inventory.md` **n'existe pas** — c'est toi qui
le crées ; (2) `JwtService.java` n'est **pas** dans ton périmètre, il appartient à #323 en vague 2.
Le reste (`ordre_ecriture` vague 1/vague 2, règle secrets) tient.
...
- **Vague 1.** Tu tournes en parallèle de #322 (garde Host), qui touche
  `frontend/middleware.ts`, `frontend/middleware.test.ts`, `frontend/e2e/auth-guard.spec.ts` et
  `docs/adr/ADR-004-garde-serveur-middleware.md`. Aucun chevauchement avec toi.
- **#323 (RS256) est en vague 2** et livrera : `JwtService` en RS256, `ExportTokenService` sur un
  secret dédié `EXPORT_TOKEN_SECRET`, et la suppression de `JWT_SECRET` de la configuration.
  Ta section `JWT_SECRET` du runbook doit **renvoyer à #323**, pas décrire une rotation HS256
  qui n'aura pas lieu.

## Fichiers que tu PEUX toucher

- `docs/memory/devops/**` (runbook + nouvel inventaire)
- `docs/memory/audits/secret-exposure-audit.md` (nouveau)

## Fichiers que tu NE DOIS PAS toucher

- `backend/**` et `frontend/**` — **aucun fichier de code**. Ton livrable est documentaire.
- `docs/adr/**` — réservé à #322 (vague 1, en parallèle) et #323 (vague 2).
- `docs/memory/sprint-history.md` — le lead seul y écrit.
- `.env.example`, `application*.properties` — ne les modifie pas ; si tu constates un défaut,
  signale-le en `RECOMMAND_FOLLOWUP` au lieu de le corriger.
...
- **Commit** : 1 commit logique, message gitmoji en français.
  `git add` **ciblé sur tes fichiers** — JAMAIS `git add -A` ni `git add .` : un autre agent
  travaille dans le même working tree en parallèle.
- **Vérifie ton propre commit avant de conclure** : `rtk proxy git show --stat HEAD` doit ne
  contenir QUE tes fichiers. Si tu vois des fichiers de l'autre agent, tu as fait un `add` trop
  large — signale-le immédiatement, ne tente pas de « réparer » son travail.
- **Tests** : pas de test applicatif à écrire (livrable documentaire). En revanche, les commandes
  d'audit que tu documentes doivent être **réellement exécutables et exécutées** — ne documente pas
  une commande que tu n'as pas lancée.
- **Docs en français** (convention projet), code/identifiants en anglais.

## Honnêteté attendue

- Tout ce que tu affirmes dans l'audit doit être **mesuré**, avec la commande à l'appui.
  « Non exposé » sans preuve n'a aucune valeur.
- Si tu ne peux pas déterminer si un secret a été exposé (ex. historique réécrit, blob inaccessible),
  écris **« indéterminé »** et pourquoi. N'invente pas une conclusion rassurante.
- Liste explicitement ce qui reste NON fait (les 3 critères opérationnels de #249, notamment).
- Ne qualifie pas ton travail de « complet ».

## Livrable attendu (format strict, MAX 500 tokens, style caveman — pas de prose)

Écris ton bilan dans `docs/memory/sprints/sprint-50/issue-249-done.md` ET renvoie-le :

```
RETOUR :
- commits: [SHA1, ...]
- audit: DB_PASSWORD=<expose oui/non + où> | JWT_SECRET=<...> | BREVO_API_KEY=<...>
         (longueurs et emplacements uniquement — AUCUNE valeur)
- fichiers livres: <liste>
- services externes inventories: <liste constatée dans le code>
- criteres acceptation #249: <chacun des 5 → SATISFAIT / NON + raison>
- reste a faire par le dev: <les actions opérateur, précisément>
- [MEMORY:*] signaux: <pitfall / decision si applicable>
- recommandations suite: <RECOMMAND_FOLLOWUP éventuels, ex. #250, purge historique #112>
```
