...
## Issue
[FEATURE] Sécuriser la persistance auth : sortir le user PII du localStorage (A17)
...
Aujourd'hui, certaines informations personnelles de l'utilisateur (email, nom d'utilisateur, nom) sont conservées en clair dans le stockage local du navigateur (localStorage). Si une faille permettait d'injecter du code malveillant dans la page (XSS), ce code pourrait lire directement ces informations personnelles. Le jeton de connexion (JWT), lui, est déjà protégé correctement (cookie HttpOnly, donc invisible pour un script). Il reste donc une surface d'exposition sur les données personnelles elles-mêmes.

## À faire
`AuthContext` (frontend/src/contexts/AuthContext.tsx) stocke l'objet `user` complet (PII : email, username, name) en clair dans localStorage, lisible par tout payload XSS. Le JWT est déjà en cookie HttpOnly et n'est pas concerné par ce risque.

À évaluer puis implémenter une des deux options :
1. Supprimer entièrement le miroir localStorage du user : passer en session httpOnly-only et re-fetch `/me` au montage de l'application
2. Restreindre le contenu persisté en localStorage aux champs non sensibles uniquement (ex. exclure email/name, garder un identifiant non sensible si nécessaire à l'UI)

Anti-pattern connu et référencé (A17), signalé par l'agent security-expert et par le reviewer pendant le Sprint 7, et reporté volontairement hors scope à ce moment-là.

## BR impactées
Aucune

## Critères d'acceptation
- [ ] Décision actée entre l'option 1 (re-fetch /me au mount) et l'option 2 (restriction des champs persistés)
- [ ] Plus aucune donnée PII sensible (email, name) n'est lisible en clair dans localStorage après connexion
- [ ] Le comportement applicatif (persistance de session au refresh de page, affichage du profil) reste fonctionnel après le changement
- [ ] Tests frontend couvrant le nouveau comportement de persistance/restauration de session

## Piste technique
- `frontend/src/contexts/AuthContext.tsx`
- Tout composant lisant directement le user depuis localStorage (à rechercher dans le frontend)
- Endpoint `/api/me` pour le re-fetch si option 1 retenue

## Dépendances
Aucune

## Risques techniques
Si l'option 1 (re-fetch systématique) est retenue, attention à l'effet sur le temps de chargement initial perçu (flash non authentifié avant la réponse de /me) — prévoir un état de chargement adapté. Vérifier aussi tous les endroits du frontend qui lisent actuellement le user depuis localStorage pour ne rien casser.

## Estimation
M (1-3 jours) — nécessite une décision d'architecture (session vs persistance partielle), une modification de AuthContext et la vérification de tous les consommateurs du contexte côté frontend, plus les tests associés.
...
- AUCUNE dépendance sur #44 (fichiers 100% disjoints : frontend TS vs backend Java/SQL).
- NE PAS toucher au backend (réservé #44).

## Décision d'architecture requise AVANT code
Trancher entre les deux options (documenter le choix dans le retour) :
  1. **Re-fetch /me au mount** : supprimer entièrement le miroir localStorage du user, re-fetcher `/api/me` au montage de l'app. Attention au flash non-authentifié → prévoir un état de chargement (loading) adapté.
  2. **Restriction des champs persistés** : ne garder en localStorage que des champs non sensibles (exclure email/name).
Recommandation architect : le pont /me existe déjà (PAT-S7-004, hook useCurrentUser). L'option 1 est cohérente avec l'existant mais vérifier le flash. Le dev tranche selon l'état réel du code.

## Point dur
- Rechercher TOUS les consommateurs qui lisent le user depuis localStorage : `grep -rn "localStorage" frontend/src/` et vérifier qu'aucun ne casse.
- Le JWT est déjà en cookie HttpOnly (NON concerné) — ne pas y toucher.

## Designer
Non applicable (pas de nouveau composant visuel — comportement de persistance/session uniquement).

## Contraintes
- Branche cible : sprint/9 (déjà checkout, NE PAS changer de branche).
- Commit : 1 commit logique gitmoji français (ex: `:lock: Sortir le user PII du localStorage (#135, A17)`).
- Tests frontend OBLIGATOIRE via ./scripts/test-quiet.sh frontend (couvrir persistance/restauration session + absence PII localStorage après login).
- Fichier de test attendu : frontend/src/contexts/AuthContext.test.tsx (existant à étendre).
- Si volume tests > 500 OU temps > 3min : signaler RECOMMAND_TEST_RUNNER.
- Ne PAS toucher aux fichiers backend (réservés #44).

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: <option retenue (1 ou 2) + fichiers cles + consommateurs localStorage vérifiés + tests passed/failed>
- [MEMORY:decision] option retenue pour la persistance auth (si structurant)
- [MEMORY:pitfall] si flash non-authentifié ou piège localStorage découvert
- recommandations suite: <RECOMMAND_SECURITY si doute exposition PII / RECOMMAND_FOLLOWUP / autre>
