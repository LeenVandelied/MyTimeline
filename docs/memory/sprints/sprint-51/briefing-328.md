[BRIEFING ISSUE #328 — Sprint 51, Vague 1]
...
**#328 — [BUG] Le scroll horizontal de la frise est perdu à la rotation portrait ↔ paysage**
Labels : `bug`, `epic:events`, `priority:P1`, `size:M`, `frontend`

### Contexte
Sur mobile, quand on tourne le téléphone (portrait ↔ paysage), la frise (timeline) affiche à
nouveau son tout début au lieu de rester là où l'utilisateur en était. Le zoom et l'élément
sélectionné, eux, sont bien conservés — seul le défilement horizontal est perdu. Concrètement :
l'utilisateur fait défiler la frise, tourne son téléphone, et se retrouve ramené au point de départ.

### À faire
`useTimelineMobileState` fait remonter `zoom` et `viewportStart` en state React, ce qui les
préserve car `TimelineResponsive` reste monté pendant la rotation. En revanche `scrollLeft` est un
état **DOM**, porté par l'élément de la variante (portrait ou paysage) qui vient d'être démontée
lors du switch de layout — il est donc perdu. `scrollToToday` n'est câblé qu'au **montage** du hook
et ne rejoue pas lors du changement de variante, donc rien ne vient recaler le scroll de la nouvelle
variante montée.

Effet de bord : `viewportStart` garde son ancienne valeur, ce qui désynchronise la fenêtre affichée
par la minimap de ce que l'utilisateur voit réellement à l'écran.

Mesuré : `scrollLeft` passe de **400 à 0** après rotation.

### BR impactées
Aucune.

### Critères d'acceptation
- [ ] Après rotation portrait → paysage (et l'inverse), la position de scroll horizontal de la frise est restaurée (ou recalculée de façon cohérente avec `viewportStart`)
- [ ] La fenêtre affichée par la minimap reste synchronisée avec le scroll visible après rotation
- [ ] Le zoom et la sélection restent conservés comme aujourd'hui (pas de régression)
- [ ] Un test E2E de rotation vérifie explicitement la conservation du `scrollLeft` (et pas seulement de la sélection)

### Risques techniques (énoncé de l'issue)
Le fix doit éviter de recaler le scroll uniquement au montage (comportement actuel de
`scrollToToday`) — il faut un mécanisme qui réagisse au changement de variante lui-même, pas
seulement au montage initial du hook.
...
Ne les re-vérifie pas à l'aveugle, mais **confirme-les avant d'écrire** — les numéros de ligne
bougent dès la première édition.

- `useTimelineMobileState.ts` fait **230 lignes**. `const [viewportStart, setViewportStart] = useState(0)` à la **ligne 91**.
- `scrollLeft` est resté **exclusivement DOM** : lu ligne **140** (`setViewportStart(el.scrollLeft / railWidth)`), écrit lignes **173** (`el.scrollLeft = start * railWidth`) et **183** (`el.scrollLeft = Math.max(0, target)`).
- `scrollToToday` défini ligne **179**, appelé ligne **188** — **au seul montage**. C'est la cause exacte décrite par l'issue, confirmée par lecture.
- Le commentaire ligne **40** documente déjà l'intention : « `viewportStart` / `zoom.level` restent en state React et NE sont PAS réinitialisés ».
- `TimelineResponsive.tsx` fait **104 lignes** ; son commentaire d'en-tête (~ligne 32) documente « La rotation portrait ↔ paysage démonte/remonte la variante mais PAS l'état (il vit au-dessus) » — l'intention était déjà là, `scrollLeft` est le seul morceau d'état qui n'a pas suivi.
- `TimelineResponsive` instancie `useTimelineMobileState(props.events, props.resources, props.locale, props.today)` et passe `state` aux deux variantes `TimelineMobilePortrait` / `TimelineMobileLandscape`.
- Spec E2E de rotation existante : `frontend/e2e/timeline-mobile.spec.ts` (15,5 Ko) — c'est le fichier livré en #205 à étendre.
...
- **Vague 1** : tu travailles **en parallèle** d'un autre agent sur l'issue #349 (perf de la frise, fichiers `TimelineView.tsx` + `lib.ts`). Aucune dépendance fonctionnelle entre vous, mais **working tree partagé**.
- **Vague 2 (après toi)** : #351 touchera `TimelineView.tsx` et `useTimelineViewport.ts`. Aucun blocage pour toi.
...
- `frontend/src/components/timeline/useTimelineMobileSelection.ts` / `useTimelineMobileGestures.ts` (si strictement nécessaire)
- `frontend/e2e/timeline-mobile.spec.ts`
...
- ❌ `frontend/src/components/timeline/TimelineView.tsx` → issue #349, en cours **en ce moment même**
- ❌ `frontend/src/components/timeline/lib.ts` → issue #349, en cours **en ce moment même**
- ❌ `frontend/src/components/timeline/TimelineView.perf.stories.tsx` / `stress-fixtures.ts` → issue #349
- ❌ `frontend/src/components/timeline/useTimelineViewport.ts` → issue #351 (vague 2)
- ❌ `frontend/src/components/calendar/TimelineCalendar.tsx` et `timeline/index.ts` → issue #350 (vague 2)
- ❌ `docs/memory/sprint-history.md` → le lead seul l'écrit

Si ton correctif **exige** de toucher un fichier interdit : **ne le fais pas**. Retourne
`STATUS: PARTIAL` + `BLOQUE_SUR: besoin de modifier <fichier> (périmètre #349/#351)` et explique
en 2 lignes ce qu'il faudrait y changer. Le lead arbitrera en vague 2.
...
- **1 commit logique**, message gitmoji en **français** (convention projet). Ex : `:bug: fix(timeline): restaure le scroll horizontal à la rotation portrait ↔ paysage`.
- **Ne pousse pas** (`git push`) — le lead s'en charge en Phase 9.

### Tests (OBLIGATOIRE — pas de « je pense que ça passe »)
```bash
cd /Users/herrh/VSProjects/MyTimeline/.claude/worktrees/new-feature-2347-14cb9a
./scripts/test-quiet.sh frontend      # suite unitaire (vitest)
cd frontend && npm run typecheck      # tsc --noEmit, strict
```
E2E (`npm run test:e2e`) : recette locale connue **fonctionnelle** (49/49 en 38 s), mais **4 pièges mesurés** :
1. le frontend doit tourner sur le **port 3000** exactement — un CORS sur un autre port se déguise en erreur « rate-limit » trompeuse ;
2. lancer avec **`--workers=1`** — au-delà, plusieurs specs `settings` rougissent par interférence, faux négatif ;
3. la base doit être amorcée (docker compose, override port 5432) ;
4. si le volume dépasse ~500 tests ou 3 min, **n'insiste pas** : retourne `RECOMMAND_TEST_RUNNER` et le lead spawnera un test-runner dédié.
...
- ⚠ **Une CI verte ne prouve pas que la page est correcte** : jsdom ne résout ni `@layer` ni le layout. Ici l'enjeu est du scroll DOM à travers un remontage — la suite unitaire seule ne le démontre pas. L'assertion E2E sur `scrollLeft` est le **vrai** critère.
...
## Designer
Non applicable — correctif de comportement, aucun changement visuel.

## Livrable attendu (format strict, MAX 500 tokens, style caveman — pas de prose)

Écris ton rapport et termine par le STATUS. Format :

```
pack_lu: OUI — <pack> §<section>
commits: [SHA1]
resume: <objectif + fichiers clés + ce qui a réellement changé + tests lancés et résultats CHIFFRÉS>
mesures: scrollLeft avant/après rotation (valeurs réelles observées, pas estimées)
[MEMORY:*] signaux: <pitfall / pattern / décision — si applicable>
recommandations suite: <RECOMMAND_TEST_RUNNER | RECOMMAND_UI_DESIGN | RECOMMAND_FOLLOWUP: <desc> [triage XS|S|M|L] | ou "aucune">
