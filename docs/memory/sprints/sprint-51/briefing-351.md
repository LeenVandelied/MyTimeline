[BRIEFING ISSUE #351 — Sprint 51, Vague 2]
...
Si `pwd` ou la branche ne correspondent pas : **ARRÊTE-TOI**, retourne `STATUS: PARTIAL` +
`BLOQUE_SUR: mauvais worktree`. **Ne fais JAMAIS `git checkout` / `git switch`** — le working tree
est PARTAGÉ avec un autre agent qui travaille en parallèle sur l'issue #350.

## Issue

**#351 — [BUG] Frise : deux défauts d'implémentation relevés en review**
Labels : `bug`, `epic:events`, `priority:P3`, `size:XS`, `frontend`

La revue du Sprint 49 a relevé deux défauts sur la frise, tous deux **hors périmètre du S49** et
donc non corrigés. Ils sont regroupés ici parce qu'ils relèvent du même lot de code.

### Défaut 1 — Structure d'accessibilité des cales de virtualisation

Les cales (`spacer`) qui préservent la hauteur totale de la frise sont des `<div>` **enfants directs
d'un conteneur `role="list"`**, sans `role="presentation"` ni `role="listitem"` — elles ne portent
qu'un `aria-hidden`. La règle `aria-required-children` exige qu'un `role="list"` n'ait que des
`listitem` comme enfants ; `aria-hidden` seul ne suffit pas à retirer un nœud de la structure
attendue selon les technologies d'assistance. Le résultat est **fragile plutôt que franchement
cassé**.

→ Poser `role="presentation"` (ou `role="none"`) sur les deux cales.

### Défaut 2 — Écouteur de défilement posé trop large

L'écouteur `scroll` est enregistré **en phase de capture sur `window`**. Conséquence : il se
déclenche pour **n'importe quel élément défilant de l'application** — dialogues, tiroirs, listes de
réglages — dès qu'une frise est montée quelque part. Chaque déclenchement programme une
`requestAnimationFrame` et une lecture de layout, donc des rafales de travail inutile pendant que
l'utilisateur fait défiler tout autre chose.

→ Filtrer sur l'élément défilant réel (`scrollEl`) ou sur `document` plutôt que capturer sur `window`.

### BR impactées
Aucune.

### Critères d'acceptation
- [ ] Les deux cales portent `role="presentation"`
- [ ] Un audit d'accessibilité automatisé sur l'écran frise ne signale plus `aria-required-children`
- [ ] Faire défiler un dialogue ou un tiroir, frise montée, **ne déclenche plus** le rappel de `useTimelineViewport` (vérifiable par compteur temporaire ou test unitaire)
- [ ] Le défilement de la frise elle-même reste correct : fenêtrage, minimap et resync inchangés
- [ ] Aucune fuite : l'écouteur est retiré au démontage
- [ ] Suites unitaire et E2E au vert

### Risques techniques (énoncé de l'issue — c'est le vrai enjeu de cette issue)
Le passage de `window` en capture vers un scroller ciblé **peut faire perdre des événements si la
frise est imbriquée dans un conteneur défilant** : vérifier le comportement **dans un tiroir et en
plein écran**. C'est la seule raison pour laquelle une issue de 3 lignes n'est pas triviale.
...
## ⚠ CHEMIN FANTÔME DANS L'ÉNONCÉ DE L'ISSUE — déjà corrigé, ne t'y laisse pas prendre

L'issue cite **`frontend/src/hooks/useTimelineViewport.ts`** : **ce fichier N'EXISTE PAS.**
Le vrai chemin est **`frontend/src/components/timeline/useTimelineViewport.ts`** (282 lignes).
Vérifié par le lead le 2026-07-29. Ce projet a connu 4 sprints consécutifs de chemins inventés — ne
crée surtout pas le fichier « manquant ».

## Constats VÉRIFIÉS PAR LE LEAD, APRÈS les commits de la vague 1 (2026-07-29)

⚠ **Les numéros de ligne de l'issue sont périmés.** L'issue annonce `TimelineView.tsx:754` et `:847` ;
l'architecte annonçait 756/849. Depuis, l'issue #349 (vague 1, commit `1cb6031`) a **profondément
remanié `TimelineView.tsx`, passé de 879 à 1113 lignes**. Positions réelles à ce jour :

- `<div role="list" aria-label={category} data-testid="timeline-lane-list">` → ligne **1055**
- 1ʳᵉ cale : `aria-hidden="true"` ligne **1058**, `data-testid="timeline-lane-spacer"` ligne **1059**
- 2ᵈᵉ cale : `aria-hidden="true"` ligne **1084**

**Localise-les par `grep data-testid="timeline-lane-spacer"`, pas par numéro de ligne.**

- `useTimelineViewport.ts` : **non touché par la vague 1**. Ligne **206** exactement comme décrit :
  `window.addEventListener('scroll', schedule, { passive: true, capture: true })`, ligne 207
  `window.addEventListener('resize', schedule)`, nettoyage lignes **209-210** (les deux
  `removeEventListener` sont bien présents — le critère « aucune fuite » est **déjà satisfait
  aujourd'hui**, ne le casse pas).

## Constat de l'agent #349 à traiter dans TON périmètre (remonté en vague 1)

L'agent qui a livré #349 a trouvé que **`useTimelineViewport.sync()` republie un objet `metrics`
neuf même quand les 3 hauteurs sont identiques**, ce qui cassait l'efficacité de `React.memo` sur
les lanes. Il l'a **contourné côté `TimelineView.tsx`** (figeage sur la valeur) faute d'avoir le
droit de toucher `useTimelineViewport.ts` — qui est **ton** fichier.

→ **Corrige-le à la source** si tu le juges sûr : ne republier `metrics` que si l'une des 3 hauteurs
a réellement changé. **Ne retire pas** le contournement posé dans `TimelineView.tsx` par #349 (il
reste correct et sert de ceinture) sauf si tu démontres par les tests qu'il devient inutile — dans
ce cas, dis-le explicitement dans ton retour.

## Triage
Taille: XS (mais vérification comportementale non triviale — cf. risques)
Modèle: opus
Effort: high
...
- **Vague 1 livrée** (déjà sur ta branche) : #328 (`5210ed5`, scroll restauré à la rotation) et #349 (`1cb6031`, mémoïsation des lanes + zoom incrémental). Tu travailles **par-dessus** ces deux commits.
- **Vague 2, en parallèle de toi** : un autre agent traite #350 (suppression de `TimelineCalendar.tsx`, code mort). Fichiers disjoints des tiens, mais **working tree partagé**.
- Aucune migration Flyway. Aucun DTO, aucun Zod.

## Périmètre fichiers — MATRICE STRICTE

**Tu es autorisé à modifier :**
- `frontend/src/components/timeline/TimelineView.tsx` (**uniquement** les `role`/`aria` des deux cales `timeline-lane-spacer` — ne re-travaille pas la mémoïsation livrée par #349)
- `frontend/src/components/timeline/useTimelineViewport.ts`
- tout fichier de test unitaire que tu crées pour cette issue

**INTERDIT de modifier :**
- ❌ `frontend/src/components/calendar/TimelineCalendar.tsx`, `frontend/src/components/timeline/index.ts`, `frontend/src/components/timeline/lib.ts`, `frontend/src/components/timeline/TimelineEditHost.tsx`, `frontend/src/styles/ds/readme.md` → **issue #350, en cours en ce moment même**
- ❌ `frontend/src/components/timeline/useTimelineMobileState.ts`, `TimelineResponsive.tsx`, `TimelineMobilePortrait.tsx`, `TimelineMobileLandscape.tsx`, `frontend/e2e/timeline-mobile.spec.ts` → livrés par #328, ne les défais pas
- ❌ `frontend/src/components/timeline/zoom.ts`, `zoom-incremental.test.ts` → livrés par #349
...
`STATUS: PARTIAL` + `BLOQUE_SUR:`.

## Contraintes d'exécution

- **Branche `sprint/51`, DÉJÀ checkout.** Aucun `checkout`/`switch`/`pull`/`rebase`/`merge`/`stash`.
- **`git add` CIBLÉ, chemin par chemin. JAMAIS `git add -A`, `git add .`, ni `git commit -a`** — un agent travaille en parallèle dans le même working tree.
- **1 commit logique**, gitmoji **français**. Ex : `:wheelchair: fix(timeline): role=presentation sur les cales et écouteur de défilement ciblé`.
- **Ne pousse pas.**
- ⚠ Si tu vois `frontend/.eslintcache` modifié/supprimé dans `git status` : **ne le commite pas et ne le restaure pas**, c'est un artefact partagé (le lead le gère).

### Vérification comportementale — le cœur de cette issue
Le défaut 2 est **3 lignes de code mais un vrai risque de régression**. Les critères d'acceptation
en exigent la démonstration, pas l'affirmation :

1. **Le scroll de la frise elle-même reste correct** : fenêtrage, minimap, resync inchangés.
2. **Le scroll d'un tiroir/dialogue, frise montée, ne déclenche plus le rappel** — l'issue autorise
   explicitement un **compteur temporaire ou un test unitaire** pour l'établir. Écris le test.
3. **Frise imbriquée dans un conteneur défilant** (tiroir, plein écran) : vérifie que l'événement
   n'est **pas perdu**. C'est le risque nommé par l'issue. Si tu ne peux pas le vérifier, dis-le dans
   `non_verifie:` — ne l'affirme pas.

### Tests (OBLIGATOIRE)
```bash
cd /Users/herrh/VSProjects/MyTimeline/.claude/worktrees/new-feature-2347-14cb9a
./scripts/test-quiet.sh frontend      # vitest — 814 tests verts au départ, ne régresse pas
cd frontend && npm run typecheck
```
La suite frontend fait **814 tests** (> seuil 500) : si le temps dépasse ~3 min, retourne
`RECOMMAND_TEST_RUNNER` plutôt que d'insister.

Pour le critère « audit d'accessibilité ne signale plus `aria-required-children` » : si aucun
outil d'audit a11y automatisé n'est câblé dans ce dépôt, **ne prétends pas l'avoir passé** —
constate-le et écris-le dans `non_verifie:`.

### Pièges d'outillage sur ce poste (mesurés, ne les redécouvre pas)
- ⚠ **Le hook RTK avale la sortie de `git diff`, `wc -c`, et même `vitest`/`grep` redirigés** (3 manifestations mesurées ce sprint). Contournement : `rtk proxy <cmd>`. **Ne conclus jamais « aucun changement » sur un `git diff` vide.**
- ⚠ `detect-domain.sh` et `check-prereq.sh` sont **cassés**. Ne les appelle pas.
- ⚠ **Une CI verte ne prouve pas que la page est correcte** : jsdom ne résout ni `@layer` ni le layout.
- ⚠ Écrire un fichier sous `frontend/` pendant qu'un Storybook/Vite tourne déclenche un full-reload HMR qui tue tout contexte Playwright attaché (piège relevé par #349).

### Checkpoint de lecture du pack
Le context-pack ci-dessus est **inliné dans ce briefing** — aucun fichier externe à ouvrir.
Ouvre ton retour par `pack_lu: OUI — <pack> §<titre d'une section réelle que tu as utilisée>`.
Si le pack ne t'a servi à rien : `pack_lu: OUI — sans usage (raison en 5 mots)`. Ne l'invente pas.

## Designer
Non applicable — accessibilité et écouteur d'événement, aucun changement visuel.
...
commits: [SHA1]
resume: <ce qui a changé pour le défaut 1 ET le défaut 2 + tests lancés et résultats CHIFFRÉS>
verification_comportementale:
  scroll frise: <constat>
  scroll tiroir/dialogue frise montée: <déclenche encore ? preuve — compteur ou test>
  frise imbriquée dans conteneur défilant: <constat ou NON VÉRIFIÉ>
  metrics republié par sync(): <corrigé à la source ? contournement #349 conservé ?>
[MEMORY:*] signaux: <si applicable>
recommandations suite: <RECOMMAND_* | RECOMMAND_FOLLOWUP: <desc> [triage] | ou "aucune">
non_verifie: <liste explicite — obligatoire, "rien" interdit sans justification>
STATUS: COMPLETED
```

Dernière ligne = `STATUS: COMPLETED` (ou `STATUS: PARTIAL` avec `BLOQUE_SUR:` juste au-dessus).
