[BRIEFING ISSUE #349 — Sprint 51, Vague 1]

## Garde-fou d'environnement (À EXÉCUTER EN PREMIER, AVANT TOUTE LECTURE DE CODE)

Tu travailles dans un **worktree git**, PAS dans le dépôt principal. Un défaut de `cwd` sur
`/Users/herrh/VSProjects/MyTimeline` te ferait lire et modifier les MAUVAIS fichiers et
produirait un faux KO (piège mesuré sur ce projet).

```bash
cd /Users/herrh/VSProjects/MyTimeline/.claude/worktrees/new-feature-2347-14cb9a
pwd
git rev-parse --abbrev-ref HEAD   # DOIT afficher : sprint/51
```

Si `pwd` ou la branche ne correspondent pas : **ARRÊTE-TOI** et retourne
`STATUS: PARTIAL` + `BLOQUE_SUR: mauvais worktree ou mauvaise branche`.
**Ne fais JAMAIS `git checkout` / `git switch`** — le working tree est PARTAGÉ avec un
autre agent qui travaille en parallèle ; changer de branche casserait son travail.

## Issue

**#349 — [FEATURE] Frise : lisser les dernières saccades de défilement et alléger les recalculs de zoom**
Labels : `enhancement`, `epic:events`, `priority:P2`, `size:S`, `frontend`

### Contexte
La frise chronologique se figeait lors d'un défilement rapide sur les grandes collections
d'événements. La virtualisation livrée au Sprint 49 (#69) a supprimé le blocage : la frame la plus
lente est passée de **133,4 ms à 33,4 ms**.

Il reste un écart avec la fluidité cible : sur un défilement violent (7 200 px/s), **7 à 10 frames
sur 89 dépassent 16,7 ms**, soit le budget d'une image à 60 images/seconde. Le ressenti n'est plus un
blocage mais une légère saccade. Cette issue vise à la supprimer.

### À faire
Deux pistes, toutes deux mesurées au S49 :

**1. Mémoïser les lanes (`React.memo`).** Aujourd'hui, franchir une frontière de bande de
virtualisation réconcilie **toutes** les lanes visibles. Avec une lane mémoïsée sur ses props,
seules les lanes **entrantes et sortantes** seraient reconstruites.

**2. Rendre incrémentaux les calculs de zoom.** `computeRange`, `positionEvents` et
`buildMinimapBuckets` parcourent **la totalité des événements à chaque changement de zoom**, en O(n)
— environ **5 ms pour 1 000 événements** (mesuré). Une mémoïsation incrémentale (cache invalidé par
plage plutôt que recalcul global) est possible.

### BR impactées
Aucune.

### Critères d'acceptation
- [ ] Sur le banc `TimelineView.perf.stories.tsx` à 7 200 px/s, **au plus 2 frames sur 89** dépassent 16,7 ms (contre 7 à 10 aujourd'hui)
- [ ] La frame la plus lente reste ≤ 33,4 ms (pas de régression sur l'acquis du S49)
- [ ] Le coût de recalcul au zoom pour 1 000 événements est mesuré **avant/après** et documenté dans l'issue
- [ ] Aucun événement manqué ni doublon en bord de bande de virtualisation (le fenêtrage fermé validé au S49 reste correct)
- [ ] Navigation clavier et `aria-setsize` inchangés (`windowEvents` doit continuer à porter l'index du modèle complet)
- [ ] Aucune fuite : rAF annulé, écouteurs retirés, timer de resync purgé au démontage

### Risques techniques (énoncé de l'issue)
- `React.memo` sur une lane est inefficace si une prop est recréée à chaque rendu parent (callback ou objet littéral) : **vérifier la stabilité des props avant de conclure à un gain**.
- Le débounce de 400 ms garde volontairement la bande **trop large** plutôt que trop étroite. Toute mémoïsation incrémentale doit préserver cette marge de sécurité, sinon on réintroduit des trous de frontière.
- Optimisation sans régression fonctionnelle attendue : c'est un travail **à mesurer, pas à estimer**. **Sans chiffres avant/après, l'issue n'est pas terminée.**
...
## Constats VÉRIFIÉS PAR LE LEAD sur le code réel (2026-07-29, HEAD 47730f9)

- `frontend/src/components/timeline/TimelineView.tsx` : **879 lignes** — présent.
- `frontend/src/components/timeline/lib.ts` : **280 lignes** — présent.
- Banc de mesure **présent et rejouable** : `frontend/src/components/timeline/TimelineView.perf.stories.tsx` (104 lignes) + `frontend/src/components/timeline/stress-fixtures.ts` (94 lignes).
- ADR de référence **présent** : `docs/adr/ADR-007-virtualisation-timeline.md` (273 lignes) — **lis-le avant de toucher au fenêtrage**, il fixe le budget perf et justifie la marge du débounce 400 ms.
- `<div role="list" aria-label={category} data-testid="timeline-lane-list">` à la ligne **753**, cales `data-testid="timeline-lane-spacer"` aux lignes **757** et **850**.
  ⚠ **NE TOUCHE PAS aux `role` de ces cales** — c'est le périmètre exclusif de l'issue #351 (vague 2). Tu peux modifier le rendu des lanes autour, mais laisse les attributs `role`/`aria-hidden` des spacers strictement inchangés.
...
- **Vague 1** : tu travailles **en parallèle** d'un autre agent sur l'issue #328 (scroll perdu à la rotation, fichiers `useTimelineMobileState.ts` + `TimelineResponsive.tsx` + variantes mobiles). Aucune dépendance fonctionnelle entre vous, mais **working tree partagé**.
- **Vague 2 (après toi)** : #351 reprendra `TimelineView.tsx` (les `role` des cales) et `useTimelineViewport.ts`. **Laisse-lui ces deux points intacts** (détail ci-dessous).
- Aucune migration Flyway dans ce sprint. Aucun changement de schéma, aucun DTO, aucun Zod.

## Périmètre fichiers — MATRICE STRICTE

**Tu es autorisé à modifier :**
- `frontend/src/components/timeline/TimelineView.tsx`
- `frontend/src/components/timeline/lib.ts`
- `frontend/src/components/timeline/TimelineView.perf.stories.tsx`
- `frontend/src/components/timeline/stress-fixtures.ts`
- `frontend/src/components/timeline/zoom.ts` (si `computeRange` / `positionEvents` / `buildMinimapBuckets` y vivent — vérifie où ils sont réellement définis)
- tout fichier de test unitaire que tu crées pour cette issue

**INTERDIT de modifier (périmètre d'un autre agent — conflit garanti) :**
- ❌ `frontend/src/components/timeline/useTimelineMobileState.ts` → issue #328, en cours **en ce moment même**
- ❌ `frontend/src/components/timeline/TimelineResponsive.tsx`, `TimelineMobilePortrait.tsx`, `TimelineMobileLandscape.tsx` → issue #328
- ❌ `frontend/e2e/timeline-mobile.spec.ts` → issue #328
- ❌ `frontend/src/components/timeline/useTimelineViewport.ts` → issue #351 (vague 2)
- ❌ Les attributs `role` / `aria-hidden` des cales `data-testid="timeline-lane-spacer"` (lignes ~757 et ~850) → **périmètre exclusif de #351**. Tu peux restructurer le rendu des lanes autour, mais ne « corrige » pas ces `role` au passage : #351 le fera et tu créerais un conflit de merge inutile.
- ❌ `docs/memory/sprint-history.md` → le lead seul l'écrit

Si ton optimisation **exige** de toucher un fichier interdit : **ne le fais pas**. Retourne
`STATUS: PARTIAL` + `BLOQUE_SUR: besoin de modifier <fichier> (périmètre #328/#351)`.

## Contraintes d'exécution

- **Branche cible : `sprint/51`, DÉJÀ checkout.** Ne fais aucun `checkout`, `switch`, `pull`, `rebase`, `merge`, `stash`. Le working tree est partagé avec un agent parallèle.
- **Commit : `git add` CIBLÉ, fichier par fichier. JAMAIS `git add -A`, JAMAIS `git add .`, JAMAIS `git commit -a`.** Un `add -A` embarquerait le travail en cours de l'autre agent dans ton commit (piège mesuré sur ce projet). Liste explicitement chaque chemin.
- **1 commit logique**, message gitmoji en **français**. Ex : `:zap: perf(timeline): mémoïse les lanes et rend incrémentaux les recalculs de zoom`.
- **Ne pousse pas** (`git push`) — le lead s'en charge en Phase 9.

### L'exigence centrale de cette issue : MESURER, pas estimer
L'issue le dit deux fois et c'est le critère de complétude : **sans chiffres avant/après, l'issue
n'est pas terminée.** Le banc est déjà là et rejouable (`TimelineView.perf.stories.tsx` +
`stress-fixtures.ts`).

- Relève le **baseline AVANT** toute modification (frames > 16,7 ms sur 89, frame la plus lente, coût de recalcul au zoom pour 1 000 événements).
- Relève les **mêmes métriques APRÈS**.
- Si tu ne peux pas faire tourner le banc (Storybook indisponible, headless impossible…) : **dis-le explicitement** dans `non_verifie` et retourne `STATUS: PARTIAL` plutôt que d'affirmer un gain non mesuré. Une affirmation de perf non mesurée est pire que pas de correctif.
- Cible : ≤ 2 frames sur 89 au-dessus de 16,7 ms ; frame la plus lente ≤ 33,4 ms (acquis S49, pas de régression).

### Tests (OBLIGATOIRE)
```bash
cd /Users/herrh/VSProjects/MyTimeline/.claude/worktrees/new-feature-2347-14cb9a
./scripts/test-quiet.sh frontend      # suite unitaire (vitest)
cd frontend && npm run typecheck      # tsc --noEmit, strict
```
Vérifie particulièrement le **fenêtrage fermé** validé au S49 : aucun événement manqué ni dupliqué
en bord de bande. Si le volume dépasse ~500 tests ou 3 min : retourne `RECOMMAND_TEST_RUNNER`.

### Pièges d'outillage sur ce poste (mesurés, ne les redécouvre pas)
- ⚠ **Le hook RTK avale la sortie de certaines commandes** : `git diff` et `wc -c` renvoient vide/0 alors que le contenu existe. Contournement : `rtk proxy git diff`, ou redirige vers un fichier et lis-le. **Ne conclus jamais « aucun changement » sur la foi d'un `git diff` vide.**
- ⚠ `detect-domain.sh` et `check-prereq.sh` sont **cassés** (blocage infini / faux exit). Ne les appelle pas.
- ⚠ **`React.memo` piège connu, rappelé par l'issue** : inefficace si une prop est recréée à chaque rendu parent. Vérifie la stabilité réelle des props (callbacks, objets littéraux) **avant** de conclure au gain — sinon tu ajoutes du coût de comparaison sans bénéfice.

### Checkpoint de lecture du pack (anti-code-à-l'aveugle)
Le context-pack ci-dessus est **inliné dans ce briefing** — il n'y a aucun fichier externe à ouvrir.
Dans ton retour, ouvre par une ligne `pack_lu: OUI — <nom du pack> §<titre d'une section réelle que tu as utilisée>`.
Si le pack ne t'a servi à rien, écris `pack_lu: OUI — sans usage (raison en 5 mots)`. Ne l'invente pas.

## Designer
Non applicable — optimisation de rendu, aucun changement visuel attendu.

## Livrable attendu (format strict, MAX 500 tokens, style caveman — pas de prose)

```
pack_lu: OUI — <pack> §<section>
commits: [SHA1]
resume: <objectif + fichiers clés + ce qui a réellement changé + tests lancés et résultats CHIFFRÉS>
mesures:
  avant: frames>16.7ms = X/89 | frame max = Y ms | recalcul zoom 1000 ev = Z ms
  apres: frames>16.7ms = X'/89 | frame max = Y' ms | recalcul zoom 1000 ev = Z' ms
  banc_execute: OUI/NON (si NON : pourquoi)
[MEMORY:*] signaux: <pitfall / pattern / décision — si applicable>
recommandations suite: <RECOMMAND_TEST_RUNNER | RECOMMAND_FOLLOWUP: <desc> [triage XS|S|M|L] | ou "aucune">
non_verifie: <liste explicite de ce que tu n'as PAS pu vérifier — obligatoire, "rien" interdit sans justification>
STATUS: COMPLETED
```

Dernière ligne = `STATUS: COMPLETED` (ou `STATUS: PARTIAL` avec une section `BLOQUE_SUR:` juste au-dessus).
