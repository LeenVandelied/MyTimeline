[BRIEFING ISSUE #435 — SPRINT 67, VAGUE 1]
...
```
Attendu : branche `claude/sprint-67-start-a731f5`, HEAD `3e05f26`.
Si tu es ailleurs (notamment `/Users/herrh/VSProjects/MyTimeline` sans `.claude/worktrees/...`),
tu es dans le MAUVAIS dépôt : `cd` explicitement dans le chemin ci-dessus et recommence.
Tous les chemins de ce briefing sont relatifs à cette racine.

⚠ `git diff` est avalé par le hook RTK sur ce poste (sortie ~vide, trompeuse).
Utilise `rtk proxy git diff ...` ou redirige vers un fichier puis lis-le. Ne conclus JAMAIS
« aucun changement » sur un `git diff` nu.

## Issue
**[BUG] Réparer `npm audit fix` sur `frontend/`** (size:S, priority:P2, label `frontend`)

`frontend/package.json` déclare `"overrides": { "postcss": "$postcss" }` — une référence
auto-référentielle que l'arbre de dépendances virtuel construit par `npm audit fix` ne sait pas
résoudre, ce qui fait échouer **toute** invocation avec l'erreur
`npm error Unable to resolve reference $postcss`.

Remplacer cette référence par la version littérale actuellement utilisée, puis retester
`npm audit fix` **et** le build Tailwind v4. L'override existe pour une raison précise — la
comprendre et la vérifier avant de la retirer ou de la modifier.

### Critères d'acceptation (issue)
- [ ] `overrides.postcss` utilise une version littérale au lieu de `$postcss`
- [ ] `npm audit fix` s'exécute sans l'erreur `Unable to resolve reference`
- [ ] Le build Tailwind v4 (`npm run build`) passe toujours après le changement
- [ ] La raison d'être de l'override postcss est documentée (commentaire ou README) pour éviter
      qu'un futur nettoyage de dépendances ne la supprime par erreur

### BR impactées
Aucune. Ce sprint ne touche aucune règle métier — c'est de l'outillage de dépendances.

## État réel vérifié par le lead sur `3e05f26` (fais-en ton point de départ, ne le re-devine pas)

`frontend/package.json` :
```json
"overrides": { "postcss": "$postcss", "sharp": "^0.35.0" }
```
et `devDependencies.postcss = "^8.5.23"` (postcss n'est PAS dans `dependencies`).

Commande reproduite par le lead :
```
$ npm audit fix --dry-run
npm error Unable to resolve reference $postcss
```
→ le défaut est **confirmé**, ce n'est pas une hypothèse de lecture.

Contexte historique, extrait du bloc de commentaires de `.github/workflows/ci.yml` (job `security`) :
> Étape BLOQUANTE = dépendances de PRODUCTION uniquement (0 vuln au sprint 45,
> après bump postcss 8.5.23 / sharp 0.35.3 **via overrides** / next 15.5.22).

C'est la piste sur l'existence des overrides : ils forcent des versions non vulnérables de
`postcss` et `sharp` dans des chaînes **transitives** (Tailwind v4 / Next dépendent fortement de
postcss). Vérifie-la, ne la recopie pas telle quelle.

## Ce qu'on attend précisément

`$postcss` se résout vers le spec déclaré dans les deps du paquet, soit `^8.5.23`. Écrire la
littérale `"^8.5.23"` doit donc être **strictement équivalent en résolution**. D'où un oracle fort :

> **`frontend/package-lock.json` ne doit PAS changer.**
> Si le lockfile bouge après ta modification, c'est que l'équivalence est fausse — arrête-toi,
> explique ce qui a bougé et pourquoi, plutôt que de committer un lockfile modifié sous couvert
...
cd .. && rtk proxy git diff --stat -- frontend/package-lock.json   # attendu : vide
```

## Périmètre — ce que tu NE fais PAS

Cette issue **répare l'outil**, elle ne corrige **aucune** vulnérabilité. Les 7 HIGH actuelles sont
le périmètre des issues #182 et #438, traitées en vague 2 par un AUTRE agent, sur ce même lockfile.

- ❌ Ne lance PAS `npm audit fix` sans `--dry-run` (il écrirait dans le lockfile).
- ❌ Ne bump AUCUNE dépendance (ni `eslint`, ni `storybook`, ni `vite`, ni `vitest`).
- ❌ Ne touche pas à `.github/workflows/ci.yml` — l'arbitrage sur l'étape informative appartient à #438.
- ❌ Ne supprime pas l'override `sharp` (il est déjà littéral, donc hors sujet).
- ✅ Tu peux et tu dois exécuter `npm audit fix --dry-run` : c'est l'oracle du critère d'acceptation.

Si en cours de route tu constates que l'override `postcss` est devenu **inutile** (la chaîne
transitive résout déjà ≥ 8.5.23 sans lui), ne le supprime pas pour autant : **documente-le** et
signale-le en `RECOMMAND_FOLLOWUP`. Le retirer changerait le lockfile et empiéterait sur la vague 2.
...
- Vague 1 : tu es **seul** à écrire. Aucun autre agent ne tourne en parallèle.
- Ta livraison **débloque** la vague 2 (#182 + #438) : sans `npm audit fix` fonctionnel, l'agent
  suivant devrait de nouveau bricoler des contournements manuels. C'est tout l'objet de l'issue.
- Tu partages `frontend/package.json` et `frontend/package-lock.json` avec la vague 2. Laisse
  l'arbre dans un état résolu et cohérent (`npm install --package-lock-only` propre).

## Designer
Non applicable — aucune surface visuelle touchée.

## Contraintes

- **Branche cible** : `claude/sprint-67-start-a731f5` (déjà checkout, ne pas en créer d'autre).
- **Commit** : 1 seul commit logique, message gitmoji en **français**.
  Exemple de forme : `:wrench: fix(frontend): override postcss littéral pour débloquer npm audit fix`
- **`git add` ciblé** : liste les fichiers un par un. **JAMAIS `git add -A` / `git add .`** — le
  worktree est partagé avec l'orchestrateur, qui a des fichiers en cours dans `docs/memory/`.
- **Tests obligatoires avant commit** (depuis `frontend/`) :
  - `npm run build` — le critère Tailwind v4. C'est le test qui compte pour cette issue.
  - `npm run lint`
  - `npm run test` (vitest)
  - `npm run typecheck`
  Si le harnais projet `./scripts/test-quiet.sh frontend` fonctionne, il est préférable ; sinon les
  commandes npm ci-dessus font foi. **Rapporte les compteurs réels** (passés/échoués), pas « vert ».
- **Ne modifie pas** : `.github/workflows/ci.yml`, `docs/memory/sprint-history.md`,
  tout fichier backend, toute spec E2E.
- Un test qui échouait **déjà avant** ta modification n'est pas à toi de le corriger : constate-le
  sur `3e05f26` (stash ou `git stash` interdit ici — utilise un checkout de vérification ou compare
  au CI) et signale-le, ne le maquille pas.

## Documentation attendue (critère d'acceptation n°4)

La raison d'être de l'override doit survivre à un futur nettoyage. Le plus robuste est un
commentaire **au plus près de la déclaration**. `package.json` n'accepte pas de commentaires JSON :
utilise donc une des deux formes, à ton choix argumenté —
- une clé sœur documentaire dans `package.json` (ex. `"overridesRationale"`), ou
- une section dans `frontend/README.md` explicitement pointée depuis le message de commit.

Dis dans ton retour laquelle tu as choisie et pourquoi. N'invente pas un fichier de doc nouveau
si un README existe déjà.

## Livrable attendu (format strict, MAX 500 tokens, style caveman — pas de prose)

Écris ton retour sous cette forme exacte :

```
RETOUR :
- commits: [SHA]
- resume: <ce qui a changé + oracle lockfile (bougé ou non) + resultat npm audit fix --dry-run
  + compteurs build/lint/test/typecheck>
- override_rationale: <ce que tu as VÉRIFIÉ sur la raison d'être de l'override, et où tu l'as documenté>
- [MEMORY:*] signaux: <pitfall/decision/pattern si applicable>
- recommandations suite: <RECOMMAND_* ou "aucune">
```

Puis, en toute dernière ligne, exactement :
`STATUS: COMPLETED`
ou, si tu es bloqué :
`STATUS: PARTIAL` (avec une section `BLOQUE_SUR:` juste au-dessus expliquant le blocage).

## Honnêteté de rapport (non négociable)

- Si le lockfile a bougé, **dis-le** et explique quoi. Ne le committe pas en le qualifiant de bruit.
- Si `npm audit fix --dry-run` passe mais propose des changements, c'est **normal et attendu** :
  le critère est qu'il ne plante plus sur `Unable to resolve reference`. Ne les applique pas.
- N'écris jamais « tout est vert » sans les compteurs. Un test non lancé se dit « non lancé ».
