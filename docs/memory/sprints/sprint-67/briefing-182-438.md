[BRIEFING ISSUES #182 + #438 — SPRINT 67, VAGUE 2]

Tu traites DEUX issues dans un seul agent, en DEUX commits séparés. Elles sont groupées parce
qu'elles écrivent le même `frontend/package-lock.json` : deux agents concurrents produiraient deux
résolutions d'arbre npm incompatibles.

## Garde-fou d'environnement (EN PREMIER)
...
```
Attendu : branche `claude/sprint-67-start-a731f5`, HEAD `9e6e3ea` (commit de la vague 1).

⚠ `git diff` est avalé par le hook RTK. Utilise `rtk proxy git diff ...` ou redirige vers un
fichier puis lis-le. Ne conclus jamais « aucun changement » sur un `git diff` nu.

## Acquis de la vague 1 (issue #435, commit `9e6e3ea`) — ne pas défaire

- `overrides.postcss` est passé de `"$postcss"` à `"^8.5.23"`. **`npm audit fix` fonctionne à nouveau.**
- **Les deux overrides sont load-bearing.** Mesuré : `next@15.5.22` épingle `postcss` à la version
  EXACTE `8.4.31` (vulnérable). Sans l'override, `npm audit --omit=dev` — l'étape **BLOQUANTE** du
  job CI `security` — repasse de 0 à 2 vulnérabilités de PRODUCTION.
  → **Ne touche à aucun des deux overrides.** Ne les « nettoie » pas, ne les déplace pas.
  Raison documentée dans `frontend/package.json` (`_overridesRationale`) et `frontend/README.md`.

## État de l'audit, mesuré par le lead sur `9e6e3ea`

`npm audit` (dev+prod) : **8 vulnérabilités — 1 moderate + 7 high, 0 critical**.
`npm audit --omit=dev --audit-level=high` (étape CI bloquante) : **0 vulnérabilité**. Rien n'est
livré au runtime : tout est dans l'outillage de développement.

| Sévérité | Paquet | Chaîne / effet |
|---|---|---|
| high | `brace-expansion` | via `minimatch@3.1.5` ← `@eslint/eslintrc@3.3.6` (+ `5.0.8` via `minimatch@10`) |
| high | `fast-uri` | SSRF, host confusion (5 advisories) |
| high | `js-yaml` | CPU quadratique sur `!!omap` |
| high | `browserslist` | croissance mémoire non bornée, crash `normalizeStats` |
| high | `image-size` | DoS, boucles infinies ICNS/JXL/HEIF |
| high | `vite-plugin-storybook-nextjs` | dépend de `image-size` |
| high | `@storybook/nextjs-vite` | dépend de `vite-plugin-storybook-nextjs` |
| moderate | `@humanfs/node` | copie récursive suivant les symlinks |

## Ce que `npm audit fix --dry-run` propose (relevé par le lead, NON appliqué)

197 ajouts, 22 modifications, 9 suppressions. **Aucun majeur sur une dépendance directe** :

```
change brace-expansion            1.1.16 => 1.1.18      (patch)
change brace-expansion            5.0.8  => 5.0.9       (patch)
change js-yaml                    4.3.0  => 4.3.2       (patch)
change fast-uri                   3.1.4  => 3.1.7       (patch)
change browserslist               4.28.4 => 4.28.8      (patch)
change @humanfs/node              0.16.6 => 0.16.8      (patch)
change @humanfs/core              0.19.1 => 0.19.2      (patch)
change update-browserslist-db     1.2.3  => 1.3.2
change storybook                  10.4.6 => 10.6.0      (mineur)
change @storybook/react-vite      10.4.6 => 10.6.0
change @storybook/nextjs-vite     10.4.6 => 10.6.0
change @storybook/builder-vite    10.4.6 => 10.6.0
change @storybook/react           10.4.6 => 10.6.0
change vite-plugin-storybook-nextjs 3.3.0 => 10.6.0     (réaligné sur le versioning Storybook)
remove image-size 2.0.2                                 (remplacé par probe-image-size 7.4.0)
add    ~197 paquets, dont les binaires de plateforme @oxc-resolver/* et @emnapi/*
```

Le gros des 197 ajouts est constitué de **binaires de plateforme** tirés par le nouveau builder
Storybook. C'est volumineux dans le lockfile mais ce n'est pas du code applicatif.

## Le fait qui périme l'énoncé d'origine de #438

`.github/workflows/ci.yml` documente, depuis le Sprint 45, que `brace-expansion` serait
...
**Ce raisonnement ne tient plus.** Le correctif proposé aujourd'hui est `1.1.16 → 1.1.18`, un patch
qui reste dans la plage `^1.1.x` attendue par `minimatch@3.1.5` : la question de la forme d'export
de la branche 5.x ne se pose donc pas. Le blocage amont est levé.

Vérifie-le toi-même — ne me crois pas sur parole. C'est le cœur de #438.

## Les deux issues

### #182 — [CHORE] Nettoyer les CVE devDeps npm (chaîne Storybook/Vitest) · size:S · priority:P3
La chaîne d'origine (`vite`, `vitest`, `flatted`, `minimatch`, `picomatch`) **n'existe plus** : elle
a déjà été résorbée, et il n'y a plus aucune vulnérabilité `critical`. Ce qui reste :
`@storybook/nextjs-vite` → `vite-plugin-storybook-nextjs` → `image-size`.

Critères (mis à jour dans l'issue) :
- [ ] la chaîne Storybook ne remonte plus de HIGH
- [ ] `npm run build` sans erreur
- [ ] suite de tests frontend verte **avec compteurs**
- [ ] `npm run storybook` démarre toujours **et** `npm run build-storybook` passe
- [ ] aucun bump majeur non intentionnel tiré au passage (vérifier le **diff du lockfile**, pas
      seulement le résultat d'audit) — `PIT-S31-001`

### #438 — [BUG] Vulnérabilités HIGH résiduelles · size:M · priority:P1
Périmètre **élargi par arbitrage dev du 2026-09-03** : traiter l'ensemble des 7 HIGH actuelles, pas
seulement la chaîne eslint de l'énoncé d'origine.

Critères (mis à jour dans l'issue) :
- [ ] chaque HIGH est soit **corrigée**, soit **justifiée par écrit** (blocage amont nommé, version
      testée, raison de l'échec) — pas de « non corrigeable » recopié d'un sprint antérieur
- [ ] `npm audit` dev+prod au vert, **ou** résiduel documenté entrée par entrée
- [ ] `npm run build`, `lint`, `test`, `typecheck` verts avec compteurs

## Triage
Taille: M — Modèle: opus — Effort: xhigh
...
## Ordre de travail imposé (deux commits, un par issue)

Le lockfile change de façon atomique : si tu lances `npm audit fix` d'un bloc, tu ne peux plus
séparer les deux issues. Procède donc en deux temps.

**Étape 1 — #182, chaîne Storybook seule.**
Cible uniquement `@storybook/*` + `vite-plugin-storybook-nextjs` (+ le remplacement d'`image-size`),
via `npm update <paquets ciblés>` ou `npm install <paquet>@<version>` explicite.
**N'utilise PAS `npm audit fix` à cette étape** — il corrigerait tout d'un coup.
Vérifie, puis commit n°1.

**Étape 2 — #438, le reste.**
`npm audit fix` (jamais `--force`) pour `brace-expansion`, `js-yaml`, `fast-uri`, `browserslist`,
`@humanfs/node`. Vérifie, puis commit n°2.

Si cette séparation s'avère impossible (par exemple une résolution croisée qui rend l'étape 1
inséparable de l'étape 2), **ne force pas** : fais un seul commit, dis-le explicitement dans ton
retour et explique pourquoi. Un commit honnête vaut mieux qu'un découpage artificiel.

## Interdits

- ❌ `npm audit fix --force` — il accepte les bumps majeurs (`PIT-S60-006`, `PIT-S31-001`).
- ❌ Toucher aux `overrides` ou à `_overridesRationale` (load-bearing, cf. acquis vague 1).
- ❌ **Modifier `.github/workflows/ci.yml`** — l'arbitrage sur l'étape d'audit CI est en cours de
  validation par le dev et sera appliqué séparément. Tu ne touches pas à ce fichier.
- ❌ `git add -A` / `git add .` (`PIT-S12-003`) — le worktree est partagé avec l'orchestrateur, qui
  a des modifications en cours dans `docs/memory/`. `git add <fichiers explicites>` uniquement.
- ❌ `git stash` — la pile est partagée entre worktrees.
- ❌ Modifier du code applicatif pour faire passer un test cassé par un bump, sans le dire. Si un
  bump casse quelque chose, le signaler est prioritaire sur le faire passer.

## Vérifications obligatoires — à chacune des deux étapes

Depuis `/Users/herrh/VSProjects/MyTimeline/.claude/worktrees/sprint-64-start-e506bb/frontend`
(chemin absolu — `PIT-S16-002` / `PIT-S19-001` : le cwd bash peut déraper sur le repo principal) :

| Commande | Pourquoi |
|---|---|
| `npm run build` | `next build` attrape des erreurs invisibles aux tests RTL |
| `npm run lint` | le bump `brace-expansion` touche la chaîne eslint — c'est LE test qui compte pour lui |
| `npm run test` | 1030 tests au départ (vague 1). Tout écart se justifie. |
| `npm run typecheck` | — |
| `npm run build-storybook` | critère #182 |
| `npm audit --omit=dev --audit-level=high` | **doit rester à 0** : c'est l'étape CI bloquante |
| `npm audit` | l'état résiduel dev+prod, à documenter entrée par entrée |

Pour `npm run storybook` (serveur interactif) : ne le laisse pas tourner en avant-plan. Un
`build-storybook` vert + un démarrage vérifié brièvement suffisent. ⚠ `PIT-S61-007` : les scripts
turbopack infèrent un mauvais workspace root en worktree quand plusieurs lockfiles coexistent —
si un serveur de dev part en vrille, c'est probablement ça, pas ton bump. `rtk proxy npx ...` contourne.

**Rapporte les compteurs réels.** « Vert » sans chiffre n'est pas un rapport. Un test non lancé se
dit « non lancé ».

## Ce que #438 attend en plus du code : un verdict écrit

Pour **chacune** des 7 HIGH, ton retour doit dire : corrigée (version avant → après), ou non
corrigée avec le blocage amont **nommé et vérifié**. Interdiction de recopier le « non corrigeable »
du Sprint 45 sans l'avoir re-testé — c'est exactement l'erreur que #438 existe pour réparer.

Rappel `PIT-S45-006` : *« une 2e passe AGGRAVE, et les fix available mentent »*. `fixAvailable: true`
est une annonce de npm, pas une preuve. La preuve, c'est ton `npm audit` après coup + la suite verte.

Si l'audit dev+prod atteint 0, dis-le explicitement : cela conditionne l'arbitrage sur l'étape CI
(traité hors de ton périmètre).

## Commits

Deux commits, gitmoji **français**, `git add` ciblé fichier par fichier.
Formes attendues :
- `:arrow_up: chore(frontend): bump chaîne Storybook 10.6.0, retire image-size (#182)`
- `:lock: fix(frontend): résorbe les 7 CVE HIGH devDeps (#438)`

## Livrable attendu (format strict, MAX 500 tokens, caveman — pas de prose)

```
RETOUR :
- commits: [SHA1 (#182), SHA2 (#438)]   ou [SHA] + raison si commit unique
- resume: <ce qui a bougé dans le lockfile : compte add/change/remove, majeurs éventuels>
- verdict_par_CVE:
    brace-expansion: <corrigée x.y.z -> a.b.c | non corrigée + blocage nommé>
    fast-uri: ...
    js-yaml: ...
    browserslist: ...
    image-size: ...
...
- audit_final: dev+prod = <n> | --omit=dev = <n>   (le second DOIT être 0)
- tests: build=<exit> lint=<exit> test=<n/n> typecheck=<exit> build-storybook=<exit>
- [MEMORY:*] signaux:
- recommandations suite: <RECOMMAND_* ou "aucune">
```

Dernière ligne exactement `STATUS: COMPLETED`, ou une section `BLOQUE_SUR:` suivie de
`STATUS: PARTIAL`.
