...
## Issue
[REFACTOR] Events : DTO EventResponse + port EventService sans DTO applicatif + adapter câblé sur le port

### Contexte
Architecture hexagonale : le cœur métier (« domaine ») doit rester indépendant des détails techniques. Sur le module Événements, cette séparation n'est pas respectée à trois endroits (audit qualité 2026-07-02, axe 1 « Fragile »).

### Description — 3 violations à corriger
1. `EventController` sérialise directement le modèle domaine `Event` en réponse REST (endpoints POST/PATCH), alors que tous les autres domaines exposent un DTO `*Response` dédié. De plus, le POST renvoie **200 au lieu de 201**.
2. Le port domaine `EventService` importe des DTOs applicatifs `EventCreationRequest`/`EventUpdateRequest` — le domaine dépend de la couche application (inversion du sens de dépendance attendu).
3. `EventRepositoryJpaImpl` injecte directement la classe concrète `ProductRepositoryJpaImpl` au lieu de dépendre du port `ProductRepository` (interface).

Même famille de problèmes que #123 et #94 (déjà traités pour d'autres domaines) — s'inspirer de ces patterns existants dans le code.

### Critères d'acceptation
- [ ] Création de `EventResponse` (DTO de sortie), aligné sur le contrat v3 consommé côté frontend (#150 suivra en V2)
- [ ] Le port `EventService` utilise une commande domaine pure (pas d'import de la couche application)
- [ ] `EventRepositoryJpaImpl` dépend du port `ProductRepository`, pas de l'implémentation concrète
- [ ] Plus aucun import `application.*` sous `domain/`
- [ ] `POST /api/events` renvoie **201** avec `EventResponse`
- [ ] Tests adaptés aux nouvelles signatures

### Piste technique (chemins body issue partiellement stale — CONFIRMER par grep)
- Controller réel : chercher `EventController.java` (le body cite `interfaces/rest/` mais l'arbo réelle utilise `infrastructure/adapters/controllers/` — VÉRIFIER avant d'éditer)
- `domain/ports/services/EventService.java`
- `infrastructure/persistence/EventRepositoryJpaImpl.java`

### Risques
Changement de code HTTP 200→201 sur POST /api/events : le frontend (#150) suivra en V2 et consommera ce nouveau contrat. Documenter dans le retour la forme exacte de `EventResponse` (champs + types) car #150 en dépend directement.

## Plan d'implémentation (architect, /sprint plan)
```yaml
issue_0165:
  fichiers_cles:
    - "backend/.../EventController.java (chemin exact à confirmer par grep : infrastructure/adapters/controllers/ probable)"
    - "backend/.../domain/ports/services/EventService.java"
    - "backend/.../infrastructure/persistence/EventRepositoryJpaImpl.java"
    - "nouveau DTO EventResponse.java (aligner l'emplacement sur les *Response existants des autres domaines)"
  ...
  risque_regression: "HTTP 200→201 sur POST. Port EventService : retirer imports application.* sans casser les use cases. BR-EVE-001 (event↔user) préservée."
  ordre_ecriture: "domain (commande pure + port) → application (use case) → infra (DTO + controller + adapter câblé sur port)"
  zod_dto_sync: "OUI — EventResponse = source du contrat consommé par #150. Décrire sa forme exacte dans le retour."
```
...
- Aucune dépendance amont (V1, premier de la chaîne).
- **Livre le contrat** consommé par #150 (V2) puis validé par #163 (V3). Le retour DOIT documenter la forme exacte de `EventResponse` (nom + type de chaque champ, notamment `color` unique, `recurrenceUnit` enum WEEK/MONTH/YEAR, `recurrenceEndDate` nullable, `archived`).

## Designer
Non applicable (refactor backend, aucune surface visuelle).

## Contraintes (worktree — LIRE ATTENTIVEMENT)
- **Tu opères dans un worktree git.** `cd /Users/herrh/VSProjects/MyTimeline/.claude/worktrees/nice-goldberg-86ef14` AVANT toute commande. Vérifie `git rev-parse --abbrev-ref HEAD` == `sprint/15` avant de commit. Si tu te retrouves sur `dev` ou un autre chemin, STOP.
- Branche cible : `sprint/15` (déjà checkout dans ce worktree).
- Commit : 1 commit logique, message gitmoji français (ex: `:recycle: Events : DTO EventResponse + port domaine pur + adapter câblé sur port (#165)`).
- Tests inline via `./scripts/test-quiet.sh <scope>` (backend) — OBLIGATOIRE avant de retourner.
- Si volume tests > 500 OU temps > 3min : signaler RECOMMAND_TEST_RUNNER au lieu de tout lancer.
- Ne PAS toucher aux fichiers frontend (réservés à #150/#163). Ne PAS toucher aux fichiers partagés hors scope (migrations, DTOs d'autres domaines).
- Ne PAS spawner d'autres subagents (profondeur 1 stricte).

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: objectif + BR touchées + fichiers clés + **forme exacte de EventResponse (champs+types)** + pitfalls + tests (passed/failed)
