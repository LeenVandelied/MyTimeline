...
## Issue
[FEATURE] E2E Playwright golden-path : inscription → connexion → produit + événement → timeline (+ job CI)

### Contexte
Aucun parcours utilisateur complet n'est vérifié automatiquement de bout en bout. Le dossier `frontend/e2e/` ne contient qu'un `.gitkeep` alors que `frontend/playwright.config.ts` est déjà configuré. Recommandation P0 de l'audit 2026-07-02 axe 3.

### Objectif — golden path testé automatiquement
1. Inscription d'un nouvel utilisateur
2. Connexion
3. Création d'un produit avec un événement associé
4. Vérification de l'affichage correct dans la timeline

Plus : job CI GitHub Actions exécutant ce test E2E.

### Critères d'acceptation
- [ ] Le test Playwright du parcours complet passe en local
- [ ] Le test passe également en CI
- [ ] Le test utilise **exclusivement** des sélecteurs `data-testid` (jamais texte/classe CSS)
- [ ] Un nouveau job E2E ajouté à `.github/workflows/ci.yml`

### ⚠ ÉTAT RÉEL DU CODE (recon lead — À LIRE ABSOLUMENT)

**1. data-testid : PARTIELLEMENT présents.**
- ✅ Écrans AUTH déjà instrumentés (S8 #53) — À RÉUTILISER tels quels :
  - register : `register-form`, `register-email`, `register-name`, `register-username`, `register-password`, `register-confirm-password`, `register-submit`, `register-error`, `register-success`
  - login : `login-form`, `login-username`, `login-password`, `login-submit`, `login-error`
- ❌ **Dashboard / création produit / formulaire événement / timeline : AUCUN data-testid.** (grep confirmé : seuls les tests unitaires en contiennent, pas les composants runtime.)
  → Tu DOIS **ajouter les `data-testid` manquants** sur les composants du parcours produit+événement+timeline (dashboard `page.tsx`, `ProductDrawer`, `EventEditForm`/formulaire de création event, rendu timeline `TimelineCalendar`/`EventContent`). C'est dans le scope de #163 (sans testids, un golden path testid-only est impossible). Reste minimal : uniquement les ancres nécessaires au parcours. Ne PAS refactorer la logique de ces composants.

**2. Le golden path est FULL-STACK.** register/login/create product/create event = vrais appels API backend. Le frontend lit `NEXT_PUBLIC_API_URL` (`frontend/src/services/apiClient.ts:6`). Il n'y a PAS de docker-compose. Le job CI doit donc lever la stack complète :
   - **Postgres 16** (service container). DB `eventmanager`, user `eventuser`, password au choix.
   - **Backend Spring Boot** (port **8080** par défaut, non overridé). Profil `dev` convient pour l'E2E (cookie `Secure=false`, `Domain=localhost`, CORS `http://localhost:3000` déjà autorisé). Env requis : `DB_URL=jdbc:postgresql://localhost:5432/eventmanager`, `DB_USERNAME=eventuser`, `DB_PASSWORD=...`, `JWT_SECRET=...` (le profil dev a un default, mais passe-le explicitement en CI), `SPRING_PROFILES_ACTIVE=dev`. Flyway rejoue V1..Vn au boot. Attendre que le backend soit up (health/readiness) avant de lancer Playwright.
   - **Frontend** : `NEXT_PUBLIC_API_URL=http://localhost:8080`. Deux options — soit `npm run build && npm run start`, soit laisser le `webServer` de `playwright.config.ts` lancer `npm run dev`. NB : `NEXT_PUBLIC_*` est lu au build pour `next build`/`start` ; en `next dev` il est lu au runtime via l'env. Choisis l'approche la plus fiable en CI et documente-la.
   - `npx playwright install --with-deps chromium` avant le run.

**3. Config Playwright existante** (`frontend/playwright.config.ts`) : `testDir: './e2e'`, projet `chromium`, `retries: 2` en CI, `workers: 1` en CI, `webServer` lance `npm run dev` sauf si `PLAYWRIGHT_BASE_URL` est posé. Script : `"test:e2e": "playwright test --pass-with-no-tests"`.

**4. i18n `localePrefix: 'always'`** : toutes les routes sont préfixées par la locale (`/fr/...`, `/en/...`). Les URLs de navigation dans le spec doivent inclure le préfixe locale (ou passer par les liens de l'UI). Redirections login/register localisées.

**5. CI actuelle** (`.github/workflows/ci.yml`) : jobs `backend` (mvnw verify) et `frontend` (build/test/typecheck/lint). Branch protection PAS encore activée (commentaire en tête du fichier). Ajoute un job `e2e` SANS casser les 2 existants. Ne modifie pas la config de branch protection.

### Piste technique
- `frontend/e2e/golden-path.spec.ts` (NEW)
- `.github/workflows/ci.yml` (nouveau job `e2e`)
- data-testid à ajouter : `frontend/app/[locale]/dashboard/page.tsx`, `frontend/src/components/products/ProductDrawer.tsx`, formulaire création event, `frontend/src/components/calendar/TimelineCalendar.tsx` / `EventContent.tsx`

### Risques
Un test E2E flaky en CI bloque les merges. Soigner les `await expect(...).toBeVisible()` / `waitFor`, éviter les timeouts hardcodés, attendre la disponibilité réelle du backend avant de démarrer. `retries:2`/`workers:1` déjà posés en CI.
...
issue_0163:
  ...
  risque_regression: "Greenfield E2E. Sélecteurs data-testid uniquement. Risque flaky en CI — soigner waitFor. data-testid auth déjà posés (S8 #53) ; ceux du parcours produit/event/timeline sont À AJOUTER."
  ordre_ecriture: "frontend (ajout testids parcours → spec) → CI (job)"
  zod_dto_sync: "NON"
```
...
- **Dépend de #150 (V2, déjà commit sur `sprint/15`)** : le formulaire event et la timeline consomment le contrat v3 (color unique, isAllDay, recurrenceUnit enum). Le parcours event doit fonctionner avec ces schémas migrés.
- **Dépend de #165 (V1)** : POST /api/events renvoie 201 + EventResponse. Le create d'event via l'UI passe par ce endpoint.
- Dernier de la chaîne (V3) — rien en aval.

## Designer
Non applicable. L'ajout de `data-testid` est non-visuel (attributs seulement). Ne PAS modifier l'apparence des composants.

## Contraintes (worktree — LIRE ATTENTIVEMENT)
- **Tu opères dans un worktree git.** Fais `cd /Users/herrh/VSProjects/MyTimeline/.claude/worktrees/nice-goldberg-86ef14` AVANT toute commande. Vérifie `git rev-parse --abbrev-ref HEAD` == `sprint/15` avant de commit. Si tu te retrouves sur `dev` ou ailleurs, STOP et signale-le.
- Branche cible : `sprint/15` (déjà checkout, contient déjà #165 + #150).
- Frontend sous `frontend/`, CI sous `.github/workflows/ci.yml` (racine).
- Commit : 1 commit logique, message gitmoji français, **header ≤ 100 caractères** (commitlint header-max-length:100 — cf. #150). Ex: `:white_check_mark: E2E golden-path Playwright + job CI (#163)`.
- **Faire tourner le spec en LOCAL avant de retourner** : `cd frontend && npm run test:e2e` (nécessite le backend + Postgres + frontend up localement, OU un `PLAYWRIGHT_BASE_URL`). Si tu ne peux PAS lever la stack complète en local (Docker/Postgres indisponible dans ton env), dis-le explicitement, décris ce que tu as vérifié (typecheck du spec, dry-run `--list`), et signale STATUS PARTIAL avec ce blocage — ne prétends pas un run vert non exécuté.
- Vérifie que `next build` + `npm run typecheck` + `npm run lint` passent (l'ajout de testids ne doit pas casser le build — cf. pitfall #150 : vitest vert ≠ build vert).
- Ne PAS toucher aux fichiers backend (livré #165). Ne PAS modifier les 2 jobs CI existants (backend/frontend), seulement AJOUTER le job e2e. Ne PAS activer la branch protection.
- Ne PAS spawner d'autres subagents (profondeur 1 stricte).

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: objectif + fichiers créés/modifiés + testids ajoutés (liste) + design du job CI (comment la stack est levée) + résultat run local E2E (passed/failed OU non exécuté+pourquoi) + build/typecheck/lint OK
- [MEMORY:*] signaux: (pitfall/pattern si applicable — ex. lancement stack E2E en CI)
- recommandations suite: RECOMMAND_* ou RECOMMAND_FOLLOWUP si découverte hors scope
