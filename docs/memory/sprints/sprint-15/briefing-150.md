...
## Issue
[FRONTEND] Sync Zod/types frontend sur le nouveau contrat DTO events v3 (couleurs, recurrenceUnit enum, archived)

### Contexte
Le Sprint 9 a aligné le modèle métier backend sur le design v3 (migration V7). Le contrat des DTO events a changé de forme et le frontend n'est pas encore synchronisé. **La V1 de CE sprint (#165, déjà mergée sur `sprint/15`) vient de finaliser le contrat de sortie `EventResponse`** — tu consommes EXACTEMENT ce contrat (voir section « Contrat backend livré » ci-dessous).

### Description
Synchroniser les schémas Zod / types frontend avec le nouveau contrat DTO events :
- Couleurs : `backgroundColor` + `borderColor` + `textColor` → un seul champ `color` (les 3 anciens champs n'existent plus côté backend).
- `EventUpdateRequest` expose désormais `recurrenceEndDate` (date nullable) et `archived` (boolean).
- `recurrenceUnit` : valeur canonique = enum `WEEK`/`MONTH`/`YEAR` — le frontend doit migrer vers ces valeurs.
- Rappel pitfall Zod projet : `.nullable()` pour un champ nullable backend, **jamais `.nullish()`** en schéma manuel.

### Contrat backend livré par #165 (EventResponse — SOURCE DE VÉRITÉ)
Réponse de `POST /api/events` (201) et `PATCH /api/events/{id}`. Nom + type de CHAQUE champ JSON :
| champ JSON | type | notes |
|---|---|---|
| id | UUID (string) | |
| title | String | (le create envoie `name` en entrée, mais la RÉPONSE expose `title`) |
| type | String | "duration"/"single"/autre |
| durationValue | Integer | nullable |
| durationUnit | String | nullable ; "days"/"weeks"/"months"/"years" (unité de DURÉE, minuscule — ≠ recurrenceUnit) |
| isRecurring | Boolean | |
| recurrenceUnit | String enum | "WEEK"/"MONTH"/"YEAR" (MAJUSCULE), nullable |
| recurrenceEndDate | string date "YYYY-MM-DD" | nullable |
| startDate | string date "YYYY-MM-DD" | |
| endDate | string date "YYYY-MM-DD" | |
| productId | UUID (string) | |
| isAllDay | Boolean | **nom JSON = `isAllDay`** (pas `allDay`) — cf. BR-EVE-010, aligner le schéma dessus, nullable |
| color | String | nullable, format hex libre (pas de validation backend) |
| archived | boolean | toujours présent ; false au create |

⚠ Attention à la distinction : `durationUnit` = minuscule (days/weeks/months/years), `recurrenceUnit` = MAJUSCULE enum (WEEK/MONTH/YEAR). Ne pas les confondre.

### Critères d'acceptation
- [ ] Schémas Zod events (création + édition) alignés sur `color` unique (plus de bg/border/text).
- [ ] `recurrenceEndDate` et `archived` typés côté frontend.
- [ ] `recurrenceUnit` migré vers `WEEK`/`MONTH`/`YEAR`.
- [ ] Les composants consommateurs (form event, timeline) compilent et passent les tests.

### Piste technique
- `frontend/src/types/event.ts` (schéma Zod source de vérité — `eventSchema`, `eventCreationSchema`, `eventEditSchema`)
- `frontend/src/components/EventEditForm.tsx` (conservait le modèle 3-couleurs, lignes ~262-264)
- `frontend/src/services/eventService.ts` (service API)
- Le mapping `mapToFullCalendarEvent` lit `event.allDay` → à corriger vers `isAllDay` (BR-EVE-010).
- Vérifier tout composant timeline consommant les 3 anciens champs couleur.

### Risques
Fusion bg/border/text → `color` unique : tout composant consommant les 3 anciens champs casse à la compilation. Traquer les usages avant de supprimer.
...
issue_0150:
  ...
  risque_regression: "Fusion bg/border/text → color unique : tout composant consommant les 3 anciens champs casse. Pitfall : .nullable() jamais .nullish(). Doit consommer EXACTEMENT le EventResponse livré par #165."
...
- **Dépend de #165 (V1, déjà mergée sur `sprint/15`)** : le contrat `EventResponse` est figé, cf. tableau ci-dessus. Ne pas redéfinir un contrat divergent.
- **Livre le socle** consommé par #163 (V3, E2E) : les `data-testid` et le formulaire event doivent rester fonctionnels pour le golden path.

## Designer
Non applicable si la tâche reste au niveau types/schémas/services. Si tu modifies visuellement `EventEditForm` (sélecteur de couleur unique remplaçant 3 champs) : rester iso-fonctionnel visuellement, pas de refonte UI (hors scope, → RECOMMAND_FOLLOWUP si besoin).
...
- Branche cible : `sprint/15` (déjà checkout, contient déjà le commit #165).
- Le frontend est sous `frontend/` relatif à la racine du worktree.
- Commit : 1 commit logique, message gitmoji français (ex: `:recycle: Front events : sync Zod/types sur contrat v3 (color unique, recurrenceUnit enum, archived) (#150)`).
- Tests inline via `./scripts/test-quiet.sh frontend` (Vitest) — OBLIGATOIRE avant de retourner. Vérifie aussi que `tsc`/build passe (pas de type errors résiduels après suppression des 3 champs couleur).
- Ne PAS toucher aux fichiers backend (livré par #165). Ne PAS créer de spec E2E (réservé à #163).
- Ne PAS spawner d'autres subagents (profondeur 1 stricte).

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: objectif + fichiers clés modifiés + champs Zod migrés (avant→après) + pitfalls Zod + tests (passed/failed) + confirmation build/tsc OK
- [MEMORY:*] signaux: (pitfall/pattern si applicable)
