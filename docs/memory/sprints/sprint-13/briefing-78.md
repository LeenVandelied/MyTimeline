...
## Issue
[FEATURE] Backend : suppression de compte (DELETE /me)

### Contexte
Aucun moyen pour un utilisateur de supprimer son compte. Requis RGPD (droit à l'effacement). Le modèle actuel ne définit pas de cascade explicite entre `users`, `products`, `events`, `categories`.

### À faire
- `DELETE /api/me` avec confirmation par re-saisie du username (body JSON : `{ "username": "..." }`).
- Vérifier que le username fourni == utilisateur authentifié avant suppression.
- Supprimer en cascade dans le MÊME `@Transactional` : events → products → categories(owner) → user.
- Invalider le token JWT courant : appeler `sessionService.revokeAllSessions(userId)` (issue #73 mergée dans ce sprint) + effacer le cookie (MaxAge=0).
- Retourner `204 No Content`.
- Documenter la politique de rétention (logs, analytics) en commentaire de code.

### BR impactées
- BR-AUT-001 (variante ownership) — seul l'utilisateur identifié peut supprimer son propre compte (identité dérivée du JWT, jamais d'un param).

### Critères d'acceptation
- [ ] `DELETE /api/me` sans corps ou avec un username incorrect → `400`.
- [ ] `DELETE /api/me` avec le bon username supprime le compte → `204`.
- [ ] Tous les produits ET événements de l'utilisateur supprimés dans la même transaction.
- [ ] Le cookie JWT est effacé dans la réponse (MaxAge=0).
- [ ] Un second appel avec le même token (si non révoqué) → `401` (utilisateur introuvable).
- [ ] Suppression irréversible et documentée.

### Piste technique (adaptée à l'état RÉEL du code — vérifié par le lead)
- **`UserController` existe déjà** (`@RequestMapping("/api/me")`, issue #70) — ajouter simplement un `@DeleteMapping` (racine, pas de sous-path). Il injecte déjà les PORTS `UserService` + `JwtService`, helper `resolveCaller(token)` disponible. RÉUTILISER, ne pas recréer.
- **`UserService` port** : ajouter `void deleteAccount(User caller, String confirmUsername)`. Impl dans `UserServiceImpl` (`@Transactional`).
- **`SessionService.revokeAllSessions(UUID userId)`** est DÉJÀ disponible (livré par #73, commit d3a776f) — l'appeler dans la transaction de suppression.
- Nouveau DTO `application/dtos/DeleteAccountRequest.java` (`@NotBlank String username`) + `@Valid`.

### Graphe FK réel (vérifié par le lead — À RESPECTER)
- `sessions.user_id` → users : **ON DELETE CASCADE** (V10) → auto-purge, pas besoin de le gérer manuellement (mais `revokeAllSessions` explicite = OK).
- `password_reset_tokens.user_id` → users : **ON DELETE CASCADE** (V6) → auto-purge.
- `products.user_id` → users : **PAS de cascade** (FK anonyme V1) → suppression applicative requise.
- events (`user_id`) → users : **PAS de cascade** (FK anonyme V1) → suppression applicative requise.
- `categories.owner_id` → users : **PAS de cascade** (FK anonyme V8) → supprimer les catégories possédées (owner_id = user) ; NE PAS toucher les catégories SYSTÈME (owner_id NULL).
- `products.category_id` → categories NOT NULL → **supprimer les produits AVANT les catégories**.
- Ordre transactionnel imposé : **events → products → categories(owner=user) → user**.

## Plan d'implémentation (architect, /sprint plan — recalé par le lead)
```yaml
issue_0078:
  fichiers_cles:
    - "backend/.../infrastructure/adapters/controllers/UserController.java  # AJOUTER @DeleteMapping (existe déjà)"
    - "backend/.../domain/ports/services/UserService.java  # + deleteAccount(caller, confirmUsername)"
    - "backend/.../application/services/UserServiceImpl.java  # impl @Transactional suppression ordonnée"
    - "backend/.../application/dtos/DeleteAccountRequest.java  # nouveau (username @NotBlank)"
    - "backend/.../domain/ports/repositories/{Product,Event,Category}Repository.java  # + suppression par user"
  couches_touchees: ["domain","application","infrastructure"]
  strategie_test: "integration (DELETE /me bon username->204 cascade, mauvais/absent->400, 2e appel->401, cookie MaxAge=0, produits+events+catégories purgés)"
  risque_regression: "Cascade transactionnelle partielle si FK non purgée -> suppression à mi-chemin (rollback). ATTENTION @SQLRestriction(archived=false) sur ProductEntity : une lecture JPA ne voit PAS les produits archivés -> leur FK bloque le delete user. Utiliser SQL NATIF bindé pour purger TOUS les produits (cf. PAT-S10-001 / ProductRepositoryJpaImpl.countByCategoryId qui bypass déjà le @SQLRestriction)."
  ordre_ecriture: "DeleteAccountRequest -> UserService.deleteAccount -> repos suppression par user (SQL natif produits) -> UserController DELETE -> revokeAllSessions + clear cookie"
  zod_dto_sync: "NON (frontend Réglages #86/#87 hors sprint)"
```
...
- **Dépend de #73 (déjà livrée, commit d3a776f)** : consomme `SessionService.revokeAllSessions(UUID userId)`. Le port est en place, l'appeler dans la transaction.
- Vague 2 : #73 est déjà commitée sur sprint/13 — tu pars du HEAD `d3a776f`.

## Designer
Non applicable (backend pur, aucun composant frontend).

## Contraintes
- Branche cible : sprint/13 (déjà checkout dans le worktree courant). NE PAS changer de branche.
- Vérifier HEAD au démarrage : `git log --oneline -1` → doit être `d3a776f` (#73 sessions). Si KO → STOP et signale.
- Commit : 1 commit logique gitmoji français sur sprint/13.
- **Migration : préférer la suppression applicative ordonnée (PAS de migration).** Les FK products/events/categories sont anonymes (V1/V8) → un `ALTER` pour ajouter ON DELETE CASCADE est fragile (contraintes sans nom). Si malgré tout tu ajoutes une migration, ce sera **V11** exactement (`V11__...sql`, dernière en place = V10).
- **PIÈGE @SQLRestriction** : `ProductEntity` porte `@SQLRestriction("archived = false")` → toute lecture JPA (findAll/findByUserId) IGNORE les produits archivés. Si tu supprimes via une lecture JPA puis delete, les produits archivés restants gardent leur FK `user_id` → le `DELETE users` échouera. Utilise du **SQL NATIF bindé** (`entityManager.createNativeQuery("delete from products where user_id = :uid")`) pour purger TOUS les produits, archivés inclus (cf. pattern existant `ProductRepositoryJpaImpl.countByCategoryId`).
- Confirmation username : comparer `confirmUsername` à `caller.getUsername()`. Mismatch OU body absent/vide → `400` (via `@Valid` + check métier). Ne PAS révéler si le compte existe.
- Identité TOUJOURS dérivée du JWT (`resolveCaller`), jamais du body/param.
- Après suppression : effacer le cookie `jwt` (MaxAge=0, réutiliser le helper `buildJwtCookie`/pattern logout si accessible, sinon ResponseCookie MaxAge=0 cohérent avec BR-AUT-010).
- `UserController` : rester sur les PORTS (déjà le cas), ne pas injecter d'`*Impl`.
- Tests inline via `./scripts/test-quiet.sh backend` (OBLIGATOIRE, Docker requis). Couvrir les 6 critères d'acceptation, dont : utilisateur avec plusieurs produits (dont 1 archivé) + événements + catégorie possédée → tout purgé, catégorie système intacte, 204 ; 2e appel → 401.
- Si volume tests > 500 OU temps > 3min : `RECOMMAND_TEST_RUNNER`.
- Touche auth/suppression PII/RGPD → `RECOMMAND_SECURITY`.
- Si tu ajoutes une migration V11 → `RECOMMAND_DB_EXPERT`.
- Ne PAS toucher : `SessionController.java`, `JwtFilter.java`, `JwtService.java` (livrés #73, ne pas régresser).
- Écris `docs/memory/sprints/sprint-13/issue-78-done.md` avec le retour ci-dessous.

## Livrable attendu (format strict, MAX 500 tokens caveman) — écris-le dans docs/memory/sprints/sprint-13/issue-78-done.md
RETOUR :
- commits: [SHA1, ...]
