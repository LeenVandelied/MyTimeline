...
## Issue
[FEATURE] Backend : gestion des sessions actives (jti + révocation)

### Contexte
MyTimeline utilise des JWT stateless : aucun registre de sessions, aucune révocation possible. Si un token est volé, il reste valide jusqu'à expiration (2 jours) sans possibilité de le révoquer.

### À faire
- Mécanisme de révocation de token : table `sessions` (jti, userId, deviceInfo, ipAddress, lastActivity, createdAt, revokedAt). Redis absent sur ce projet → **table DB avec index sur jti** + nettoyage sessions expirées.
- `GET /api/sessions` : liste des sessions actives de l'utilisateur courant (device, ville approximative, dernière activité).
- `DELETE /api/sessions/{id}` : révoquer une session spécifique.
- `DELETE /api/sessions/others` : révoquer toutes les sessions sauf la courante.
- `JwtService.generateToken` : inclure un `jti` (UUID) unique.
- `JwtFilter` : vérifier que le `jti` n'est pas révoqué à chaque requête.

### BR impactées
- BR-AUT-002 — le système STATELESS actuel ne permet pas de révocation ; implémentation partielle d'une gestion de session conforme à la règle d'expiration intentionnelle.
- BR-AUT-009 — `POST /refresh` doit rejeter les tokens révoqués (jti dans la blacklist).

### Critères d'acceptation
- [ ] Chaque JWT généré contient un claim `jti` unique.
- [ ] `GET /api/sessions` retourne la liste des sessions actives de l'utilisateur courant.
- [ ] `DELETE /api/sessions/{id}` révoque la session ; requêtes suivantes avec ce token → `401`.
- [ ] `DELETE /api/sessions/others` révoque toutes les sessions sauf la courante.
- [ ] `POST /logout` révoque également le token courant.
- [ ] Le `JwtFilter` vérifie la liste de révocation à chaque requête authentifiée.
- [ ] Aucune adresse IP complète stockée en clair (tronquée/hashée, RGPD — tronquer dernier octet IPv4).

### Piste technique
- Nouveaux `SessionEntity.java` + `SessionRepository.java` (infrastructure).
- `JwtService.java` : ajouter `jti` (UUID) dans `generateToken`.
- `JwtFilter.java` : lookup `jti` dans `SessionRepository`.
- `SessionController.java` : nouveaux endpoints.
- Migration Flyway **V10** : table `sessions` avec **index sur jti obligatoire**.
- Géolocalisation "ville" = optionnel (skip pour cette version, laisser deviceInfo/IP tronquée).

### Risques (issue)
- RGPD : `ipAddress` = donnée personnelle → tronquer dernier octet IPv4.
- Performance : lookup `jti` à chaque requête authentifiée → index DB obligatoire.

## Plan d'implémentation (architect, /sprint plan)
```yaml
issue_0073:
  fichiers_cles:
    - "backend/.../infrastructure/entities/SessionEntity.java  # nouveau"
    - "backend/.../infrastructure/adapters/repositories/jpa/SessionRepository.java  # nouveau"
    - "backend/.../infrastructure/security/JwtService.java  # ajouter jti (UUID)"
    - "backend/.../infrastructure/security/JwtFilter.java  # lookup jti revoque"
    - "backend/.../infrastructure/adapters/controllers/SessionController.java  # nouveau (GET, DELETE /{id}, DELETE /others)"
    - "backend/.../domain/ports/services/SessionService.java  # nouveau"
    - "backend/src/main/resources/db/migration/V10__create_sessions.sql  # nouveau (index sur jti) — V10 (dernière réelle = V9)"
  ...
  risque_regression: "Lookup jti a CHAQUE requete authentifiee sans index -> scans sequentiels (index FK obligatoire) ; IP en clair = violation RGPD (tronquer dernier octet)."
  ordre_ecriture: "domain port -> entity+repo -> JwtService jti -> JwtFilter lookup -> SessionController -> migration V10"
  zod_dto_sync: "NON (frontend Reglages/sessions #86/#87 hors sprint)"
```
...
- Aucune dépendance amont (Vague 1, fondation).
- **DOIT être livrée AVANT #78** : #78 consommera `SessionService.revokeAllSessions(userId)`. Exposer cette méthode dans le port `SessionService`.

## Designer
Non applicable (backend pur, aucun composant frontend).

## Contraintes
- Branche cible : sprint/13 (déjà checkout).
- Vérifier HEAD au démarrage : `git log --oneline -1` doit être sur sprint/13 (base origin/dev, 580c7ae). Si KO → STOP.
- Commit : 1 commit logique gitmoji français.
- Migration : **V10** exactement — `V10__create_sessions.sql` (dernière migration en place vérifiée = V9 ; #78 prendra V11 si cascade DB requise — NE PAS consommer V11). ⚠ Le pack cp-backend cite « dernière V8 » : périmé, la vraie dernière est V9.
- Redis absent → implémentation table DB uniquement.
- Tests inline via `./scripts/test-quiet.sh unit` (OBLIGATOIRE). Intégration : jti dans JWT, révocation → 401 requête suivante, /others, logout révoque.
- Si volume tests > 500 OU temps > 3min : signaler RECOMMAND_TEST_RUNNER (le lead spawnera test-runner).
- Migration Flyway V10 ajoutée → signaler RECOMMAND_DB_EXPERT.
- Touche auth/JWT/PII (ipAddress) → signaler RECOMMAND_SECURITY.
- Ne PAS toucher : `UserController.java` (réservé #78), `db/migration/V11*` (réservé #78).
