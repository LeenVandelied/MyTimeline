[BRIEFING ISSUE #307 — Sprint 61]

## Issue
**[PRODUIT] Que faire d'un événement archivé ? (réouverture / désarchivage)**

Le Sprint 42 (PR #306) a livré le toggle « archiver » sur un événement (PATCH-only, BR-EVE-013).
Problème : une fois archivé, l'événement **disparaît complètement** de la frise
(`ProductDetailView.tsx:59` filtre `!event.archived`) et **plus personne ne peut le retrouver
ni le désarchiver** — il n'existe aucun écran « événements archivés ».
Un utilisateur qui archive par erreur n'a aucun moyen de revenir en arrière depuis l'UI.

### DÉCISION PRODUIT TRANCHÉE PAR LE DEV (2026-08-17) — **OPTION A**
> Ajouter une vue/filtre « archivés » dans la frise permettant de retrouver, ré-ouvrir en édition
> et désarchiver un événement.
>
> L'option B (archivage définitif) est ÉCARTÉE. Ne pas la ré-instruire, ne pas renommer le bouton
> « archiver », ne pas documenter d'irréversibilité.

### BR impactées
BR-EVE-013 (archivage d'événement, PATCH-only), BR-EVE-011 (quota events actifs = non archivés).

### Critères d'acceptation (adaptés à l'Option A)
- [ ] Un événement archivé reste **atteignable via l'UI** depuis la vue détail produit.
- [ ] Il peut être **ré-ouvert en édition** (le formulaire se pré-remplit avec ses valeurs).
- [ ] Il peut être **désarchivé** (retour dans les events actifs) via le PATCH existant.
- [ ] Le critère E2E de #232 (« pré-remplissage du formulaire à la réouverture d'un événement
      archivé ») devient testable — un spec E2E le prouve.
- [ ] Le compteur d'events actifs (`nonArchivedCount`) et l'historique restent cohérents
      avec BR-EVE-011 (un archivé ne compte PAS comme actif).

## Plan d'implémentation (lead, /sprint start — pas de mini-plan architect pour ce sprint)

```yaml
issue_307:
  fichiers_cles:
    - "frontend/src/components/products/ProductDetailView.tsx"      # :59 filtre events frise, :122 nonArchivedCount, :125 history
    - "frontend/src/components/products/ProductDetailView.test.tsx" # fixtures archived:true déjà présentes (:81, :90, :108)
    - "frontend/public/locales/{fr,en,de,es}/products.json"         # namespace products.detail.*
    - "frontend/e2e/<nouveau>.spec.ts"                              # E2E réouverture/désarchivage
  couches_touchees: ["frontend"]
  strategie_test: "unit (vitest/RTL sur ProductDetailView) + E2E (parcours archiver → retrouver → désarchiver)"
  risque_regression: "BR-EVE-011 — si le filtre archivés fuit dans nonArchivedCount ou dans la frise par défaut, le quota d'events actifs devient faux"
  ordre_ecriture: "ProductDetailView (état filtre + surface archives) → i18n → tests unit → E2E"
  zod_dto_sync: "NON — `archived` est déjà exposé par EventResponse (backend) et typé côté Zod (frontend/src/types/event.ts:32). AUCUNE modification backend attendue."
  possibly_done: false
  etat_reel_du_code: |
    Vérifié par le lead en Phase 0.5 (2026-08-17) :
    - ProductDetailView.tsx:59 filtre encore `.filter((event) => !event.archived)` — la frise masque les archivés.
    - ProductDetailView.tsx:122 `nonArchivedCount` et :125 `history` filtrent aussi `!e.archived`.
    - Aucun composant / onglet / filtre « archivés » n'existe dans frontend/src.
    - Le style `.mt-evt--archived{opacity:.45; filter:grayscale(.5);}` EXISTE DÉJÀ
      (frontend/src/styles/ds/components/timeline.css:67) — RÉUTILISE-LE, n'en crée pas un autre.
    Conclusion : issue bien à faire, aucun NO-OP.
```
...
## Dépendances intra-sprint — LIRE ATTENTIVEMENT (périmètre partagé)

Sprint 61 contient 3 issues sur le MÊME flux « archivage ». Le découpage est strict :

| Issue | Vague | Périmètre | Fichiers réservés |
|---|---|---|---|
| **#307 (TOI)** | V1 | Rendre l'archivé **atteignable** : filtre/onglet « archivés », réouverture en édition, action désarchiver | `ProductDetailView.tsx` + son test + `products.json` |
| #67 | V1 (en parallèle) | Flag `capped` récurrence + hint formulaire | `EventEditForm.tsx`, backend récurrence |
| #230 | V2 (APRÈS toi) | Confirmation à l'archivage + mention quota, événement **grisé** dans la frise, champs désactivés si archivé | `EventEditForm.tsx`, styles frise |

### Interdictions dures (conflits de fichiers — un autre agent y travaille EN MÊME TEMPS)
- ❌ **NE TOUCHE PAS** `frontend/src/components/EventEditForm.tsx` ni `EventEditForm.test.tsx`.
  L'action « désarchiver » doit vivre dans **ta** surface (vue détail / liste archives), PAS dans le formulaire.
- ❌ **NE TOUCHE PAS** au backend (aucune modification Java/SQL attendue : `archived` est déjà exposé).
- ❌ **NE FAIS PAS** la confirmation d'archivage, le grisage dans la frise, ni la désactivation de champs —
  **c'est le périmètre de #230 en vague 2**. Si tu le fais, tu crées un conflit de merge garanti.
- ❌ **NE MODIFIE PAS** `frontend/src/components/timeline/**` (composant central du dashboard, réservé #230).
- ❌ `git add` **CIBLÉ uniquement** sur tes fichiers — JAMAIS `git add -A` / `git add .`
  (le working tree est PARTAGÉ avec un autre subagent qui commite en même temps).

### Ce que #230 fera après toi (ne l'anticipe pas, mais ne le bloque pas)
Un événement archivé restera **visible grisé** dans la frise. Conçois donc ton filtre comme un
**état de vue** (ex. « actifs » / « archivés » / « tous ») plutôt qu'en dur sur `!archived`,
pour que #230 puisse brancher le grisage sans réécrire ta logique.

## Designer
Non applicable — pas de nouvelle page. Réutilise les primitives existantes
(`@/components/ui/button`, patterns de `ProductDetailView`) et le style
`.mt-evt--archived` déjà présent dans `timeline.css:67`.

## Contraintes
- **Branche cible : `sprint/61`** (déjà checkout, base `origin/dev` @ d5f60eb). NE CHANGE PAS DE BRANCHE.
- **Garde-fou worktree** : avant de commencer, vérifie que tu es bien dans le worktree du sprint :
  `git rev-parse --abbrev-ref HEAD` doit renvoyer `sprint/61`. Si non → STOP et remonte l'anomalie.
- **1 seul commit logique**, message gitmoji en **français**, référence `#307`.
- **Code en anglais, docs/commentaires/commits en français** (convention projet).
- **TypeScript strict** — pas de `any`, pas de `@ts-ignore`.
- **i18n obligatoire** : aucune chaîne en dur dans le JSX. Toute clé ajoutée dans
  `frontend/public/locales/fr/products.json` doit l'être **aussi** dans `en`, `de`, `es`
  (le namespace = nom du fichier, cf. `frontend/i18n.ts`).
- **Tests inline OBLIGATOIRES** via `./scripts/test-quiet.sh` (scope frontend). Si le volume dépasse
  500 tests ou 3 min → renvoie `RECOMMAND_TEST_RUNNER` plutôt que de bloquer.
- **E2E** : ajoute un spec Playwright couvrant archiver → retrouver → ré-éditer → désarchiver.
  Respecte l'isolation des seeds et les conventions de sélecteurs du projet.
  Tout nouveau `data-testid` que tu introduis DOIT être référencé par un spec E2E
  (un check de couverture automatique tourne en fin de sprint et bloque la PR sinon).
- **Piège E2E connu (mémoire projet)** : un `curl` qui réussit ne disculpe PAS un souci CORS
  (curl n'envoie pas d'`Origin`). Lis les statuts instrumentés par la fixture avant toute hypothèse.
- **Piège jsdom connu** : un test de scroll sous jsdom ne prouve rien (jsdom ne clampe pas
  `scrollLeft`). Si ton assertion porte sur du scroll/viewport → exige un E2E.
- **Piège outillage** : `git diff` renvoie ~vide sous le hook RTK de ce poste.
  Utilise `rtk proxy git diff` ou redirige vers un fichier puis lis-le.

## Livrable attendu (format strict, MAX 500 tokens, style caveman — pas de prose)
Écris ton rapport final au format :
```
RETOUR :
- commits: [SHA]
- resume: <objectif + BR touchées + fichiers clés + pitfalls rencontrés + tests passés/total>
- decision_produit: <comment tu as matérialisé l'Option A en UI, 2 lignes max>
- [MEMORY:pitfall|pattern|decision|business-rule] <signal si applicable>
- recommandations suite: <RECOMMAND_TEST_RUNNER | RECOMMAND_SECURITY | RECOMMAND_UI_DESIGN
  | RECOMMAND_FOLLOWUP: <desc> [triage XS/S/M/L] | ou négation explicite "Pas de RECOMMAND_X car ...">
- ABSORBED: <découvertes XS intégrées au passage, ou "aucune">
STATUS: COMPLETED
```
Dernière ligne exactement `STATUS: COMPLETED` (ou `STATUS: PARTIAL` + une section `BLOQUE_SUR`).
