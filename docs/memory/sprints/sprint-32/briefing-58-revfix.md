[BRIEFING SPRINT 32 — CORRECTIFS REVIEW issue #58 (post-review batch)]

## Contexte
Export RGPD (#58) livré sur `sprint/32` (commits `0d1d739` + `f663d98`, 353/353 verts).
Le reviewer batch a relevé **1 MAJEUR + 2 MINEUR**. Périmètre STRICT : ces 3 points + tests.
Ne PAS refactorer le reste. Aucun CRITIQUE.

## Correctif 1 — [MAJEUR] `ExportJob.markCompleted` dérive `completedAt` par magie
`domain/models/export/ExportJob.java` (~l.52-58) :
```java
public void markCompleted(String storageRef, LocalDateTime expiresAt) {
    ...
    this.completedAt = expiresAt.minusHours(24);   // ← magic 24h, duplique le TTL applicatif
    this.expiresAt = expiresAt;
}
```
`completedAt` est reconstruit en soustrayant 24h (magic number) à `expiresAt`, ce qui **duplique
silencieusement** `AsyncExportRunner.DOWNLOAD_TTL_HOURS`. Si le TTL change un jour (24h → autre)
sans toucher ce fichier domaine, `completedAt` devient FAUX et aucun test ne le détecte.

**Fix attendu** : passer `completedAt` en **paramètre explicite** :
`markCompleted(String storageRef, LocalDateTime completedAt, LocalDateTime expiresAt)`.
Mettre à jour l'unique appelant (`AsyncExportRunner`) : il connaît déjà `now` et calcule
`expiresAt = now.plus(TTL)` → passer `now` comme `completedAt`. Supprimer le `.minusHours(24)`.
Le domaine ne doit plus connaître la durée du TTL (règle : le TTL vit dans la couche application).

## Correctif 2 — [MINEUR] `AsyncConfig.exportExecutor` : commentaire faux + rejet par défaut
`infrastructure/config/AsyncConfig.java` (~l.31, bean `exportExecutor`) : le commentaire affirme
« à saturation, la politique par défaut (CallerRunsPolicy) applique un backpressure sans perdre de
job ». **C'est faux** : `ThreadPoolTaskExecutor` de Spring utilise `AbortPolicy` par défaut →
à saturation (queue=50 pleine, maxPool atteint), la tâche est **rejetée** avec
`RejectedExecutionException` non catchée → 500 côté requête + **job laissé PENDING orphelin** en DB.

**Fix attendu** : configurer explicitement `executor.setRejectedExecutionHandler(
new ThreadPoolExecutor.CallerRunsPolicy())` sur le bean `exportExecutor` (aligne le comportement
réel sur l'intention documentée : backpressure, pas de perte de job). Garder le commentaire (il
devient vrai). **Périmètre : UNIQUEMENT `exportExecutor`.** Le bean `passwordResetExecutor` a le
même commentaire trompeur mais il est HORS PÉRIMÈTRE de ce sprint (pré-existant) → NE PAS le
toucher, signale-le en `RECOMMAND_FOLLOWUP`.

## Correctif 3 — [MINEUR] `CsvExportRenderer.neutralizeFormula` oublie `\n` en tête
`infrastructure/adapters/export/CsvExportRenderer.java` (~l.110) : `neutralizeFormula` déclenche
sur `= + - @ \t \r` mais PAS sur `\n` en tête de champ (caractère déclencheur Excel/Sheets).
**Fix attendu** : ajouter `\n` à la liste des caractères déclencheurs (cohérence OWASP CSV
injection). Étendre le test existant `csv_neutralizesFormulaInjectionOnUserControlledFields`
(ou en ajouter un) pour couvrir le champ commençant par `\n`.

## État réel
Fichiers touchés UNIQUEMENT :
- `domain/models/export/ExportJob.java` (signature `markCompleted` — domaine PUR, aucun import framework)
- `application/services/AsyncExportRunner.java` (unique appelant de `markCompleted`)
- `infrastructure/config/AsyncConfig.java` (bean `exportExecutor` seulement)
- `infrastructure/adapters/export/CsvExportRenderer.java` (+ son test)
- tests associés (`ExportRenderersTest`, éventuellement un test domaine `completedAt`)
...
## Contraintes
- Branche cible : `sprint/32` (déjà checkout — vérifie `git branch --show-current`).
- Périmètre STRICT : les 3 correctifs ci-dessus + tests. Rien d'autre.
- `ExportJob` est en couche `domain/` → PUR Java, AUCUN import Spring/JPA/Jakarta.
- Commit : 1 commit logique gitmoji **français**, `git add` CIBLÉ sur les fichiers touchés
  (JAMAIS `git add -A` — worktree partagé).
- Tests OBLIGATOIRES via `./scripts/test-quiet.sh unit` — la suite ENTIÈRE doit rester verte
  (baseline actuelle : **353/353**). En particulier :
  - vérifier que le nouvel appel `markCompleted(storageRef, completedAt, expiresAt)` est cohérent
    dans `AsyncExportRunner` et que les tests d'intégration async (job COMPLETED → downloadUrl +
    expiresAt) restent verts ;
  - test CSV `\n` en tête neutralisé.
- Idéalement, ajouter un test domaine léger prouvant que `completedAt` == la valeur passée (et non
  `expiresAt - 24h`), pour verrouiller le MAJEUR contre régression (le reviewer a noté qu'aucun
  test ne couvrait `completedAt`).
- Ne PAS toucher : `passwordResetExecutor` (hors scope), `ExportTokenService`, `ExportController`,
  migration V13, snapshot `UserDataExport`, autres renderers.

## Livrable — écris `docs/memory/sprints/sprint-32/issue-58-revfix-done.md`
(MAX 400 tokens caveman)
RETOUR :
- commits: [SHA1]
- resume: <3 correctifs + fichiers + tests + total tests passés/total>
- [MEMORY:*] signaux: <si applicable>
- recommandations suite: <RECOMMAND_FOLLOWUP passwordResetExecutor comment ; autres ou "aucun car ...">
