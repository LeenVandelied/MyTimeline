[BRIEFING PR #263 — CORRECTIFS REVIEW (2 MINEUR propres)]

## Contexte
PR #263 (export RGPD #58, branche `sprint/32`) a été re-reviewée : **0 CRITIQUE, 0 MAJEUR
bloquant**. 2 MINEUR propres/sûrs à corriger. Le MAJEUR « storage-path partagé » est traité
séparément en follow-up (NE PAS y toucher). Périmètre STRICT = ces 2 points + garder 355/355 verts.

## Correctif 1 — [MINEUR] `ExportServiceImpl.download` tient une connexion DB pendant l'I/O disque
`application/services/ExportServiceImpl.java` (l.94-96, méthode `download`) :
```java
@Override
...
public Optional<RenderedExport> download(UUID jobId, UUID ownerId) {
    ...
    ...
    return storagePort.load(job.getStorageRef())...   // ← I/O DISQUE bloquante DANS la tx
}
```
Le `@Transactional(readOnly=true)` maintient une connexion DB ouverte pendant la lecture du
fichier sur disque (`storagePort.load`). Or `download` ne fait qu'UN seul `SELECT`
(`findByIdAndOwnerId`) — aucune atomicité multi-statements n'est requise.

**Fix attendu** : **retirer `@Transactional(readOnly = true)` de la méthode `download`**. Le
`SELECT` unique du repository ouvre/ferme sa propre connexion ; l'I/O disque se fait ensuite
sans monopoliser une connexion du pool. NE PAS toucher à `getJob` ni `submit` (leur annotation
reste). Comportement fonctionnel inchangé (mêmes retours/404).

## Correctif 2 — [MINEUR] `CsvExportRenderer.render` dépasse la convention ≤20 lignes
`infrastructure/adapters/export/CsvExportRenderer.java` (l.30-68 env, méthode `render`) : ~38
lignes, au-dessus de la convention projet (méthodes ≤20L). Elle assemble plusieurs sections
(profil, catégories, produits, événements) dans un `StringBuilder`.

**Fix attendu** : extraire chaque section dans un helper privé dédié (ex.
`appendProfileSection(StringBuilder, UserDataExport)`, `appendCategoriesSection(...)`, etc.,
ou des helpers retournant les lignes). `render` orchestre les appels. **La sortie CSV doit
rester BYTE-IDENTIQUE** (mêmes en-têtes, même ordre, même échappement `row`/`escape`/
`neutralizeFormula`). Les tests existants (`ExportRenderersTest`) verrouillent la sortie —
ils doivent rester verts SANS modification. Ne PAS toucher `row`/`escape`/`neutralizeFormula`.

## État réel
Fichiers touchés UNIQUEMENT :
- `application/services/ExportServiceImpl.java` (retrait annotation sur `download`)
- `infrastructure/adapters/export/CsvExportRenderer.java` (extraction helpers `render`)
...
- Périmètre STRICT : les 2 correctifs ci-dessus. Rien d'autre (le storage-path est un follow-up séparé).
- Commit : 1 commit logique gitmoji **français** (ex. `:recycle:` ou `:zap:`), `git add` CIBLÉ
  sur les 2 fichiers (JAMAIS `git add -A` — worktree partagé). Puis `git push origin sprint/32`.
- Tests OBLIGATOIRES via `./scripts/test-quiet.sh unit` — la suite ENTIÈRE doit rester
  **355/355 verte**, SANS modifier les tests existants (la sortie CSV doit être byte-identique,
  le comportement de `download` inchangé). Si un test CSV casse → ta refacto a changé la sortie, corrige.
- Ne PAS toucher : `getJob`/`submit` (tx), `row`/`escape`/`neutralizeFormula`, storage/config,
  migration, tokens, autres renderers.

## Livrable — écris `docs/memory/sprints/sprint-32/issue-58-prfix-done.md`
(MAX 300 tokens caveman)
RETOUR :
- commits: [SHA1]
- resume: <2 correctifs + fichiers + confirmation sortie CSV identique + total tests passés/total>
