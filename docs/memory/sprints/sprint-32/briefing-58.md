[BRIEFING ISSUE #58 — Sprint 32]

## Issue
[FEATURE] Backend : export des données utilisateur (RGPD)

### Contexte
Aucune fonction d'export des données utilisateur n'existe. Cette absence bloque la
conformité RGPD (droit à la portabilité, Art. 20 — copie lisible de toutes ses données,
délai légal 1 mois). Objectif : endpoint d'export couvrant l'intégralité du périmètre RGPD.

### À faire
- **Scope données** : profil utilisateur + produits + événements + catégories (toutes les
  entités appartenant au user authentifié).
- **Formats synchrones (inline)** : JSON et Markdown — génération directe, retour immédiat
  dans la réponse HTTP.
- **Formats asynchrones (job)** : ZIP et CSV — génération en job avec polling ; une URL
  signée expirant sous 24h est retournée au client une fois le job terminé.
- Endpoint dédié protégé par l'auth existante (ownership : uniquement les données du user
  authentifié).
- Journalisation de chaque demande d'export (audit trail minimal : userId, timestamp, format).

### Critères d'acceptation
- [ ] `GET /api/…/export?format=json` retourne toutes les données du user en JSON (200).
- [ ] `GET …/export?format=markdown` retourne les données formatées en Markdown.
- [ ] `POST …/export?format=zip` déclenche un job asynchrone et retourne un `jobId`.
- [ ] `GET …/export/job/{jobId}` retourne le statut du job et, terminé, une URL signée 24h.
- [ ] `GET …/export?format=csv` fonctionne de façon analogue au ZIP (async).
- [ ] Un user NE PEUT PAS accéder au job/fichier d'un autre user (test ownership).
- [ ] L'URL signée expire bien après 24h (test d'expiration).
- [ ] Chaque demande d'export est tracée (log ou table audit).

### Piste technique (issue)
- Nouveau `ExportController` + use case export (architecture hexagonale stricte).
- Agréger via les repositories existants : `UserRepository`, `ProductRepository`,
  `EventRepository`, `CategoryRepository`.
- Job asynchrone : `@Async` Spring OU table `export_jobs` en DB (solution légère, pas de MQ).
- URL signée : #34 (secrets externalisés) est CLOSED. Voir contrainte stockage ci-dessous.
- Jackson `ObjectMapper` (JSON), OpenCSV / Apache Commons CSV (CSV), `java.util.zip.ZipOutputStream` (ZIP).

### Risques techniques (issue)
- Pas de file de jobs aujourd'hui : `@Async` + table DB suffisant pour MVP, à documenter en dette.
- Génération ZIP/CSV serveur : mémoire élevée pour gros volumes — envisager streaming/pagination interne.
- URL signée : dépend du stockage — prévoir fallback token interne (voir contrainte ci-dessous).

## État réel du code (relevé par le lead — À NE PAS RE-DEVINER)

**Package racine** : `com.matimeline.eventmanager`. Hexagonal strict (imports Spring/JPA
interdits dans `domain/`, cf. pack ci-dessous).

**Structure réelle** (≠ des chemins « … » du mini-plan architect, qui sont indicatifs) :
- `domain/models/` — entités métier PUR Java : `User`, `Product`, `Event`, `Category`,
  `Session`, `PasswordResetToken`, `Role`, `AvatarContent`, `Recurrence*`.
- `domain/ports/repositories/` — `UserRepository`, `ProductRepository`, `EventRepository`,
  `CategoryRepository`, `SessionRepository`, `PasswordResetTokenRepository`.
- `domain/ports/services/` — ports de services (dont `StoragePort`, `AvatarService`…).
- `application/{dtos,mappers,services}/` — DTOs, mappers, implémentations des ports (use cases).
- `infrastructure/adapters/controllers/` — `AuthController`, `UserController`,
  `ProductController`, `EventController`, `CategoryController`, `SessionController`,
  `GlobalExceptionHandler`. **Étudie `ProductController`/`CategoryController` pour le
  pattern ownership (récupération du user authentifié + filtrage par propriétaire).**
- `infrastructure/adapters/repositories/jpa/` — impls JPA. `infrastructure/entities/` — entities JPA.
- `infrastructure/security/` — JWT (JwtService/JwtFilter), extraction du user courant.

**Périmètre RGPD à exporter (entités possédées par le user)** : `User` (profil),
`Product`, `Event`, `Category`. → 4 repositories déjà listés ci-dessus, tous filtrables
par owner. **Complétude légale = ne rien omettre.** Décision à trancher et documenter dans
l'ADR : inclure ou non les métadonnées `Session` (sessions actives) et
`PasswordResetToken` ? Recommandation : exclure les tokens (secrets internes, pas des
« données personnelles » au sens portabilité), mentionner explicitement l'exclusion dans
l'ADR pour tracer le choix.

**Migrations = Flyway** (PAS ddl-auto malgré une note CLAUDE.md périmée) :
`backend/src/main/resources/db/migration/` contient V1→V12. **Prochaine migration = V13.**
Si tu retiens une table de suivi des jobs : `V13__export_jobs.sql` (numérotation stricte,
UNE seule plage réservée à ce sprint).

**Stockage & URL signée — CONTRAINTE FORTE** : `StoragePort` (ADR Sprint 21) est un
stockage **LOCAL** (`LocalStorageAdapter`), PAS S3/MinIO — il n'y a **AUCUN presignedUrl**
disponible. La référence renvoyée par `store()` est opaque. Donc l'« URL signée 24h » de
l'async DOIT être : un **endpoint de téléchargement interne** (`GET …/export/download/{ref}`)
protégé par un **token court signé** (HMAC/JWT courte durée, réutiliser le mécanisme de
signature existant — cf. `infrastructure/security/`). Ne PAS inventer d'intégration S3.
...
issue_58:
  ...
  strategie_test: |
    - domain: test unitaire pur assemblage snapshot (profil+produits+événements+catégories),
      aucune fuite de champ tiers
    - application: use case orchestration (sync JSON/MD inline vs async ZIP/CSV)
    - infrastructure: test intégration endpoint (JSON+MD inline 200), job async
      (soumission→polling→URL signée valide 24h→expirée après)
    - respecter hexagonal : ports d'abord, adapters ensuite
  risque_regression: |
    - RGPD = complétude légale : OMETTRE une entité = non-conformité → checklist exhaustive
      des données perso avant merge (User, Product, Event, Category)
    - async sans infra existante = risque archi majeur → ADR REQUIS
      (choix: @Async/ThreadPool vs table+scheduler vs futur queue). [MEMORY:decision] au lead.
  ...
  ordre_ecriture: "ADR job async → domain snapshot + ExportPort → use case →
    JsonExporter+MarkdownExporter (chemin SYNC d'abord) → ExportController sync →
    puis ZipCsv+job async+polling+URL signée → migration V13 si retenue"
  zod_dto_sync: "OUI — schéma de réponse export (statut job + format) à FIGER et documenter
    côté contrat : #59 (frontend, Sprint 33) s'aligne dessus. Source de vérité = ce sprint."
```
...
- Sprint mono-issue : aucune dépendance intra-sprint.
- **#34 (secrets externalisés) : CLOSED** — prérequis satisfait.
- **Ce sprint BLOQUE #59 (frontend export, Sprint 33)** : le **contrat/DTO de réponse
  export** (statut job, format, structure du snapshot) doit être **figé et documenté** en
  fin de ce sprint. C'est la source de vérité pour l'alignement frontend S33.

## ADR REQUIS (livrable bloquant)
Aucune infra de jobs async n'existe aujourd'hui. Tu DOIS produire un ADR avant de coder
l'async :
- Fichier : `docs/adr/ADR-<num>-export-rgpd-async-job.md` (prends le prochain numéro libre).
- Décision : mécanisme async (`@Async`+ThreadPool vs table `export_jobs`+scheduler vs futur MQ),
  mécanisme d'URL signée (endpoint interne + token HMAC/JWT courte durée), rétention/expiration 24h,
  périmètre exact des entités exportées (et exclusion tokens/sessions justifiée).
- Signaler `[MEMORY:decision]` au lead dans le retour.
- **Le chemin SYNC (JSON/MD inline) reste livrable même si l'async glisse** — implémente-le
  et teste-le en premier ; ne bloque pas la livraison du sync sur l'async.

## Designer
Non applicable (backend pur, aucune surface visuelle).

## Contraintes
- Branche cible : `sprint/32` (déjà checkout, ne pas changer de branche).
- Architecture hexagonale STRICTE : `domain/` sans aucun import Spring/JPA/Jakarta
  (sauf `jakarta.validation`). Ports d'abord, adapters ensuite. Voir le pack ci-dessus.
- Convention create : `new X(null, ...)` pour les nouvelles entités persistées
  (@Version + @GeneratedValue rejettent un id pré-assigné) ; `getReference` pour les associations.
- Commit : 1 commit logique gitmoji **français** (`git add` ciblé sur les fichiers du scope,
  JAMAIS `git add -A` — worktree partagé).
- Tests OBLIGATOIRES inline via `./scripts/test-quiet.sh unit` (backend, Testcontainers/Docker requis).
  Couvre : snapshot domain exhaustif, use case sync/async, endpoint (JSON/MD 200), ownership
  (user A ne voit pas les données de B), job (soumission→polling→URL 24h→expirée).
- Si volume tests > 500 OU temps > 3min : signale `RECOMMAND_TEST_RUNNER` (le lead lancera test-runner).
- **Auth/PII/ownership + URL signée** : signale `RECOMMAND_SECURITY` (le lead lancera security-expert).
- **Migration V13 ajoutée** : signale `RECOMMAND_DB_EXPERT` (le lead lancera db-expert).
- Logs : aucune PII en clair (userId/email masqués via `maskId(`/`maskEmail(` si loggés).
- Ne PAS toucher aux fichiers hors scope export (autres controllers/services non liés).

## Livrable attendu (format strict, MAX 500 tokens caveman — écrit dans issue-58-done.md)
RETOUR :
- commits: [SHA1, ...]
- resume: <objectif + entités exportées + fichiers clés (domain/app/infra) + ADR produit + pitfalls + tests passés/total>
- [MEMORY:*] signaux: <[MEMORY:decision] ADR async ; autres si applicables>
- recommandations suite: <RECOMMAND_SECURITY / RECOMMAND_DB_EXPERT / RECOMMAND_TEST_RUNNER / RECOMMAND_FOLLOWUP ou "aucun car …">
- contrat DTO export figé: <résumé structure réponse pour #59>
