[BRIEFING SPRINT 32 — CORRECTIFS SÉCURITÉ issue #58 (post-audit)]

## Contexte
L'export RGPD (#58) est livré (commit `0d1d739`, branche `sprint/32`, 351/351 tests verts).
L'audit security-expert a relevé **1 MAJEUR + 1 MINEUR** à corriger AVANT merge. Périmètre
strict : ces deux findings uniquement. Ne PAS refactorer le reste de l'export.

## Correctif 1 — [MAJEUR] Rate-limit absent sur `POST /api/export`
`infrastructure/security/RateLimitingFilter.java` : la map `PATH_LIMITS` (l.51-59) ne
couvre que les routes `/api/auth/*`. `throttledLimitFor()` throttle uniquement les POST via
`PATH_LIMITS.get(request.getRequestURI())` (match EXACT sur l'URI, POST seulement). Donc
`POST /api/export?format=zip|csv` (soumission de job async) N'EST PAS throttlé → un user
authentifié peut spammer les soumissions : épuisement du pool `AsyncConfig` (queue=50,
`CallerRunsPolicy`) + accumulation illimitée de fichiers sur disque (aucun quota).

**Fix attendu** : ajouter une entrée `"/api/export"` dans `PATH_LIMITS` avec une limite/min
raisonnable pour une opération coûteuse (aligner sur l'ordre de grandeur des slots existants,
ex. 5/min/IP comme `forgot-password`/`reset-password` — opération lourde). Documenter d'un
commentaire (comme les entrées #49/#33). Le match est exact sur `/api/export` (le query
string n'est pas dans `getRequestURI()`), donc l'entrée simple suffit.
- **NB** : ce filtre s'applique aux POST. Les GET (`/api/export?format=json|markdown`,
  `/api/export/job/{id}`, `/api/export/download/{id}`) ne passent pas par ce throttle — c'est
  attendu (le MAJEUR concerne la soumission de jobs). Ne PAS élargir aux GET.

## Correctif 2 — [MINEUR] CSV injection (préfixe formule)
`infrastructure/adapters/export/CsvExportRenderer.java` (l.82-90 env) : `escape()` fait
l'échappement RFC4180 (guillemets/virgules/retours ligne) mais ne neutralise PAS le préfixe
formule sur les champs user-controlled (`product.name`, `event.title`, `category.description`,
etc.). Un champ commençant par `= + - @` (aussi tab `\t` / CR selon durcissement) est
interprété comme formule si le CSV est rouvert dans Excel/Google Sheets. Surface élargie car
RGPD Art.20 autorise la transmission du fichier à un tiers.

**Fix attendu** : dans `escape()` (ou une étape amont dédiée), si la valeur commence par un
caractère à risque (`= + - @`, et idéalement tab/CR), préfixer une apostrophe `'` (mitigation
standard OWASP CSV injection) AVANT l'échappement RFC4180 des guillemets. Garder le
comportement RFC4180 existant pour le reste.

## État réel
- Structure hexagonale : `domain/` pur, `application/services`, `infrastructure/{security,adapters/export}`.
- Le fix touche UNIQUEMENT `infrastructure/security/RateLimitingFilter.java` et
  `infrastructure/adapters/export/CsvExportRenderer.java` (+ leurs tests).
...
- Périmètre STRICT : les 2 correctifs ci-dessus + leurs tests. Rien d'autre.
- Commit : 1 commit logique gitmoji **français** (ex. `:lock:`), `git add` CIBLÉ sur les
  fichiers touchés (JAMAIS `git add -A` — worktree partagé).
- Tests OBLIGATOIRES via `./scripts/test-quiet.sh unit` — la suite ENTIÈRE doit rester verte
  (baseline actuelle : 351/351). Ajoute :
  - Rate-limit : un test prouvant que `POST /api/export` au-delà de la limite renvoie 429
    (s'inspirer de `RateLimitingAndHeadersIntegrationTest` — `@SpringBootTest` +
    `@AutoConfigureMockMvc`, le `standaloneSetup` bypasse le filtre donc ne teste PAS le 429).
  - CSV injection : un test unitaire sur `CsvExportRenderer` prouvant qu'un champ `=cmd`
    devient `'=cmd` (ou équivalent neutralisé) dans la sortie CSV, sans casser l'échappement
    RFC4180 existant (guillemets/virgules).
- Ne PAS toucher : `ExportTokenService`, `ExportController`, migration V13, snapshot domain,
  les autres renderers (Json/Markdown/Zip) — hors périmètre.

## Livrable — écris `docs/memory/sprints/sprint-32/issue-58-secfix-done.md`
(MAX 400 tokens caveman)
RETOUR :
- commits: [SHA1]
- resume: <2 correctifs + fichiers + tests ajoutés + total tests passés/total>
- [MEMORY:*] signaux: <si applicable>
- recommandations suite: <RECOMMAND_* ou "aucun car ...">
