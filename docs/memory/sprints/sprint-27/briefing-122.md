...
## Issue
[CHORE] Poser une contrainte CHECK + NOT NULL sur users.role en base de données

### Contexte
La colonne `users.role` stocke le rôle de chaque utilisateur (ROLE_USER ou ROLE_ADMIN)
sous forme de texte libre. Aucune contrainte en base ne garantit que seules ces deux valeurs
peuvent être insérées. Une valeur incorrecte (erreur de code, migration manquante) est acceptée
silencieusement → données incohérentes.

### Description
Auditer la colonne `users.role` par rapport à la baseline V1 (drift identifié comme cause commune
avec #108). Constat déjà confirmé : `V1__baseline.sql:30` = `role varchar(255)` NULLABLE, aucun
CHECK, aucune migration ultérieure ne le durcit. Poser un script de migration Flyway qui ajoute :
- `NOT NULL` sur `users.role`
- Contrainte `CHECK (role IN ('ROLE_USER', 'ROLE_ADMIN'))`

> ⚠ **NUMÉRO DE VERSION — CORRECTION IMPORTANTE**
> Le body original de l'issue mentionne "V6". C'est PÉRIMÉ : V6 existe déjà
> (`V6__create_password_reset_tokens.sql`). La dernière migration présente est **V11**.
> Ta migration DOIT être nommée **`V12__users_role_not_null_check.sql`**.
> Vérifie toi-même `ls backend/src/main/resources/db/migration/` avant d'écrire.

### Critères d'acceptation
- [ ] La divergence baseline V1 est documentée (déjà : role nullable, sans CHECK).
- [ ] La migration V12 ajoute NOT NULL + CHECK sur `users.role`.
- [ ] La migration passe sur une base VIDE et sur une base existante AVEC données.
- [ ] Les tests d'intégration existants passent toujours après la migration.

### Risques techniques (cf. précédent V4 self-safe sur base peuplée, PIT Sprint 5)
- Si des lignes `role NULL` ou hors-enum existent déjà, l'ALTER échoue → il FAUT un `UPDATE`
  correctif AVANT d'ajouter la contrainte (pattern pré-vol self-safe, cf. V4__reconcile_events).
  Ordre : `UPDATE users SET role='ROLE_USER' WHERE role IS NULL OR role NOT IN (...)` → `ALTER ... SET NOT NULL`
  → `ADD CONSTRAINT ... CHECK`. Justifier tout défaut choisi pour les NULL (ROLE_USER = moindre privilège).
- Vérifier la numérotation Flyway pour éviter tout doublon de version.

## Plan d'implémentation (architect, /sprint plan)
```yaml
issue_0122:
  fichiers_cles: ["backend/src/main/resources/db/migration/V12__users_role_not_null_check.sql (a creer)", "domain/... User entity"]
  ...
  risque_regression: "UPDATE des lignes role NULL existantes AVANT ALTER NOT NULL, sinon la migration échoue sur base réelle. ALTER TABLE = opération sensible."
...
- Vague 1 : tu tournes EN PARALLÈLE avec #93 (refactor contrôleurs — fichiers disjoints).
  Tu ne touches QUE la migration Flyway + l'entité User côté role. Aucun contrôleur.
- Aucune dépendance bloquante en aval.

## Designer
Non applicable (migration DB pure).
...
- Commit : 1 commit logique, gitmoji français (ex: `:card_file_box: #122 ...` ou `:lock: #122 ...`).
- Migration : `V12__users_role_not_null_check.sql` — self-safe sur base peuplée (UPDATE avant ALTER).
- Tests OBLIGATOIRES inline via `./scripts/test-quiet.sh backend` (ou `backend/./mvnw test`) :
  boot Flyway applique V12, insertion role=null rejetée, role invalide rejeté, rôles valides OK.
- ⚠ **NE PAS exécuter la migration contre une base de PRODUCTION.** L'ALTER TABLE est une opération
  sensible (cf. CLAUDE.md). Le rôle du subagent = écrire la migration + la valider via Testcontainers/
  base de test locale UNIQUEMENT. La bascule prod reste une décision humaine (mentionne-la en
  recommandation).
- Ne PAS toucher aux fichiers : contrôleurs (réservés #93/#154/#92).

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: <objectif + fichier migration V12 + entité modifiée + tests passés/total>
- migration: <résumé des 3 étapes SQL : UPDATE nulls -> SET NOT NULL -> ADD CHECK>
- [MEMORY:*] signaux: <pitfall/pattern/decision si applicable>
- recommandations suite: <RECOMMAND_DB_EXPERT si doute rollback/index, note bascule prod, ou "RAS">
