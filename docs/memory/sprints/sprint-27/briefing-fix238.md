[BRIEFING FIX REVIEW — PR #238 / Sprint 27]

## Contexte
Review indépendante `/review-pr 238` a trouvé 1 MAJEUR (self-DoS session) + 1 MINEUR défensif.
Tu corriges ces findings. Correction CIBLÉE et minimale, pas de refacto large.

## FINDING 1 — [MAJEUR] SessionController self-DoS via Bearer (CORRIGER)
Fichier : `backend/src/main/java/com/matimeline/eventmanager/infrastructure/adapters/controllers/SessionController.java`

Depuis #93, l'identité est résolue via `CallerResolver.currentUser()` (SecurityContext, cookie **OU**
Bearer). MAIS le `jti` de la session courante est encore extrait du COOKIE seul :
```java
@DeleteMapping("/others")
public ResponseEntity<?> revokeOtherSessions(
        @CookieValue(value = "jwt", required = false) String token) {
    ...
    String currentJti = extractJtiOrNull(token);          // token = cookie UNIQUEMENT
    sessionService.revokeOtherSessions(callerOpt.get().getId(), currentJti);
```
Conséquence : un client **Bearer-only** (pas de cookie) → `token = null` → `currentJti = null`.
Or `SessionServiceImpl.revokeOtherSessions(userId, null)` appelle `revokeAllByUserIdExcept(userId, null)`
qui est le code path IDENTIQUE à `revokeAllSessions(userId)` → **révoque TOUTES les sessions du user,
y compris celle de l'appel** (self-DoS). Le même trou existe sur `GET /api/sessions` (getActiveSessions) :
`currentJti = null` → aucune session marquée "courante" (cosmétique mais même cause).

### Correctif attendu
Résoudre le token de la MÊME façon que `JwtFilter` (cookie `jwt` en priorité, sinon header
`Authorization: Bearer <token>`), pour que `extractJtiOrNull` fonctionne dans les deux modes d'auth.
Référence exacte `JwtFilter.java` (à imiter) :
```java
// cookie "jwt" d'abord
if (request.getCookies() != null) { for (Cookie c : ...) if ("jwt".equals(c.getName())) token = c.getValue(); }
// sinon Authorization: Bearer
String authorizationHeader = request.getHeader("Authorization");
if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) token = authorizationHeader.substring(7);
```
Implémentation suggérée dans SessionController : ajouter
`@RequestHeader(value = "Authorization", required = false) String authHeader` aux deux méthodes
(`getActiveSessions`, `revokeOtherSessions`), et un helper privé `resolveToken(cookieToken, authHeader)`
qui renvoie le cookie s'il est non vide, sinon le Bearer (substring(7)), sinon null. Puis
`extractJtiOrNull(resolveToken(...))`. NE change PAS la signature de `SessionService` ni la logique de
révocation côté service — seule la SOURCE du token change dans le contrôleur.

> Défense en profondeur optionnelle (à ta discrétion, ne casse aucun test) : tu peux documenter en
> commentaire que si `currentJti` reste null (token legacy sans claim jti), `revokeOtherSessions`
> dégénère en revoke-all — mais le fix principal (résolution Bearer) suffit pour ce finding.

## FINDING 2 — [MINEUR] CallerResolver : token anonyme explicite (CORRIGER, défensif cheap)
Fichier : `backend/src/main/java/com/matimeline/eventmanager/infrastructure/security/CallerResolver.java`

Actuellement, un `AnonymousAuthenticationToken` a `isAuthenticated() == true` et `getName() ==
"anonymousUser"` ; le retour `Optional.empty()` ne vient QUE du fait qu'aucun User "anonymousUser"
n'existe en base — coïncidence fragile (un vrai compte nommé "anonymousUser" contournerait). Ajouter
un court-circuit EXPLICITE avant le lookup DB :
```java
...
```
Adapte à la structure réelle du fichier (lis-le d'abord). Comportement observable inchangé, juste durci.
...
## Tests attendus
- **Nouveau test SessionController** : un appel `DELETE /api/sessions/others` authentifié via **Bearer**
  (header Authorization, PAS de cookie) NE doit PAS révoquer la session courante — le `jti` courant doit
  être résolu et transmis à `revokeOtherSessions`, donc préservé. Regarde le test existant de SessionController
  (`SessionControllerTest` s'il existe, ou un test d'intégration sessions) et calque le style. Vérifie que
  le mock/asserts prouvent que `revokeOtherSessions(userId, <jti-non-null>)` est appelé (jti non null) quand
  l'identité vient d'un Bearer.
- Les tests SessionController existants (mode cookie) restent verts.
- Suite backend complète verte.

## Contraintes
- Branche cible : `sprint/27` (déjà checkout).
- ⚠ **GARDE-FOU WORKTREE (obligatoire — 3 agents de ce sprint se sont trompés de repo)** :
...
- ⚠ **Commit propre** : `git add` UNIQUEMENT les fichiers source/test que tu modifies (SessionController.java,
  CallerResolver.java, le test sessions). Le worktree contient des fichiers docs sprint non suivis
  (briefings, done.md, spawn-refs) — NE PAS les inclure. Utilise `git add <chemin précis>`, PAS `git add -A`.
- Commit : 1 commit logique, gitmoji français (ex: `:bug: #73/#93 fix review — SessionController jti cookie+Bearer (anti self-DoS)`).
  Après commit : `git push origin sprint/27`.
...
RETOUR :
- commits: [SHA1, ...]
- fixes: <FINDING 1 : comment résolu + FINDING 2 : comment résolu>
- test ajouté: <nom classe#méthode + ce qu'il prouve>
- tests: <passés/total backend>
- push: <OK/KO>
