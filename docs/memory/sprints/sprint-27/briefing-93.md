...
## Issue
[REFACTOR] Unifier l'extraction d'identité via SecurityContextHolder

Follow-up de la review PR #91 (Sprint 1 — sécurité backend).

Plusieurs controllers ré-extraient manuellement le JWT depuis le cookie brut
(`@CookieValue` + `jwtService.extractUsername`) alors que `JwtFilter` a déjà validé
le token et peuplé `SecurityContextHolder` avant d'atteindre le controller. Cette
double extraction crée une incohérence : une requête authentifiée par header
`Authorization: Bearer` (valide côté `JwtFilter`) est rejetée en 401 par le check
manuel côté cookie, alors qu'elle devrait être acceptée.

### Objectif
Remplacer toutes les extractions manuelles du JWT dans les controllers par :
```java
String username = SecurityContextHolder.getContext().getAuthentication().getName();
```
Cette valeur est garantie non-nulle à ce stade (la requête a passé `JwtFilter`).
Supprimer les paramètres `@CookieValue` et les appels `jwtService.extractUsername`
correspondants dans les controllers concernés.

### Piste technique (élargie par l'architect au-delà du body issue)
Le body original ne cite qu'EventController + ProductController, mais l'audit code
montre 4-5 méthodes privées `resolveCaller` DUPLIQUÉES à travers les contrôleurs.
L'objectif est de factoriser en UN helper unique (ex: `CallerResolver` dans
`infrastructure/security/`) qui résout l'identité via `SecurityContextHolder`, puis
de rebrancher CHAQUE contrôleur dessus.

Fichiers concernés (résolus) :
- `infrastructure/adapters/controllers/EventController.java` (resolveCaller ~170-177)
- `infrastructure/adapters/controllers/CategoryController.java` (resolveCaller ~172-182)
- `infrastructure/adapters/controllers/UserController.java` (resolveCaller ~270-280)
- `infrastructure/adapters/controllers/SessionController.java` (resolveCaller)
- `infrastructure/adapters/controllers/ProductController.java` (auth INLINE, pas de resolveCaller — cf. NOTE ci-dessous)
- `infrastructure/security/` → créer le helper `CallerResolver`

> **NOTE COORDINATION INTRA-SPRINT — LIRE ATTENTIVEMENT**
> ProductController.java est aussi la cible des issues #154 et #92 qui s'exécutent
> APRÈS toi (vague 2). Pour minimiser les conflits :
> - Tu CRÉES le helper `CallerResolver` et tu rebranches EventController,
>   CategoryController, UserController, SessionController.
> - Pour **ProductController**, contente-toi de rendre le helper `CallerResolver`
>   disponible/injectable. NE réécris PAS toute l'auth inline de ProductController
>   si c'est risqué — l'issue #154 est dédiée à faire adopter le CallerResolver par
>   ProductController juste après toi. Si tu touches ProductController, garde le
>   diff minimal et NE touche PAS au bloc `catch(Exception)` de `getProducts`
>   (ligne ~105) qui est réservé à l'issue #92.

## Plan d'implémentation (architect, /sprint plan)
```yaml
issue_0093:
  fichiers_cles: ["EventController.java", "CategoryController.java", "UserController.java", "SessionController.java", "ProductController.java", "infrastructure/security/ (helper CallerResolver a creer)"]
  ...
  zod_dto_sync: "NON"
  etat_reel_du_code: |
    PARTIAL/DUP. 4-5 méthodes privées resolveCaller identiques (EventController, CategoryController,
    UserController, SessionController). Aucune extraction identité via SecurityContextHolder côté
    contrôleur (AuthController l'utilise seulement pour SET le contexte au login).
```
...
- Vague 1 : tu tournes EN PARALLÈLE avec #122 (migration DB users.role — fichiers disjoints, aucun conflit).
- Tu es BLOQUANT pour la vague 2 : #154 (ProductController adopte ton `CallerResolver`) puis #92.
  → Le contrat public du helper `CallerResolver` (signature de la méthode de résolution
    d'identité + exception/statut en cas d'absence d'auth) DOIT être stable et clair, car #154
    va l'appeler. Documente-le en Javadoc courte.

## Designer
Non applicable (backend pur, aucun rendu UI).

## Contraintes
- Branche cible : `sprint/27` (déjà checkout, ne pas changer de branche).
- Répertoire de travail : `/Users/herrh/VSProjects/MyTimeline/.claude/worktrees/magical-hypatia-0e9903`
  (worktree). Vérifie `git branch --show-current` == `sprint/27` AVANT tout commit. Ne JAMAIS
  committer sur `dev` ou le repo principal.
- Commit : 1 commit logique, gitmoji français (ex: `:recycle: #93 ...`).
- Architecture hexagonale STRICTE (cf. context-pack) : le helper `CallerResolver` vit dans
  `infrastructure/security/` (adaptateur), PAS dans domain/.
- Préserver le comportement 401/403 EXISTANT de chaque contrôleur (BR-AUT-005 401 sans fuite,
  ownership 403). Un refactoring ne doit RIEN changer au contrat HTTP observable.
- Tests OBLIGATOIRES inline via `./scripts/test-quiet.sh backend` (ou `backend/./mvnw test` si le
  script est un no-op). Faire tourner les tests d'ownership/auth existants + ajouter la couverture
  du helper si nécessaire.
- Si volume tests > 500 OU temps > 3min : signaler RECOMMAND_TEST_RUNNER.
- Ne PAS toucher aux fichiers : migration Flyway V12 (réservé #122), le bloc `catch(Exception)`
  de `ProductController.getProducts` (~ligne 105, réservé #92).

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: <objectif + BR touchées (BR-AUT-005, BR-EVT-001...) + fichiers clés + pitfalls + tests passés/total>
- contrat CallerResolver: <signature exacte de la méthode publique + statut si non-auth> (CRUCIAL pour #154)
- [MEMORY:*] signaux: <liste si applicables — pitfall/pattern/decision>
- recommandations suite: <RECOMMAND_* ou pitfall subtil, ou "RAS">
