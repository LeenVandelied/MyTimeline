...
## Issue
[SECURITY] Fail-fast : refuser le boot prod si COOKIE_DOMAIN ou CORS_ALLOWED_ORIGINS vides

### Contexte
Lors de la clôture du Sprint 30, la revue de l'issue #130 (journalisation de la config prod au démarrage) a mis en évidence un manque : le système avertit qu'une configuration est dangereuse, mais ne l'empêche pas de démarrer. Ce follow-up vient combler cette lacune.

### Description
Aujourd'hui, `ProdConfigStartupLogger` (issue #130) se contente d'émettre un **WARN** au démarrage si `COOKIE_DOMAIN` (`app.cookie.domain`) ou `CORS_ALLOWED_ORIGINS` (`app.cors.allowed-origins`) sont vides en environnement prod. L'application démarre quand même.

Il faut transformer cet avertissement en **blocage strict** (fail-fast) : si l'environnement de production effectif est détecté et que `COOKIE_DOMAIN` ou `CORS_ALLOWED_ORIGINS` sont vides, l'application doit refuser de démarrer, sur le même principe que `ProfileSafetyGuard` (#111 / #216 / #254).

### Critères d'acceptation
- [ ] Prod effectif + `COOKIE_DOMAIN` vide → démarrage échoue (exception explicite, pas simple WARN)
- [ ] Prod effectif + `CORS_ALLOWED_ORIGINS` vide → démarrage échoue
- [ ] Le message d'erreur indique clairement quelle variable est manquante et pourquoi c'est bloquant
- [ ] Le comportement dev/test reste inchangé (pas de blocage)
- [ ] Un test couvre le cas de blocage effectif au boot

### Piste technique
- `ProdConfigStartupLogger` (#130) — à faire évoluer OU compléter par un composant de garde dédié
- `ProfileSafetyGuard` (#111 / #216 / #254) — pattern de référence fail-fast au démarrage
- Fichiers `application-prod.properties`

## Plan d'implémentation (architect, /sprint plan)
```yaml
issue_0253:
  fichiers_cles: ["ProfileSafetyGuard.java", "ProdConfigStartupLogger.java", "application-prod.properties"]
  couches_touchees: ["infrastructure"]
  strategie_test: "integration (test boot : prod effective + var vide → échec explicite)"
  risque_regression: "détection d'env prod erronée peut bloquer un déploiement légitime ; réutiliser la logique existante de ProfileSafetyGuard."
...
**IMPORTANT — décision d'implémentation prise par le lead :** placer le fail-fast dans
`ProfileSafetyGuard`, PAS dans `ProdConfigStartupLogger`. Raisons :
1. `ProfileSafetyGuard` réagit à `ApplicationEnvironmentPreparedEvent` (AVANT création des beans) →
   fail-fast le plus tôt possible, cohérent avec #111/#216/#254 (tous les garde-fous boot au même endroit).
2. `ProdConfigStartupLogger` réagit à `ApplicationReadyEvent` (contexte déjà démarré) et est
   `@Profile("prod")` STRICT — un blocage tardif y serait moins net.

`ProfileSafetyGuard` (couche `infrastructure/config`) contient déjà, APRÈS le commit #254 (`32c473a`) :
- `checkDevProfileInProduction` (#111)
- `checkRateLimitDisabledInProduction` (#216)
- `checkCookieInsecureInProduction` (#254) — **pattern le plus récent à copier**
- Helper `isProductionEffective(env)` (marqueur prod OU profil `prod`) — RÉUTILISER.

Ajouter un check `checkMissingProdConfigInProduction(env)` (ou 2 checks séparés) appelé dans
`onApplicationEvent`, qui, si `isProductionEffective(env)` :
- lit `app.cookie.domain` (`@Value`/property `${app.cookie.domain:}`) → vide/blank → `IllegalStateException`
  message `#253` mentionnant `COOKIE_DOMAIN` / `app.cookie.domain`.
- lit `app.cors.allowed-origins` (`${app.cors.allowed-origins:}`, format LISTE — vérifier vide OU
  tous éléments blank, comme le fait `ProdConfigStartupLogger` l.65-66) → `IllegalStateException`
  message `#253` mentionnant `CORS_ALLOWED_ORIGINS` / `app.cors.allowed-origins`.
- Attention lecture liste depuis `env.getProperty` : `app.cors.allowed-origins` est un CSV. Lire en
  `String` et vérifier `null || blank` (après trim de chaque token séparé par virgule) suffit ici.

**Cohérence avec `ProdConfigStartupLogger`** : le WARN de `ProdConfigStartupLogger.logEffectiveConfig`
(l.58-71) devient redondant en prod (le boot est déjà bloqué avant). Décider : soit RETIRER les 2
blocs WARN (domain/CORS) de `ProdConfigStartupLogger` puisque le guard bloque désormais, soit les
garder en INFO. Recommandation : retirer les 2 WARN devenus morts (le log INFO de la config effective
l.53-56 RESTE utile) — documenter le retrait en commentaire pointant vers #253/ProfileSafetyGuard.
Vérifier qu'aucun test n'assert ces WARN spécifiques avant de les retirer.
...
- **Vague 2** : dépend de #254 (Vague 1), DÉJÀ committé sur `sprint/35` (HEAD `32c473a`).
- ⚠ CONFLIT FICHIER : #253 modifie le MÊME `ProfileSafetyGuard.java` que #254 (déjà mergé sur HEAD).
  Travailler sur le HEAD courant (qui inclut le check #254) — ajouter les checks #253 SANS toucher
  au check #254 existant. Réutiliser `isProductionEffective`.

## Designer
Non applicable (backend pur, aucune surface UI).

## Contraintes
- Branche cible : `sprint/35` (déjà checkout dans ce worktree — vérifier `git branch --show-current` == `sprint/35`,
  NE PAS `cd` vers un autre repo).
- Architecture : composant en couche `infrastructure/config` — OK.
- Code EN, commentaires/docs FR (convention projet).
- Commit : 1 commit logique gitmoji français (ex: `:lock: #253 fail-fast boot prod si COOKIE_DOMAIN/CORS vides`).
  `git add` CIBLÉ sur tes fichiers (jamais `git add -A` — worktree partagé).
- Tests OBLIGATOIRES via `./scripts/test-quiet.sh unit` (ou `backend/./mvnw` ciblé). RÉUTILISER le
  pattern de `ProfileSafetyGuardTest` (instanciation directe + ConfigurableEnvironment, PAS de @SpringBootTest lourd).
- Tests à ajouter : prod effectif + COOKIE_DOMAIN vide → exception ; prod effectif + CORS vide → exception ;
  cas de contrôle dev/test → PAS d'exception. ⚠ Comme pour #254, en prod effectif les tests existants
  qui ne posent pas domain/CORS pourraient désormais tomber — vérifier et poser `app.cookie.domain` +
  `app.cors.allowed-origins` valides dans les tests prod-effectifs existants (#111/#216/#254) qui ne
  testent pas spécifiquement ce cas, pour les garder focalisés.
- Si tu retires des WARN de `ProdConfigStartupLogger`, vérifie qu'aucun test n'assert leur présence.
- Si volume tests > 500 OU temps > 3min : signaler RECOMMAND_TEST_RUNNER.

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: <objectif + fichiers clés + emplacement du check (guard) + sort des WARN ProdConfigStartupLogger + pitfalls + tests passés/total>
