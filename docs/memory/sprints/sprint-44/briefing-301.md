...
## Issue
[FEATURE] Écran frise/timeline complet (/timeline) — P0, size:L, epic:events, frontend

### Contexte
Le Sprint 40 a ajouté la route `/timeline` sous le shell applicatif, avec un item de navigation fonctionnel dans la sidebar. Mais le contenu affiché sur cette route est un **placeholder** : il sert uniquement à démontrer que l'enveloppement par le shell fonctionne, pas à afficher la frise chronologique elle-même — pourtant la fonctionnalité cœur du produit.

**Point d'attention important** : les notes internes du Sprint 40 (et le commentaire du placeholder lui-même) référencent à tort l'issue **#166** comme étant « l'écran frise complet ». Vérification faite : #166 est un chore ArchUnit backend fermé au Sprint 16, sans rapport. Cette issue-ci (#301) est la bonne référence — purger les mentions erronées de #166.

### À faire
- Construire l'écran frise/timeline complet destiné à remplacer le placeholder actuel sur `/timeline`, à l'intérieur du shell applicatif déjà en place.
- S'appuyer sur les vues Timeline existantes (desktop, mobile portrait, mobile paysage — déjà livrées, cf. issues fermées #55/#63/#64, et le host d'édition S42) : intégrer la frise réelle sous ce point d'entrée shell, SANS dupliquer ni réécrire les composants de frise.
- S'assurer que la navigation, les URLs et les tests E2E du golden-path restent cohérents après le remplacement du placeholder.

### Critères d'acceptation
- [ ] `/timeline` (sous le shell applicatif) affiche la frise chronologique complète et fonctionnelle, plus un placeholder
- [ ] Les vues desktop / mobile portrait / mobile paysage existantes sont intégrées sous ce point d'entrée (pas de réécriture des composants de frise)
- [ ] La navigation sidebar « Timeline » du shell pointe vers l'écran réel, avec état actif correct
- [ ] Aucune régression sur le golden-path E2E existant
- [ ] Les références erronées à #166 sont purgées (commentaire du placeholder notamment)
...
issue_301:
  ...
  strategie_test: "RTL page /timeline (loading/vide/rempli via mock useProductsWithEvents) + AppShell.test.tsx nav active ; E2E golden-path non régressé (gate CI, NE PAS lancer localement)."
  ...
  ordre_ecriture: "1) décider source données (agrégé useProductsWithEvents vs par-produit) 2) câbler host dans page.tsx 3) purger réfs erronées #166 4) état actif nav AppShell 5) tests RTL."
  zod_dto_sync: "NON"
  possibly_done: false
  etat_reel_du_code: |
    timeline/page.tsx = TimelinePlaceholder explicite. Composants frise livrés S42
    (TimelineResponsive/TimelineEditHost) déjà consommés par ProductDetailView et dashboard,
    non montés sous cette route. La décision d'agrégation (frise globale multi-produits vs autre)
    est à trancher et à SIGNALER en [MEMORY:decision] dans ton retour.
```
...
- Aucune dépendance entrante : #301 est la Vague 1 (seule).
- #300 (création d'événement, Vague 2) démarre APRÈS toi et modifiera `AppShell.tsx` (Dialog→drawer) : NE TOUCHE PAS au Dialog « Nouvel événement » d'AppShell ni à `eventService.ts` / `EventEditForm.tsx` — hors périmètre.

## Designer
Non applicable — remontage d'écrans/vues déjà livrés et validés (#55/#63/#64, host S42) sous une route existante ; aucune nouvelle surface visuelle. (ui-design interviendra sur #300/V2.)

## Contraintes
- ⚠ WORKTREE OBLIGATOIRE : travaille EXCLUSIVEMENT dans `/Users/herrh/VSProjects/MyTimeline/.claude/worktrees/sprint-44-start-7b5814` (cd explicite en début de session et avant CHAQUE commande shell). GARDE-FOU avant tout travail : `git branch --show-current` DOIT renvoyer `claude/sprint-44-start-7b5814` et `git rev-parse HEAD` doit être `aa1839d7a6dff70277fc95fffb59ecaaa5b6686e` (ou un descendant) — sinon STOP et signale-le.
- Branche cible : `claude/sprint-44-start-7b5814` (déjà checkout — c'est la branche sprint 44, pas de branche `sprint/44`).
- Commit : 1 commit logique, message gitmoji en français, `git add` CIBLÉ (jamais `git add -A`).
- Tests unitaires frontend OBLIGATOIRES : `cd frontend && npx vitest run` (ou `./scripts/test-quiet.sh frontend` depuis la racine). Le lead a lancé `npm ci` dans `frontend/` — si `node_modules` est absent/incomplet à ton arrivée, relance `npm ci --no-audit --no-fund` (piège connu worktree #272).
- E2E Playwright : NE PAS lancer localement (stack down, gate CI only). Si tu ajoutes des `data-testid`, liste-les dans ton retour (couverture E2E vérifiée en Phase 8).
- Frontend uniquement : AUCUNE modification backend, AUCUNE migration.
- TypeScript strict, next-intl pour toute string UI (locale principale fr), Tailwind v4 tokens DS (jamais de valeurs arbitraires type `w-[Npx]` sans backing token).
- Ne PAS toucher aux fichiers : `frontend/src/services/eventService.ts`, `frontend/src/components/EventEditForm.tsx`, le Dialog « Nouvel événement » d'`AppShell.tsx` (périmètre #300/V2), `frontend/middleware.ts` (périmètre #302, hors sprint).
- Découverte hors scope : micro-fix XS → absorbe et note `ABSORBED: <desc>` ; non-XS → `RECOMMAND_FOLLOWUP: <desc> [triage X | domaine Y]`, ne l'implémente PAS.

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: <objectif + décision agrégation données ([MEMORY:decision]) + fichiers clés + pitfalls rencontrés + tests (n passed)>
- data-testids nouveaux: <liste ou aucun>
