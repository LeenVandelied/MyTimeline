...
## Issue
[CHORE] Frontend : exposer la limite de 4000 occurrences récurrentes

Quand une récurrence sans date de fin plafonne à 4000 occurrences (flag `capped`), l'utilisateur n'en est pas informé. Objectif : afficher un hint contextuel NON bloquant sous le champ `recurrenceEndDate` quand `capped=true`.

## Contrat backend DÉJÀ LIVRÉ (vague 1 #439, commit ba8f585) — à consommer tel quel
- **Route :** `POST /api/events/recurrence-preview` (authentifié, via `apiClient` qui préfixe `/api` → poster sur `/events/recurrence-preview`).
- **Requête JSON :** `{ startDate: "YYYY-MM-DD", recurrenceUnit: "WEEK"|"MONTH"|"YEAR", recurrenceEndDate?: "YYYY-MM-DD"|null }`.
- **Réponse 200 :** `{ count: number, capped: boolean }`.
- **Erreurs :** 400 (champ requis manquant), 422 (`recurrenceEndDate` < `startDate`), 401 (non authentifié).
- ⚠️ **PIÈGE (source #439) :** `recurrenceUnit` est bindé sur l'enum EXACT en MAJUSCULES (`WEEK`/`MONTH`/`YEAR`) — **pas** de parsing tolérant `fromString`. Envoyer `"weeks"` legacy → 400. Le formulaire utilise DÉJÀ les valeurs majuscules (cf. `data-testid="recurrence-unit-option-WEEK"`, pont enum ligne ~150 `EventEditForm.tsx`), donc consommer `form.watch('recurrenceUnit')` tel quel convient. Ne PAS re-mapper en minuscules.

## Plan d'implémentation
```yaml
issue_67:
  fichiers_cles:
    - "frontend/src/types/event.ts — ajouter le schéma Zod du contrat preview (RecurrencePreviewResponse: {count:number, capped:boolean}) — RECOMMAND_FOLLOWUP #439 (typage frontend, aucune sync DTO backend requise)"
    - "frontend/src/services/eventService.ts — ajouter previewRecurrence(payload): Promise<{count,capped}> via apiClient.post('/events/recurrence-preview', ...)"
    - "frontend/src/hooks/ — hook useQuery TanStack (ex. useRecurrencePreview) keyé sur les params DÉBOUNCÉS, enabled UNIQUEMENT si isRecurring=true ET recurrenceUnit défini ET startDate défini"
    - "frontend/src/components/EventEditForm.tsx — brancher le hook sur les watches débouncés DÉJÀ présents (previewStartDate, previewRecurrenceUnit, previewIsRecurring + un watch débouncé de recurrenceEndDate) ; afficher le hint sous le champ recurrenceEndDate (bloc ~ligne 646) quand data?.capped === true"
    - "frontend/public/locales/{fr,en,de,es}/products.json — ajouter la clé products.add.event.form.recurrenceCappedHint (4 locales, sinon next-intl missing-key)"
    - "frontend/src/components/EventEditForm.test.tsx — mock previewRecurrence: capped=true → hint visible ; capped=false/absent → hint absent ; soumission non bloquée"
  couches_touchees: ["frontend"]
  strategie_test: "unit (EventEditForm.test.tsx, mock du hook/service) + E2E Playwright (nouveau data-testid → sera vérifié par le coverage-E2E check Phase 8)"
  risque_regression: "additif non bloquant. NE PAS toucher au backend (déjà livré #439), NE PAS élargir EventResponse. Ne pas casser les tests existants EventEditForm.test.tsx (seriesErr, colorErr)."
  ordre_ecriture: "type Zod → service → hook → form (hint + testid) → i18n 4 locales → tests"
  zod_dto_sync: "OUI côté frontend uniquement (typer la réponse preview) ; aucune modif backend."
```
...
## Context-pack complet committé (chemin stable, PAS un /tmp)
Le pack domaine `br-events` (exact) est inliné ci-dessus. Les packs cp-frontend / cp-hexagonal
et l'archive `pit-frontend` (pitfalls frontend) NE sont pas recopiés ici pour économiser le contexte.
**Lis EN PRIORITÉ, avant tout code**, le briefing complet committé `docs/memory/sprints/sprint-69/briefing-67.md`
— il inline cp-hexagonal, cp-frontend, br-events, pit-frontend + rules-jit/frontend.md + ux-patterns.md.
Chemin versionné, lecture fiable (contrairement au piège /tmp).


## Dépendances intra-sprint
- **Dépend de #439 (vague 1, DÉJÀ LIVRÉE, commit ba8f585)** : l'endpoint `POST /api/events/recurrence-preview` existe sur la branche. Consomme-le, ne le recrée pas, ne touche PAS au backend.

## Designer
Wording du hint imposé par l'issue (pas de composant dédié — `<p role="status">` + classe utilitaire Tailwind). Textes à poser dans `products.add.event.form.recurrenceCappedHint` :
- **fr** : « La série dépasse 4 000 occurrences. Définissez une date de fin pour obtenir le compte exact. »
- **en** : "The series exceeds 4,000 occurrences. Set an end date to get the exact count."
- **de** : „Die Serie überschreitet 4.000 Vorkommen. Legen Sie ein Enddatum fest, um die genaue Anzahl zu erhalten.“
- **es** : «La serie supera las 4.000 repeticiones. Define una fecha de fin para obtener el recuento exacto.»
Accessibilité : `<p role="status" aria-live="polite">` (annonce non intrusive) + `data-testid="event-form-recurrence-capped-hint"` (obligatoire pour la couverture E2E Phase 8). Ton informatif, pas d'erreur (ne pas styliser en rouge/danger — c'est un avertissement neutre, cf. `recurrenceEndHint` voisin).

## Contraintes
- Branche cible : `claude/sprint-69-d576fe` (déjà checkout — NE PAS créer/switcher de branche).
- TS strict, Zod, next-intl. Le hint ne DOIT PAS bloquer la soumission (pas de champ requis, pas d'erreur RHF).
- Le hint ne s'affiche QUE si `capped === true`. `capped === false` OU réponse absente/en cours OU `isRecurring=false` → aucun hint. Il disparaît naturellement dès que `recurrenceEndDate` ramène `count` sous 4000 (la query réactive débouncée renvoie `capped:false`).
- Débounce : réutilise le `useDebounced` déjà en place dans `EventEditForm.tsx` (ne pas spammer l'endpoint à chaque frappe).
- Commit : 1 commit logique, gitmoji français (ex. `:sparkles: feat(events): hint frontend plafond 4000 occurrences récurrentes (#67)`). `git add` CIBLÉ (JAMAIS `git add -A` : worktree partagé).
- Tests inline OBLIGATOIRES via `./scripts/test-quiet.sh frontend`. Fais-les tourner AVANT de rendre.
- Ajoute une spec Playwright E2E si le harnais E2E le permet ; sinon signale-le explicitement pour `/create-e2e` post-merge (le check coverage-E2E Phase 8 relèvera le nouveau `data-testid`).
- NE PAS toucher : tout fichier `backend/`, `EventResponse.java`, l'endpoint #439.

## Livrable attendu (format strict, MAX 500 tokens caveman)
Écris `docs/memory/sprints/sprint-69/issue-67-done.md` avec :
- commits: [SHA, ...]
- resume: objectif + fichiers clés modifiés + hint (testid, wording, 4 locales OK) + comment le hint est piloté (hook réactif débouncé) + tests ajoutés (compte + noms) + résultat test-quiet
- couverture E2E: spec créée OU signalement /create-e2e (avec le testid)
