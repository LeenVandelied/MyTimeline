...
- **Constat (MAJEUR)** : métrique `lg` répétée sur les 2 CTA en valeurs arbitraires (`px-[22px] text-[14px] min-h-[46px]` + `py-3 leading-snug h-auto text-center whitespace-normal`).
- **Correctif** : UNE source, `@utility cta-lg` dans `frontend/src/styles/globals.css` (après `container-landing`) : `height:auto; min-height:46px; padding-block:var(--space-3); padding-inline:22px; font-size:14px; line-height:var(--leading-snug); text-align:center; white-space:normal`. Tokens là où ils existent (`--space-3`, `--leading-snug`) ; 22 / 14 / 46 px sans token DS, commentés avec renvoi à `.mt-btn--lg` (core.css:25) et `.mt-btn--wrap.mt-btn--lg` (i18n.css:82). Aucune couleur déclarée. Les 2 CTA portent `cta-lg min-w-min transition-all` + leurs couleurs.
- **Piège vérifié par compilation** (`@tailwindcss/postcss` sur `globals.css`, script jetable dans le scratchpad) : l'utilitaire reste dans `@layer utilities` mais y est émise AVANT `.px-4` et `.text-sm` (positions 15 576 < 35 312 < 38 001) — le variant `Button` (`h-9 px-4 py-2 text-sm whitespace-nowrap`) aurait repris padding, corps et hauteur, et `tailwind-merge` ne retire pas des classes face à `cta-lg` qu'il ne connaît pas. D'où le corps en `&:is(a, button)` : spécificité (0,1,1) > (0,1,0), sans dépendre de l'ordre d'émission, et toujours sous une variante `hover:` (0,2,0).
- **Rendu attendu** : valeurs calculées identiques aux utilitaires précédentes (mêmes 8 propriétés et valeurs) → 229 × 46 en `fr` à 1024 attendu. **NON mesuré par l'agent** : le serveur :3000 est un `next start` de production sans rechargement, E2E interdit par la consigne — le lead rebuild et rejoue `sprint-104-hero-cta-single-line` + specs landing.
- **Tests** : `HeroSection.test.tsx` +2 tests structurels (les 2 CTA portent `cta-lg` sans classe à valeur arbitraire ; `@utility cta-lg` porte `&:is(a, button)`, la métrique et aucune couleur). `globals.css` lu via `__dirname` (sous jsdom, `import.meta.url` n'est pas en `file:` — même choix que `HeroTimelineAnimation.test.tsx`). `rtk proxy npx vitest run src/components/landing` → 8 fichiers, **61 passés**. `tsc --noEmit` exit 0 ; `next lint --file` 0 avertissement ; `prettier --check` (HeroSection.tsx, HeroSection.test.tsx, globals.css) exit 0.
- **Contrôles négatifs** : (1) CTA secondaire rétabli en `min-h-[46px] px-[22px]` sans `cta-lg` → 1 rouge (test structurel CTA) ; (2) `&:is(a, button)` remplacé par `&` dans `globals.css` → 1 rouge (test de l'utilitaire). Sources restaurées, suite à nouveau 61/61.
