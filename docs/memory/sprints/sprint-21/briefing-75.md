...
## Issue
[FEATURE] Backend : upload avatar (POST /me/avatar)

### Contexte
L'utilisateur ne peut pas personnaliser son avatar. Le modèle `User`/`UserEntity` a déjà un champ `avatar` (V7 #44) mais aucun endpoint ne permet d'uploader une image. Sans ça, la page Réglages (#86) ne peut pas afficher d'avatar personnalisé.

### À faire
- `POST /api/me/avatar` en multipart/form-data (1 fichier image)
- Valider : type MIME (image/jpeg, png, webp) + taille max 5 Mo
- Stocker le fichier (voir DÉCISION STOCKAGE ci-dessous)
- Persister le chemin/référence dans `User.avatar`
- Exposer l'avatar dans le DTO user (aligne dette #151 : `UserResponse`/`UserSchema` frontend n'exposent pas encore `avatar`)
- `DELETE /api/me/avatar` : réinitialise `avatar` à null + supprime le fichier stocké
- Servir l'avatar via un endpoint AUTHENTIFIÉ (pas de fichier statique public)

### BR impactées
- BR-AUT-001 — L'avatar appartient à l'utilisateur identifié ; seul lui peut le modifier/supprimer (ownership).

### Critères d'acceptation
- [ ] `POST /api/me/avatar` accepte JPEG/PNG/WebP, max 5 Mo
- [ ] Fichier non-image ou trop lourd → `400` avec message explicite
- [ ] `User.avatar` mis à jour en base après upload réussi
- [ ] `DELETE /api/me/avatar` réinitialise `avatar` à null
- [ ] L'avatar n'est accessible que par requête authentifiée (pas de lien public permanent)
- [ ] Ownership BR-AUT-001 : un utilisateur ne peut modifier/supprimer QUE son propre avatar

## Plan d'implémentation (architect, /sprint plan)
```yaml
issue_0075:
  fichiers_cles:
    - backend/.../domain/models/User.java (champ avatar déjà présent V7 #44)
    - backend/.../application/ (port StoragePort + service avatar)
    - backend/.../infrastructure/adapters/controllers/ (UserController ou nouveau AvatarController : POST/DELETE/GET /api/me/avatar)
    - backend/.../infrastructure/ (LocalStorageAdapter — impl du StoragePort)
    - backend/src/main/resources/db/migration/V12__user_avatar.sql (si colonne pas déjà en base — VÉRIFIER V7 #44)
  couches_touchees: [backend/domain, backend/application, backend/infrastructure, db]
  strategie_test: JUnit service + MockMvc POST multipart + validation type/taille (OWASP upload) + ownership
  risque_regression: MOYEN — upload multipart = surface sécurité (MIME magic bytes, taille, path traversal)
  ordre_ecriture: [vérifier colonne avatar/V7, StoragePort + LocalStorageAdapter, service avatar + validation, endpoints REST POST/DELETE/GET, exposer avatarUrl dans DTO, config multipart, tests]
  zod_dto_sync: exposer avatar dans UserResponse → aligne UserSchema frontend (#151, consommé par #86)
```
...
## DÉCISION STOCKAGE (ADR — security-expert, /sprint start Sprint 21)
**Ne PAS monter MinIO/S3 ce sprint.** Constat sécurité : aucun docker-compose, aucune dépendance Maven MinIO/S3, aucune variable `STORAGE_*` dans le repo → l'infra objet n'existe pas, la monter mid-sprint = scope creep.

**Approche imposée :**
- Stockage **local privé** : répertoire hors webroot, chemin configurable `${STORAGE_AVATAR_PATH}` (suivre la convention #34 : PAS de default en prod = fail-fast, default dev OK — cf. `application-prod.properties` `${JWT_SECRET}`/`${DB_PASSWORD}` sans default, `application-dev.properties` avec fallback).
- Servir l'avatar via un endpoint **authentifié** `GET /api/me/avatar` qui streame les octets (`no static resource mapping`, pas d'URL publique permanente).
- **`StoragePort` (interface hexagonale) défini maintenant** + `LocalStorageAdapter` comme impl. Le swap futur vers S3/MinIO = nouvelle impl derrière le port, pas de réécriture controller/service. NE PAS câbler un SDK vendor dans le controller/service.
- ⚠ Ceci dévie du critère d'acceptation initial « stocké dans MinIO/S3 et non dans le FS serveur » et « URL signée ». Assumé et documenté ici (ADR). Dans ton done.md, signale explicitement cette déviation (`[MEMORY:decision]`) pour la PR.

## CHECKLIST OWASP UPLOAD (obligatoire — security-expert)
- **MIME par MAGIC BYTES** (byte-sniff manuel ou Apache Tika) — ne JAMAIS faire confiance au header `Content-Type` ni à l'extension du filename. Autoriser uniquement jpeg/png/webp.
- **Limite 5 Mo serveur** : `spring.servlet.multipart.max-file-size=5MB` + `spring.servlet.multipart.max-request-size` dans application.properties (ACTUELLEMENT ABSENT — à ajouter) ET contrôle applicatif (defense in depth).
- **Nom de fichier stocké = UUID généré**, jamais le filename client → anti path-traversal, anti-collision, anti fuite de métadonnée.
- **Ownership BR-AUT-001** : POST/DELETE/GET avatar n'agissent QUE sur le `UserEntity` de l'appelant. Résoudre l'id user depuis le cookie `jwt` (pattern ACK-002 déjà utilisé dans `UserController`), JAMAIS depuis un param id client.
- **Pas de log du contenu binaire / octets multipart bruts.** Le handler d'exception NE DOIT PAS fuiter stack trace / chemin dans le body de réponse (anti-pattern A4 de br-auth : `catch(Exception)` renvoyant l'objet exception → à ne pas reproduire).
- **Cleanup** : au remplacement d'avatar ou au DELETE, supprimer l'ancien fichier stocké → pas d'orphelins sur disque.
- **Un seul fichier** : rejeter les requêtes multipart avec des parts inattendues.

## Dépendances intra-sprint
- Consommé par #86 (frontend desktop, vague 1) et #87 (mobile, vague 2). Le contrat de réponse doit être clair : `POST /api/me/avatar` → JSON avec la référence/URL d'accès de l'avatar (ex. `{ "avatarUrl": "/api/me/avatar" }` ou champ dans le user). Documenter le contrat exact dans ton done.md pour que le front s'aligne.
- Exposer `avatar` dans le DTO user (`UserResponse`) → débloque #151 et l'affichage front.

## Contraintes
- Branche cible : `sprint/21` (déjà checkout). cwd = racine repo ; vérifie `git branch --show-current` = sprint/21 AVANT de commit.
- Scope STRICT backend. NE PAS toucher `frontend/**` (le front est traité par #86/#87 en parallèle).
- Migration : si la colonne `avatar` existe déjà (V7 #44), NE PAS recréer de colonne — V12 ne sert que si un ajout schéma est réellement nécessaire (ex. type/longueur). Vérifier l'état réel de la colonne avant d'écrire V12. Numérotation : dernière migration réelle = V11, donc V12 libre.
- Architecture hexagonale STRICTE : port dans `application`/`domain`, adapter dans `infrastructure`. Le controller ne dépend PAS d'une impl concrète de stockage.
- Commit : 1 commit logique gitmoji français (ex: `:sparkles: #75 Upload avatar POST/DELETE /me/avatar (stockage local + StoragePort)`).
- Tests OBLIGATOIRES via `./scripts/test-quiet.sh <scope>` : MockMvc POST multipart (succès + rejet MIME + rejet taille), DELETE, ownership (un user ne touche pas l'avatar d'un autre), validation magic bytes.
- Si volume tests > 500 OU temps > 3min : signaler RECOMMAND_TEST_RUNNER.

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: objectif + BR touchées (BR-AUT-001) + fichiers clés (StoragePort, adapter, controller, migration si créée) + CONTRAT réponse exact pour le front + tests
- [MEMORY:decision] ADR stockage local vs objet (déviation AC assumée) ; [MEMORY:*] autres signaux
- recommandations suite: RECOMMAND_DB_EXPERT si V12 créée, RECOMMAND_FOLLOWUP (migration MinIO/S3 différée, resize image), pitfalls
