...
## Issue
[FEATURE] Frontend : Dashboard desktop

### Contexte
Le fichier dashboard `page.tsx` est un monolithe de ~283 lignes qui mélange données, logique et rendu. Il n'existe pas de vue dashboard structurée pour desktop. Objectif : construire un dashboard desktop complet sur le Design System Graphite de MyTimeline, lisible en un coup d'œil.

### À faire
- Extraire le monolithe en composants atomiques réutilisables
- **Ruban de densité 30 jours** : mini-calendrier de chaleur montrant l'activité événementielle
- **Agenda semaine** : liste des événements de la semaine courante avec produit associé
- **KPIs en marginalia** : nombre de produits actifs, événements ce mois, série courante
- **Liste produits compacte** : vignettes produits avec indicateur d'activité récente
- **Salutation contextuelle** (matin/après-midi/soir + prénom utilisateur)
- Appliquer les tokens Graphite (couleurs, typo, espacement) via variables Tailwind 4

### Critères d'acceptation
- [ ] La page dashboard délègue à des composants isolés (≤ 80 lignes chacun)
- [ ] Ruban de densité : 30 derniers jours avec gradient d'intensité
- [ ] Agenda semaine : événements de la semaine courante, triés chronologiquement
- [ ] KPIs visibles sans scroll sur écran 1280px
- [ ] Salutation varie selon l'heure locale du navigateur
- [ ] Aucun appel API direct dans les composants — tout via hooks TanStack Query
- [ ] Layout validé visuellement Chrome 1280px ET 1440px

### BR impactées
Aucune (agrège endpoints events/products existants — pas de nouveau endpoint backend).
...
issue_0080:
  fichiers_cles:
    ...
    - frontend/src/components/dashboard/WeekAgenda.tsx
    - frontend/src/components/dashboard/KpiMarginalia.tsx
    - frontend/src/components/dashboard/ProductList.tsx
    - frontend/src/components/dashboard/GreetingHeader.tsx
    - frontend/src/hooks/useDashboardData.ts (TanStack Query, agrège produits + événements)
    - "frontend/app/[locale]/dashboard/page.tsx" (refactorisation — cible réelle du monolithe)
    - frontend/src/components/layout/ (RISQUE fichier partagé — header/sidebar, NE PAS casser header auth)
  ...
  risque_regression: MOYEN — touche components/layout/ (partagé) ; vérifier non-collision header auth existant (LanguageSelector, LogOut, useAuth)
  ordre_ecriture: [structure dossier + tokens DS, cards KPI, layout desktop, câblage page, tests]
  zod_dto_sync: aucun
  possibly_done: false
  etat_reel_du_code: |
    VÉRIFIÉ PAR LE LEAD (2026-07-05) :
    - frontend/src/components/dashboard/ = INEXISTANT (à créer)
    - Monolithe réel = frontend/app/[locale]/dashboard/page.tsx (283 lignes), PAS src/app/dashboard (chemin issue erroné)
    - Router = next-intl, segment [locale] ; page.tsx utilise 'use client', useTranslations, useAuth, getProducts (service direct), TimelineResponsive
    - Prérequis TOUS présents sur dev : tokens Graphite (frontend/src/styles/ds/tokens/) #45 ✓ | @tanstack/react-query ^5.101.2 #48 ✓ | components/timeline/ (dont mobile S19) #47 ✓
```
...
- #80 est la VAGUE 1 (fondation) : tu poses la structure `frontend/src/components/dashboard/` que #83 (mobile portrait) et #85 (mobile paysage) réutiliseront.
- **Découpage composants réutilisables impératif** : #83/#85 vont réutiliser DensityRibbon, WeekAgenda, KpiMarginalia, ProductList, GreetingHeader. Ne les couple PAS à une largeur desktop fixe — props/variants pour permettre la déclinaison responsive ultérieure.
- Aucune migration. Aucun endpoint backend nouveau.

## Designer — SPEC AUTORITAIRE (passe ui-design, REJET → corrections OBLIGATOIRES)

> **CHARTE DE VÉRITÉ = `docs/design/graphite-handoff.md`** (§3 Dashboard, §8 Shell). PAS la charte générique/ux-patterns (absente du repo). LIS le handoff avant de coder. Cette section SURCHARGE toute mention contradictoire du body issue (ex: « gradient d'intensité »).

**Hiérarchie visuelle (grille 1280px, contenu ; PAS de shell/nav 248px à créer — voir note scope) :**
1. GreetingHeader — bandeau fin en tête, texte éditorial **sentence case**, pas de gros titre display, pas d'emoji.
2. DensityRibbon — hero pleine largeur sous header (priorité n°1).
3. WeekAgenda — bloc principal, table dense (date stamp mono, filet couleur).
4. KpiMarginalia — colonne marginalia droite (ou bande fine), chiffres inline mono.
5. ProductList — liste compacte (pastille + nom + prochain événement + compteur).

**Corrections OBLIGATOIRES à appliquer :**
1. **DensityRibbon — PAS de « gradient d'intensité »** (absent charte/tokens). Densité = **hauteur de barre proportionnelle au nombre d'événements/jour** (pattern `Minimap.tsx` existant). Couleur barre = **catégorie événement `--evt-*`** (12 couleurs curatées), ligne TODAY = `--color-accent`. Graduations ruler = `--color-ink-faint` + `IBM Plex Mono`.
2. **Helper bucket unique partagé** : le helper `buildMinimapBuckets` cité en JSDoc `Minimap.tsx:13` **n'existe pas** dans `lib.ts` (vérifié lead — exports actuels : `getDaysRange`, `buildEventsByResource`). Créer UN SEUL helper de bucketing dans `frontend/src/components/timeline/lib.ts`, consommé par `Minimap.tsx` ET `DensityRibbon.tsx`. Ne PAS dupliquer la logique.
3. **KpiMarginalia** : bannir tout « gros chiffre display ». Chiffres exclusivement `IBM Plex Mono` inline dans une phrase (`--text-xs`/`--text-sm`). Jamais `--text-xl/2xl/3xl`.
4. **ProductList / KpiMarginalia** : ne PAS utiliser `<Card>` shadcn tel quel (`card.tsx` = tokens non-Graphite `bg-card/shadow-sm/border`). Construire ces blocs en **filets** (`border-b border-rule`, `bg-surface`, `shadow-none`) — pas de wrapper Card à ombre. (Si tu patches `card.tsx` pour le rendre Graphite-conforme, reste minimal et ne casse pas ses autres usages.)
5. **GreetingHeader** : sentence case strict, pas de majuscules/emoji, supprimer l'icône `Zap` et l'animation **spring-rebond** héritées (`framer-motion type:spring stiffness:260 damping:20` de l'ancien page.tsx = anti-pattern). Motion autorisée : `--ease-quart`, durées 120-280ms, sans rebond.
6. **Contrat de props responsive explicite par composant** (garantit réutilisation #83/#85 sans refonte) : ex. `WeekAgenda variant="table"|"stack"`, `DensityRibbon rangeDays` paramétrable. Composants = **largeur fluide 100%/flex**, JAMAIS `max-width`/`px-*` desktop en dur dans le composant — les contraintes de largeur restent dans le parent (page dashboard).

**Réutilisation composants existants (éviter duplication) :**
- DensityRibbon réutilise la logique de bucketing de `Minimap.tsx` (déjà a11y `role="slider"`, drag pointeur/clavier) — seul changement : barres colorées par catégorie.
- WeekAgenda réutilise `DateStamp.tsx`, `EventPill.tsx`/`EventBar.tsx` (barre déjà normée : `textOn`, `↻` récurrence, radius 6px, a11y validée `EventPill.test.tsx`) au lieu de recoder l'affichage événement.

**Dark mode** : couleurs `--evt-*` identiques clair/sombre (handoff) ; seuls accent/surfaces changent. Contraste texte-sur-barre via `textOn` existant.

**Convention data-testid** (anticiper E2E #83/#85) : `{zone}-{component}-{role}-{id}`, ex. `dashboard-density-ribbon`, `dashboard-week-agenda-row-{id}`.

**NOTE SCOPE (décision lead) :** Le handoff §3/§8 décrit un **shell applicatif avec nav latérale persistante 248px**. Ce shell N'EXISTE PAS encore dans le code et n'est PAS dans le scope de #80 (issue = 5 composants dashboard + refacto + hook). **NE CONSTRUIS PAS de shell/sidebar 248px.** Livre les composants dashboard dans la structure de page actuelle (adaptée au DS). Signale en fin de retour `RECOMMAND_FOLLOWUP: créer le shell applicatif (nav latérale 248px, handoff §8) [triage M | domaine events/frontend]` — le lead l'arbitrera en Phase 4 de /sprint end.

## Contraintes
- Branche cible : `sprint/20` (déjà checkout — vérifie `git branch --show-current` = sprint/20 AVANT tout commit ; HEAD attendu b3f823c ou descendant).
- Commit : 1 commit logique, gitmoji français (ex: `:sparkles: #80 Dashboard desktop — extraction composants + ruban densité`).
- Tests inline OBLIGATOIRE via `./scripts/test-quiet.sh <scope>` (Vitest pour les composants + un smoke Playwright 1280px si faisable).
- Si volume tests > 500 OU temps > 3min : signale RECOMMAND_TEST_RUNNER (le lead spawnera test-runner en Phase 6).
- **Ne PAS casser** le header auth existant (LanguageSelector, LogOut, useAuth) si tu touches `components/layout/`.
- **Aucun appel API direct** dans les composants — passe par `useDashboardData.ts` (TanStack Query). Migre l'appel `getProducts` actuel vers le hook.
- Respecte le DS Graphite : tokens CSS de `frontend/src/styles/ds/tokens/`, pas de couleurs en dur.
- Composants ≤ 80 lignes chacun (critère d'acceptation).

## Livrable attendu (format strict, MAX 500 tokens caveman)
Écris ton retour dans ce format (le lead le transforme en `issue-80-done.md`) :
RETOUR :
- commits: [SHA1, ...]
- resume: <objectif + fichiers clés créés/modifiés + approche hook TanStack + pitfalls rencontrés + tests (passed/failed)>
- [MEMORY:*] signaux: <pattern/pitfall/decision si applicable, sinon "aucun">
- recommandations suite: <RECOMMAND_TEST_RUNNER / RECOMMAND_UI_DESIGN / RECOMMAND_FOLLOWUP:<desc>[triage|domaine] / pitfall subtil — ou "aucune">
- STATUS: COMPLETED en toute dernière ligne (ou STATUS: PARTIAL + BLOQUE_SUR:<raison> si bloqué > 30 min)
