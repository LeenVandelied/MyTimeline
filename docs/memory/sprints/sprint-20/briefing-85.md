...
## Issue
[FEATURE] Frontend : Dashboard mobile paysage (rail 64px + 2 colonnes)

### Contexte
En orientation paysage sur mobile (hauteur < 500px, largeur > 667px), la mise en page portrait #83 n'est pas adaptée. La hauteur limitée impose un rail de navigation vertical compact plutôt qu'une nav en bas/hamburger. **Tu réutilises un maximum les composants #80 (desktop) et #83 (mobile)**, tu ajoutes surtout le rail + la grille 2 colonnes.

### À faire
- **Rail de navigation vertical persistant 64px** (icônes sans label) au bord gauche en mode paysage. Contenu minimum : **accueil, produits, déconnexion**.
- Contenu en **2 colonnes** à droite du rail : agenda compact (colonne gauche) + ruban densité / produits (colonne droite) — CSS Grid.
- Masquer le hamburger du mode portrait (remplacé par le rail en paysage).
- Détection orientation : `@media (orientation: landscape) and (max-height: 500px)` OU variantes Tailwind `landscape:` + `useMediaQuery` (hook livré par #83).

### Critères d'acceptation
- [ ] En paysage mobile, le rail vertical 64px est visible et les icônes cliquables
- [ ] Contenu principal en 2 colonnes à droite du rail, sans débordement
- [ ] Transition portrait ↔ paysage sans saut visuel brutal
- [ ] Le rail contient au minimum : accueil, produits, déconnexion
...
issue_0085:
  fichiers_cles:
    - frontend/src/components/dashboard/CompactRail.tsx (NOUVEAU — rail 64px, icônes lucide, a11y aria-label obligatoire)
    - frontend/src/components/dashboard/ (composants #80/#83 RÉUTILISÉS)
    - "frontend/app/[locale]/dashboard/page.tsx" (3e mode d'affichage : desktop / portrait / paysage — grille 2 col)
    - index.ts (barrel)
  couches_touchees: [frontend/components]
  strategie_test: Vitest grille 2 col + rail a11y + Playwright viewport 812x375 paysage
  risque_regression: MOYEN — dérive #83 ; le switch d'affichage devient ternaire, ne pas casser desktop #80 ni portrait #83
  ordre_ecriture: [CompactRail a11y, détection orientation, grille 2 col, portage composants, tests]
  zod_dto_sync: aucun
  possibly_done: false
  etat_reel_du_code: |
    VÉRIFIÉ PAR LE LEAD (#80 baafb27, #83 943b0ce livrés) :
    - Composants dashboard dispo : GreetingHeader, DensityRibbon (props `rangeDays`+`scrollable`/`minBarWidth`), WeekAgenda (`variant`), KpiMarginalia, ProductList (desktop #80) + CompactAgenda, ProductCarousel, MobileDrawer (mobile #83). Barrel index.ts.
    - Hook `frontend/src/hooks/useMediaQuery.ts` (SSR-safe, livré #83) → réutilise-le pour la media query orientation.
    - Hook data UNIQUE : useDashboardData.ts — aucun appel API direct.
    - Utility CSS `@utility scrollbar-none` (globals.css, #83) pour conteneurs scroll-x.
    - a11y focus trap : `useFocusTrap` (S19). Icônes = lucide-react.
    - page.tsx switch actuel : desktop vs mobile portrait via useMediaQuery('(max-width:767px)'). À étendre en ternaire (ajouter la branche paysage).
    - MobileDrawer/CompactAgenda/ProductCarousel non couplés portrait (confirmé #83) → réutilisables tels quels.
```
...
- #85 est la VAGUE 3 (dernière) : elle dépend de #80 (baafb27) et #83 (943b0ce), tous deux sur HEAD. Tous les composants desktop + mobile sont DÉJÀ sur la branche.
- Aucune migration, aucun endpoint backend. C'est la dernière issue du sprint.

## Designer — guidance paysage (charte = `docs/design/graphite-handoff.md`, cohérence #80/#83)
- **Rail 64px** : icônes lucide SANS label visible → **a11y OBLIGATOIRE** : chaque item `aria-label` + `title` (tooltip), `role`/`tabIndex` si custom, `focus:ring-2`. Actif = `--color-accent`, inactif = `--color-ink-muted`. Fond `--color-surface`, filet `--color-rule` séparant rail/contenu.
- **Grille 2 colonnes** : CSS Grid à droite du rail. Colonne gauche = agenda compact (`CompactAgenda` #83), colonne droite = DensityRibbon (mode scrollable) + ProductCarousel. Pas de largeurs px en dur — `grid-template-columns` fluide (fr units).
- **Transition portrait↔paysage sans saut** : éviter un remount coûteux ; garder la même source data (`useDashboardData`), ne bouger que le layout. Motion `--ease-quart` 120-280ms sans rebond.
- **Tokens Graphite uniquement** (`--evt-*`, `--color-accent`, `--color-rule`, `IBM Plex Mono`, `Archivo`). Pas de `<Card>` shadcn à ombre — filets. Theme-aware clair + sombre.
- **Réutilisation maximale** : NE recrée PAS agenda/carousel/drawer — importe `CompactAgenda`, `ProductCarousel` du barrel. La déconnexion du rail = même logique que MobileDrawer (`logout` → redirection localisée `/${locale}/login`).
- **Convention data-testid** : `dashboard-{zone}-{role}-{id}` (cohérent #80/#83, ex. `dashboard-rail`, `dashboard-rail-item-products`).

## Contraintes
- Branche cible : `sprint/20` (déjà checkout — vérifie `git branch --show-current` = sprint/20 AVANT commit ; HEAD attendu 943b0ce ou descendant).
- Commit : 1 commit logique, gitmoji français (ex: `:iphone: #85 Dashboard mobile paysage — rail 64px + grille 2 colonnes`).
- Tests inline OBLIGATOIRE via `./scripts/test-quiet.sh <scope>` (Vitest rail a11y + grille 2 col + non-régression desktop/portrait).
- Si volume tests > 500 OU temps > 3min : signale RECOMMAND_TEST_RUNNER.
- **Ne PAS casser** le rendu desktop #80 NI le mode portrait #83. Le switch d'affichage devient ternaire — teste les 3 modes.
- **Aucun appel API direct** : passe par `useDashboardData.ts`.
- i18n 4 locales (fr/en/es/de) — aucun texte FR en dur, réutilise/étends les namespaces `dashboard.*`.
- Redirection déconnexion localisée `/${locale}/login`.

## Livrable attendu (format strict, MAX 500 tokens caveman)
RETOUR :
- commits: [SHA1, ...]
- resume: <objectif + fichiers créés/modifiés + réutilisation #80/#83 + approche rail/grille/orientation + pitfalls + tests (passed/failed)>
- [MEMORY:*] signaux: <pattern/pitfall/decision si applicable, sinon "aucun">
- recommandations suite: <RECOMMAND_TEST_RUNNER / RECOMMAND_FOLLOWUP:<desc>[triage|domaine] / pitfall subtil — ou "aucune">
