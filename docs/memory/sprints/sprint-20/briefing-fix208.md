[BRIEFING FIX — Review PR #208 (Sprint 20 Dashboard responsive)]

## Objectif
Corriger les 5 findings de review (1 MAJEUR + 4 MINEUR). **Correction MINIMALE, pas de refacto élargie.** Après fix : lancer les tests, commit + push sur `sprint/20`.

## Findings à corriger

### [MAJEUR] 1 — `nextEvent` dupliqué verbatim
- `frontend/src/components/dashboard/ProductCarousel.tsx` (~l.29) ET `frontend/src/components/dashboard/ProductList.tsx` (~l.19) contiennent la MÊME fonction `nextEvent` (+ éventuelle logique de couleur de fallback dupliquée).
- **Fix** : extraire vers un helper partagé. Crée `frontend/src/components/dashboard/lib.ts` (helpers spécifiques dashboard) OU ajoute à `frontend/src/components/timeline/lib.ts` si plus cohérent. Les DEUX composants importent le helper. Zéro changement de comportement.

### [MINEUR] 2 — `handleProducts` DOM impératif
- `frontend/app/[locale]/dashboard/page.tsx` (~l.81-85) : `handleProducts` fait `document.querySelector('[data-testid=...]')` pour scroller vers la colonne produits paysage.
- **Fix** : `useRef` sur le conteneur colonne produits (branche paysage) + `ref.current?.scrollIntoView(...)`. Garde le `data-testid` sur l'élément (E2E), mais ne le sélectionne plus via querySelector.

### [MINEUR] 3 — MobileDrawer Escape listener séparé
- `frontend/src/components/dashboard/MobileDrawer.tsx` (~l.41-51) : `document.addEventListener('keydown', ...)` pour Escape, indépendant de `useFocusTrap` (qui ne gère que Tab).
- **Fix** : étendre `frontend/src/components/timeline/useFocusTrap.ts` avec un paramètre OPTIONNEL `onEscape?: () => void` (non-cassant pour les consommateurs S19 existants : TimelineBottomSheet, etc.), et faire consommer `onEscape` par MobileDrawer → retirer le listener keydown séparé. **Vérifie que les autres consommateurs de useFocusTrap ne cassent pas** (param optionnel, défaut no-op).

### [MINEUR] 4 — Nommage utility focus ring incohérent
- `MobileDrawer.tsx` / `CompactRail.tsx` / hamburger utilisent `focus-visible:ring-focus` alors que `frontend/src/components/ui/button.tsx` utilise `focus-visible:ring-ring` (même couleur résolue via `--color-ring: var(--color-focus)`, donc pas de bug visuel, juste incohérence de nom).
- **Fix** : standardiser les composants dashboard sur `ring-ring` (aligné sur button.tsx / le reste du DS). Purement cosmétique/nommage.

### [MINEUR] 5 — `CompactRail` champ `labelKey` mort
- `frontend/src/components/dashboard/CompactRail.tsx` (~l.26-32) : l'interface exportée `CompactRailItem` déclare `labelKey` requis mais il n'est jamais consommé comme prop (le composant construit les items en interne, `RailButton` reçoit `label` résolu).
- **Fix** : retirer `labelKey` de l'interface exportée `CompactRailItem` (le plus simple) — OU le threader réellement si utile. Choisis le retrait sauf raison contraire.

## Triage
Taille: S (corrections ciblées)
Modèle: opus
Effort: medium
...
## Contraintes
- Branche cible : `sprint/20` (déjà checkout — vérifie `git branch --show-current` = sprint/20 AVANT commit ; HEAD attendu f2f13b6 ou descendant).
- **Corrections minimales uniquement** — ne touche PAS aux fonctionnalités livrées, ne refactore pas au-delà des 5 findings.
- **Non-régression impérative** : desktop #80, portrait #83, paysage #85, golden-path E2E (`data-testid="dashboard"` + `TimelineResponsive`). Les `data-testid` doivent rester en place (E2E).
- Tokens Graphite, TS strict (zéro `any`/`as`), theme-aware — inchangés.
- Tests OBLIGATOIRE : `cd frontend && npx vitest run` (le vrai runner ; `./scripts/test-quiet.sh frontend` = skip). Vérifie aussi `npx tsc --noEmit` sur les fichiers touchés (ignore les erreurs pré-existantes des `*.stories.tsx`). Adapte/complète les tests si un test référençait le champ `labelKey` retiré ou le comportement Escape.
- Commit : 1 commit logique gitmoji français (ex: `:recycle: #208 review — extraction nextEvent + ref scroll + useFocusTrap onEscape + nettoyage`). Puis **push** `git push origin sprint/20`.

## Livrable attendu (format strict, MAX 400 tokens caveman)
RETOUR :
- commits: [SHA1]
- fixes: <par finding (1→5) : ce qui a été changé + fichier>
- tests: <vitest passed/failed + tsc>
- non_regression: <confirme desktop/portrait/paysage + golden-path testids intacts>
