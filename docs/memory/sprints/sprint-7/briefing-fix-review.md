[BRIEFING CORRECTION REVIEW — Sprint 7]

## Contexte
Le reviewer batch du Sprint 7 a remonté 1 CRITIQUE + 2 MAJEUR sur le diff frontend. Tu dois corriger les 2 findings actionnables ci-dessous (le 3e MAJEUR est volontairement reporté en follow-up, NE PAS le traiter). Branche sprint/7 (déjà checkout, contient #40/#70/#48).

## À corriger

### [CRITIQUE] Fuite credentials dans les logs — `frontend/src/services/apiClient.ts` (~l.65-70)
Le handler d'erreur 403 (et possiblement 401/500) fait un `console.error` qui inclut `error.config.headers` — ces headers peuvent contenir `Authorization: Bearer ...` ou le cookie, et finissent dans la console navigateur + d'éventuels breadcrumbs Sentry.
- FIX : ne logger QUE des champs non sensibles : `{ url, method, status }` (et éventuellement `response.data` SI elle ne contient pas de PII — sinon l'omettre). Retirer toute inclusion de `headers` / `config` brut dans les logs.
- Vérifier TOUS les console.* du fichier, pas seulement le 403.

### [MAJEUR] Drift DTO front/back — `frontend/src/types/auth.ts` (~l.25-30)
Le type `User` frontend ne contient pas le champ `name`, alors que le `UserResponse` backend (#70) renvoie `{ id, name, username, email, role }`. Conséquence : la réponse de `PATCH /api/me` voit son champ `name` silencieusement perdu pour les consommateurs typés.
- FIX : ajouter `name: string` à l'interface `User` (et au schéma Zod `UserSchema`/équivalent s'il existe). Aligner exactement sur `UserResponse` backend : `id, name, username, email, role`.
- Vérifier que les conscommateurs (AuthContext, useCurrentUser) compilent toujours et n'ont pas besoin d'ajustement.

## NE PAS traiter (reporté follow-up, documenté pour /sprint end)
- [MAJEUR] `AuthContext.tsx` localStorage user (XSS-lisible) : pattern pré-existant A17, changement du modèle de persistance hors scope sprint 7. Laisser tel quel.
- Les MINEURs (TOCTOU username #42, optimistic lock, authService legacy /auth/me) : acceptés / staging.
...
## Dépendances
- Correctifs post-review, sur le diff déjà committé (#40/#48). Pas de dépendance.

## Designer
Non applicable.

## Contraintes
- Branche cible : sprint/7 (déjà checkout). NE PAS changer de branche.
- Commit : 1 commit logique gitmoji français (ex `:lock: fix review S7 — log sans credentials + User.name`). Inclure uniquement `apiClient.ts`, `types/auth.ts` (+ schéma Zod si modifié), et le(s) test(s) ajustés.
- Tests inline OBLIGATOIRES : `cd frontend && npx vitest run` (le wrapper `test-quiet.sh frontend` est un no-op — utilise vitest directement). La suite (12 tests) DOIT rester verte. Si tu touches le format de log 403, ajuste/complète `apiClient.test.ts` pour asserter que les headers ne sont PLUS loggés.
- `tsc --noEmit` doit passer après ajout de `name` au type User.
- NE touche PAS au backend, ni à `AuthContext.tsx` (le finding localStorage est reporté), ni à `package.json`. NE committe PAS `.eslintcache`.

## Livrable attendu (format strict, MAX 400 tokens caveman)
Écrire le retour DANS docs/memory/sprints/sprint-7/issue-fix-review-done.md. Format :
RETOUR :
- commits: [SHA1]
- resume: <ce qui a été corrigé (CRITIQUE log + MAJEUR User.name) + tests passés/total + tsc OK>
- [MEMORY:*] signaux: <si applicable>
- recommandations suite: <RECOMMAND_* ou négation>
