...
## DEC-S77-001 — `DateStamp` : dégradation par requête de conteneur, seuil dérivé de la mesure multi-locale
Contexte : le jour de semaine est un mot insécable dans une piste `1fr` sans plancher. Décision : `@container` sur la cellule, `@min-[52px]` sur l'enveloppe — sous le seuil, jour de semaine en `sr-only` et numéro conservé ; `overflow-hidden` en filet. Le seuil est **dérivé** de la mesure du jeton le plus large sur les 4 locales (`fr` « sam. » 33,3 px + padding 16 px = 49,3 px requis), pas choisi à vue — un premier seuil à 34 px avait ouvert une bande rognée de 34 à 50 px. Pourquoi : c'est le seul arbitrage juste pour les deux consommateurs d'échelles opposées (6 graduations vs N jours), sans amputer le jeton porteur ni perdre l'information pour les technologies d'assistance. ⚠ Calibré sur Archivo 500 aux tailles actuelles : un changement de fonte, de graisse ou de `px-*` l'invalide, et rien ne l'empêche mécaniquement. (Sprint 77 #191)

## DEC-S77-002 — Un garde-fou suit la DÉCISION qu'il protège, pas l'énoncé de l'issue qui le commande
#457 demandait de détecter un `ring-*` « sans le token `--color-focus` » ; `DEC-S58-001` rejette `ring-*` comme **mécanisme**, quelle que soit sa couleur. La seule violation du dépôt portait la BONNE couleur (`ring-ring` → `--color-focus`) : le critère littéral de l'issue l'aurait laissée passer. Écart tranché en faveur de la décision, et consigné aux deux endroits (en-tête du test + `DEC-S58-001`). (Sprint 77 #457)

## DEC-S77-003 — Ne pas bumper `LEGAL_LAST_UPDATED_ISO` pour une traduction
Contexte : `legal-pages.ts` demande de bumper la constante à toute modification de `legal.json`. Décision : **non bumpé** pour #533. Pourquoi : la date est une date d'**opposabilité** ; la version française fait foi et n'a pas changé ; la bumper signalerait faussement une révision des CGU. La règle du commentaire doit distinguer « texte source » et « traduction ». (Sprint 77 #533)

## DEC-S77-004 — Tolérance du diff visuel au POINT D'APPEL, pas dans un projet Playwright dédié
Contexte : la tolérance de #294 vivait au niveau racine de `playwright.config.ts`, donc comme défaut du dépôt, alors qu'elle est calibrée sur deux surfaces seulement. Décision : la descendre en constante `VISUAL_TOLERANCE` dans la spec. Pourquoi : le gabarit de nom des références porte `{projectName}` — un projet `visual` **renommerait les 10 PNG committés** et imposerait de les régénérer en conteneur pour un simple déplacement de tolérance. Et `test.use({ expect })`, l'autre piste, **n'existe pas** : `expect` n'est déclaré que sur `TestConfig` et `TestProject`, pas sur `TestOptions`. (Sprint 77, cycle 2 de review)

## DEC-S77-005 — Références visuelles générées sur l'image du RUNNER — décision révisée par le premier run CI
Décision initiale (#294) : conserver l'image `jammy` déjà présente, poser `maxDiffPixelRatio: 0.002` et **écrire le risque** plutôt que de le diluer dans une tolérance large, en assumant que la CI trancherait. Elle a tranché : **run 1 rouge**, 4 cartes auth, ratio 0,01 contre 0,002. Décision révisée : régénérer sur `playwright:v1.61.1-noble`, qui correspond à `ubuntu-latest` (24.04), contre un build de **production**. La tolérance n'a **pas** été élargie — l'écart de distribution valait le même ordre de grandeur que la plus petite régression détectable, l'absorber aurait rendu la spec incapable d'échouer. Pourquoi consigner l'échec : la décision initiale était défendable et documentée, et c'est le fait de l'avoir **écrite comme un risque** plutôt que dissimulée qui a rendu le diagnostic immédiat. (Sprint 77 #294, PR #536)

## DEC-S78-001 — `format:check` câblé en CI et dette de 119 fichiers reformatée dans le MÊME commit
Contexte : le dépôt déclarait `format`/`format:check` sans qu'aucun workflow ne les lance. Défaut consigné sous [[BUG-S71-002]], re-signalé aux S71, S75 et S77 sans jamais être tranché, et rouvert par l'issue #510 (fermée en doublon de #528). Décision : **câbler** le gate (step `Format (Prettier)` du job `frontend`, sans `continue-on-error`, dernière commande = prettier lui-même, cf. [[PIT-S64-007]]) et reformater la dette dans le même commit.

**Chiffre corrigé** : la dette n'était pas de 2-3 fichiers comme le laissait entendre l'énoncé, ni de 104 comme l'estimait le mini-plan architect (qui avait mesuré `src e2e`). Le script vaut `prettier --check .` : **119 fichiers**, exit 1 — c'est ce que la CI aurait vu. Répartition : 90 `src/`, 14 `e2e/`, 10 `app/`, 5 à la racine (`tailwind.config.ts`, `postcss.config.mjs`, `middleware.test.ts`, `i18n.ts`, `components.json`).

**Pourquoi câbler plutôt que retirer.** (1) `next lint` ne vérifie pas le formatage : retirer prettier laissait le dépôt avec **zéro** gate de formatage, et transformait 6 options délibérées de `.prettierrc` en conventions non opposables. (2) `prettier-plugin-tailwindcss` trie les classes selon l'ordre canonique — sur une base Tailwind v4 à design system, c'est une valeur de lisibilité et de diff, pas du cosmétique. (3) Le mini-plan **du S79 dépend déjà** de ce reformatage (« touche 10 specs e2e — l'absorber avant de réécrire les specs ») : retirer prettier invalidait un plan de sprint déjà écrit. (4) La dette est bornée, mécanique et non récurrente.

**Ce qui plaidait CONTRE, et qui est assumé.** (a) 119 fichiers reformatés polluent `git blame` sur 90 fichiers `src/` sans rapport avec un quelconque sprint. (b) Le tri de classes est un risque de cascade réel sur ce dépôt (cf. [[PIT-S53-001]]/[[PIT-S53-003]]/[[PIT-S53-004]]) — écarté par mesure, pas par principe : cf. ci-dessous. (c) Le dépôt a tenu 78 sprints sans ce gate, ce qui est un indice qu'il ne bloquait rien. (d) `husky` et `lint-staged` sont déclarés mais `.husky/` **n'existe pas** et il n'y a pas de script `prepare` : le chemin pre-commit qui rendrait le gate indolore est mort, les contributeurs le découvriront donc en CI.

**Innocuité du tri, mesurée et non supposée.** Deux contrôles. (1) Empreinte par fichier du multi-ensemble de classes, HEAD vs worktree, sur les 119 fichiers : **aucune classe ajoutée, retirée ni altérée** — seul l'ordre change (les 11 deltas résiduels sont des mots français de titres de tests, artefacts du changement de quotes). (2) L'ordre dans l'attribut `class` n'intervient pas dans la cascade Tailwind (le gagnant est fixé par l'ordre dans la feuille générée) ; la seule voie par laquelle il pourrait compter est `twMerge`, qui résout last-wins **dans la chaîne**. Audit des 1354 littéraux de classes des fichiers reformatés : **1 seul** auto-conflictuel, `'@source inline("rounded-xl rounded-sm …")'` dans `base-layer.test.ts` — une directive de génération, jamais passée à `cn()`, et **laissée intacte** par le tri. Le reformatage est donc inerte au rendu.

**Arbitrage secondaire** : le step est placé **après** `Lint`, en queue du job, plutôt qu'en tête. Motif : le job existant lance déjà le plus lent (`Build`) en premier, donc il priorise déjà les erreurs réelles sur les nits ; mettre un check de 2 s en tête ferait masquer un build cassé par une virgule mal placée. (Sprint 78 #528)

## DEC-S78-002 — Scope `frontend` de `test-quiet.sh` ÉTENDU, pas renommé
Contexte : le scope n'exécutait que Vitest alors que la doc annonçait « tests + build + typecheck + lint » ([[PIT-S60-009]], corrigé en doc au S60 mais jamais en comportement). Deux options : étendre le scope, ou le renommer `frontend-unit` et documenter les commandes séparément. **Décision : étendre** — `build → vitest → typecheck → lint`, dans l'ordre du job CI, arrêt au premier échec. Un scope `frontend-unit` (Vitest seul) est ajouté à côté.

**Pourquoi pas le renommage.** `test-quiet.sh frontend` est cité dans ~60 fichiers dont **~55 archives `sprint-N/issue-*-done.md`** : les réécrire falsifie le journal des sprints, les laisser fait pointer chaque archive vers un scope disparu. Surtout, renommer **déplace** le piège au lieu de le supprimer : le prochain agent lirait « frontend-unit vert » et en conclurait toujours quelque chose de faux sur le build.

**Ordre = celui de la CI, PAS « le moins cher d'abord ».** `tsconfig.json` inclut `.next/types/**`, donc un typecheck lancé avant le build lit les types d'un build antérieur et peut rougir sur une route fantôme ([[PIT-S60-007]]). Le build doit les régénérer d'abord.

**Coût assumé, mesuré** : 53 s contre 27 s à cache `.next` chaud (×2, davantage à froid). C'est pourquoi `frontend-unit` existe — et parce que `next build` réécrit `frontend/.next`, ce qui tue sans avertissement le `next dev` d'un agent voisin en worktree partagé ([[PIT-S62-009]]). (Sprint 78 #434)

## DEC-S78-003 — Couverture de tests : MESURER, sans aucun seuil bloquant
JaCoCo (`prepare-agent` + `report` lié à `verify`) et vitest `--coverage` (provider `v8`, reporters `text-summary` + `lcov`), publiés en artefacts CI. **Aucun seuil** : ni goal `check` / `<limit>` JaCoCo, ni `thresholds` vitest. L'issue #169 demande explicitement de mesurer avant d'imposer un plancher ; un seuil ajouté ici serait devenu un gate de merge surprise, non demandé et non calibré. Première mesure de référence : backend **90,49 %** instructions / 72,18 % branches, frontend **70,77 %** statements — chiffre frontend BRUT, gonflé par des configs racine happées par v8, à ne pas lire comme une cible.

Arbitrage secondaire : les steps `upload-artifact` portent `if-no-files-found: error` **sans** `if: always()`. Ils ne tournent donc qu'après une suite verte, et leur échec a un sens unique — le câblage est cassé. Avec `always()`, tout test rouge ferait aussi rougir ces steps et noierait le signal. (Sprint 78 #169)

## DEC-S78-004 — Le scope `frontend` refuse un verdict incomplet plutôt que de sauter une étape
Issu de la revue du sprint (2 MAJEUR). La politique héritée « script npm absent ⇒ skip retour 0 » se défend pour un scope optionnel ; elle est intenable dans un scope qui **annonce** quatre étapes, où elle produisait un skip silencieux suivi d'un « ✓ OK (build + tests unitaires + typecheck + lint) ». Décision : dans le scope `frontend`, les 4 étapes sont **requises** — script manquant ⇒ sortie 3 avec diagnostic. La politique de skip reste en vigueur pour `frontend-unit`. Complément : `|| return $?` explicite sur chaque étape, pour que la propagation ne dépende pas du contexte d'appel (`set -e` est désarmé dès qu'une fonction est appelée sous `if` ou `||`). (Sprint 78, revue)
## DEC-S79-001 — Livrer un placeholder fonctionnellement no-op, parce que la valeur livrée est la DÉCOUVRABILITÉ
#428 demandait de rendre `app.cors.allowed-origins` surchargeable en dev. La mesure a montré que la surcharge marchait **déjà** (précédence env > properties). Décision : livrer quand même `${APP_CORS_ALLOWED_ORIGINS:http://localhost:3000}`, et **ne pas** élargir le défaut dev à `:3000,:3100`. Pourquoi : le multi-ports violerait le critère « le défaut reste `http://localhost:3000` » ; et ce sont l'absence de trace écrite et l'impossibilité de voir la valeur appliquée — non une limite technique — qui ont coûté trois sprints de faux diagnostics. Le placeholder rend le levier visible dans le fichier qu'on ouvre en déboguant. Assumé comme tel : c'est un correctif documentaire, et le dire est plus utile que de le maquiller en correctif technique. (Sprint 79 #428)
...
## DEC-S81-001 — Cible de déploiement : l'instance Oracle Always Free existante, en mono-instance
Plutôt qu'un PaaS payant ou une base managée. Machine **mesurée** (sonde SSH lecture seule, pas une reprise de doc) : `VM.Standard.A1.Flex`, aarch64 Neoverse-N1, 2 OCPU / 12 Go, 39 Go libres, Ubuntu 22.04.5, Docker 29.5.3 déjà installé, ports 80/443 libres. MyTimeline consomme < 3 Go, soit 4× de marge. Conséquences **explicites et révocables** : #212 (avatars sur volume local) et #102 (rate-limit en mémoire) sortent du chemin critique — ils redeviennent bloquants au premier passage à deux instances ; #115 (CSP `connect-src`) devient sans objet en mono-domaine. Voir ADR-009. (Mise en ligne #369)

## DEC-S81-002 — `matimeline.com` canonique, les deux autres domaines en 301
Trois domaines détenus (`matimeline.com`, `ma-timeline.com`, `ma-timeline.fr`). Retenu : `matimeline.com` — sans tiret (se dicte à l'oral), `.com` cohérent avec les 4 locales servies, aligné sur le package `com.matimeline.eventmanager`. Topologie **mono-domaine** derrière Caddy (`/api/*` → backend, le reste → frontend) : cookie same-origin en `SameSite=Lax`, aucun préflight CORS. Les redirections sont portées par Caddy, pas par le garde applicatif #322 — qui, lui, supprime volontairement la query string. (Mise en ligne #369)

## DEC-S81-003 — Build arm64 sur runners GitHub natifs, pas en QEMU ni sur l'hôte
L'hôte est aarch64, les runners par défaut x86. Retenu : `ubuntu-24.04-arm`, **gratuit car le dépôt est public**. Écartés : buildx + QEMU (un build Maven puis `next build` émulés se comptent en dizaines de minutes) et le build sur l'hôte de production (place les sources et la chaîne de build sur la machine qui sert les utilisateurs, et monopolise ses 2 OCPU). Aucun `Dockerfile` n'a été modifié : `eclipse-temurin:21-jre`, `node:20-alpine` et `postgres:16` publient tous de l'arm64. (Mise en ligne #370)

## DEC-S81-004 — Staging reporté sur une 2e VM gratuite, pas en cohabitation sur l'hôte
Écart assumé à #370. `NEXT_PUBLIC_API_URL` étant **bakée au build** de l'image frontend, un staging sur un autre domaine exige une seconde image, pas seulement une seconde configuration ; et faire cohabiter deux stacks sur un hôte imposerait un Caddy partagé entre deux projets Compose (un seul processus peut lier 443) avec des alias de service en collision. L'enveloppe Always Free (4 OCPU / 24 Go) laisse exactement de quoi créer une seconde VM gratuite. En attendant, la production est le seul environnement et n'est jamais déclenchée automatiquement. (Mise en ligne #370, suivi #562)

## DEC-S81-005 — `POST /api/me/avatar` est plafonné à 10/min/IP, pas 5
...
## DEC-S81-006 — #500 n'est PAS fermée malgré 4 runs verts
...
## DEC-S81-007 — Le correctif de la fuite de JWT s'arrête au périmètre du sprint
Le masquage de `Set-Cookie`/`Authorization`/`Cookie` a été appliqué au helper de diagnostic livré par #500. La fuite **complète** vient de Spring ([[PIT-S81-023]], 17 classes) et son correctif — `MockMvcPrint.NONE` ou la propriété équivalente — prive 17 classes de test de leur dump d'échec. Cet arbitrage de diagnostic n'est pas pris en fin de sprint, sur la base d'une découverte tardive : issue dédiée. **Limite déclarée dans le message de commit et dans l'audit plutôt que masquée.** (Sprint 81, review sécurité)

## DEC-S82-001 — Auth garde ses 4 routes, mais prend le layout du spec via un groupe `(auth)`
Le handoff décrit un écran unique à 3 onglets ; la prod a 4 routes, posées **dès le scaffold d'avril**, avant le DS Graphite, et jamais reconfrontées. Ni A ni B : les 4 URL restent (lien de reset adossé à un token, locale de route qui pilote la langue de l'e-mail #142, `/register` référençable, cible de redirection après 401), et l'unité visuelle vient d'un groupe `app/[locale]/(auth)/layout.tsx` portant le panneau asymétrique, les onglets devenant des `<Link>` + `aria-current`. Un layout partagé App Router ne se re-rend pas au changement d'enfant : la bascule est quasi instantanée, ce qui annule le seul avantage des vrais onglets. **Conséquence : #625 passe de M à S** (panneau écrit une fois, pas quatre). Handoff à mettre à jour. (Sprint 82 #579)

## DEC-S82-002 — Social login Google/GitHub retiré du MVP (reporté, pas rejeté)
Le blocage n'est pas OAuth mais **l'identité de compte** : MyTimeline identifie par `username`, pas par e-mail (BR tracée `lib/schemas/auth.ts:16-17`). Un fournisseur rend un e-mail → il faut trancher le `username` généré, la collision avec un compte mot-de-passe existant, et une table de liaison donc une migration. Chantier réel pour un produit dont le déploiement n'est pas fiabilisé (certificat prod non résolu, sauvegardes #371 absentes). Bloc « ou continuer avec » retiré du handoff. Réouverture conditionnée à une mesure d'abandon à l'inscription attribuable à la friction — avec la question `username` tranchée **en préalable**, pas après. (Sprint 82 #580)

## DEC-S82-003 — L'état de chargement nomme l'action en cours
Le handoff impose « Un instant… » ; le code affiche « Connexion… », « Envoi… », « Inscription… ». Le code a raison : le libellé contextuel porte une information, le générique n'en porte aucune — différence qui compte surtout pour un lecteur d'écran, le `role="status"` annonçant le texte à voix haute. Le spec n'avait vraisemblablement pas anticipé que 4 parcours partageraient le composant. Règle actée, handoff mis à jour. Le spinner générique (`spinner.loading` = « Chargement en cours ») doit hériter du contexte quand il en a un. (Sprint 82 #581)

## DEC-S82-004 — Catégories par défaut : suggestions non persistées, pas de seed
Les 7 catégories du handoff n'existent ni en base ni en seed (constat explicite dans `V8__category_ownership.sql`). Le seeding a un piège que ni « semer » ni « assumer le vide » ne voient : **les libellés de catégorie sont des données utilisateur**. Semer « Véhicules » pour un compte francophone fige du français en base ; l'utilisateur qui passe en allemand garde des catégories françaises, définitivement, puisque ce sont ses données — et aucune traduction ultérieure ne peut les rattraper sans écraser ce qu'il a pu renommer. Retenu : suggestions **localisées à l'affichage**, proposées à la première création, **persistées seulement au choix**, avec le libellé dans la langue du moment. Purger la clé orpheline `products.add.categories` (4 libellés divergents, référencée nulle part). ⚠ Bloqué par #577 : ne pas figer les couleurs avant la palette unique. (Sprint 82 #582)

## DEC-S82-005 — Trois unités de récurrence, contre le mock qui en prévoit deux
L'hebdomadaire, absent du handoff, est conservé : l'omettre créerait une asymétrie création/édition, l'édition devant afficher les séries hebdomadaires existantes. Le développeur avait raison contre la maquette — **seul cas de l'audit** — et l'avait écrit en commentaire ; la décision est consignée ici pour qu'un prochain audit ne la re-signale pas. Handoff mis à jour. Sur la forme : segmenté via `.mt-seg` (seuils de repli DE déjà présents), **mais mesurer en allemand avant de conclure** — « Keine / Wöchentlich / Monatlich / Jährlich » sur 4 positions est le cas limite ; si ça ne tient pas à 390 px, le `Select` reste acceptable et la décision doit alors être écrite, pas subie. (Sprint 82 #583)

## DEC-S82-006 — Le filtrage du champ Produit est DANS la charte : étendre `Select`, pas ajouter un combobox
`NewEventDrawer.tsx:285-286` justifie le `Select` par « aucun combobox nouveau : hors charte ». **Le commentaire est faux** : le manifeste du DS décrit sa primitive comme « Select (combobox a11y) ». Retenu : étendre le `Select` existant au filtrage, une seule primitive de sélection dans le produit. Champ de filtre visible au-delà d'une dizaine d'éléments, saisie filtrante toujours active — cas d'usage direct de MyTimeline, où le nombre de produits croît avec l'ancienneté du compte. **Corriger le commentaire en même temps que le code**, sinon il continuera de justifier le statu quo. Navigation clavier à couvrir par un test. (Sprint 82 #584)

## DEC-S82-007 — La métrique « jour de série » est retirée
Une mécanique de série récompense l'**assiduité** : elle suppose un comportement répété que l'application veut encourager. MyTimeline suit des échéances **subies** — garantie, assurance, contrôle technique — dont l'utilisateur ne choisit pas le rythme. En pratique la métrique compterait des jours de connexion consécutifs, donc récompenserait l'ouverture de l'application : de la fidélisation pour elle-même sur un outil dont la promesse est justement de **ne pas avoir à y penser tous les jours**. Elle travaille contre la valeur du produit. Remplacée par les métriques du handoff : échéances sous 14 jours, couvertures en cours, catégorie la plus chargée du mois. Le rendu de « En bref » est par ailleurs **conforme** (marginalia phrasée) — c'est un changement de données, pas de mise en forme. (Sprint 82 #585)

## DEC-S82-008 — Landing : `MobileAppSection` et `TimelinePreviewSection` retirées
`MobileAppSection` annonce des applications mobiles **qui n'existent pas** (scaffold KMP archivé, jamais commité). Ce n'est pas du design mais une **promesse publique**, engageante selon la formulation ; la muer en « bientôt » resterait un engagement, avec en prime une collecte d'e-mails donc une obligation RGPD. Suppression pure. `TimelinePreviewSection` fait doublon avec la frise du hero — doublon aujourd'hui invisible parce que cette frise est un placeholder (#611), la page montrant **deux aperçus dont aucun n'est celui du spec**. ⚠ Ordonnancement : la retirer **après** #611, sinon la landing se retrouve sans aucun aperçu du produit. (Sprint 82 #586)

## DEC-S82-009 — Bascule de thème globale, exposée aussi hors connexion
Le mode sombre est une préférence **système**, pas d'application : imposer du clair à un visiteur dont l'OS est en sombre est une mauvaise première impression, et il n'a aujourd'hui aucun recours, la bascule n'existant qu'après connexion. Tout est déjà en place (`ThemeProvider`, tokens complets, persistance `next-themes`) — seule l'exposition manque. Persistance : stockage local avant connexion ; à la connexion, la préférence de compte gagne si elle existe, sinon le choix local est adopté et devient celle du compte. **Règle une fois pour trois surfaces** : landing, 4 pages Auth, en-tête de frise. Vigilance sur le flash de thème au premier rendu des routes publiques (statiques) — vérifier le script de pré-hydratation. (Sprint 82 #587)

## DEC-S82-010 — Les points de rupture sont des tokens `--breakpoint-*` dans `@theme` (Tailwind 4)
Le débat « paliers Tailwind ou tokens DS » est sans objet en Tailwind v4 : les breakpoints s'y déclarent `--breakpoint-*` dans `@theme`, exactement comme les couleurs et espacements déjà exposés dans `globals.css`. Les y mettre les rend simultanément utilisables en classes (`md:`, `lg:`) et lisibles comme tokens — pas de namespace `--bp-*` concurrent à inventer. `AppShell` et `ProductsListView` deviennent conformes sans changement. **Exception assumée** : les seuils en *hauteur* (600 px, 400 px du mode paysage) n'ont aucun équivalent Tailwind et restent des media queries, mais avec des constantes nommées. ⚠ **`Paysage Breakpoints.dc.html` n'a pas été ouvert** : les valeurs 640/600/400 n'ont jamais été comparées à celles dessinées — les relever est le premier pas, inutile de figer des tokens sur des valeurs peut-être fausses. (Sprint 82 #588)

## DEC-S82-011 — RTL hors périmètre, mais propriétés logiques par défaut pour tout nouveau composant
Livrer le RTL coûte d'abord de la **traduction** (ar/he, relecture native), très au-delà du câblage `dir` et des propriétés logiques ; rien dans la feuille de route ne le justifie. `Preview RTL.dc.html` marquée exploratoire, classes RTL du DS (`.mt-timeline-ltr`, `.mt-dir-icon`, `.mt-sheet-accent`, `[dir="rtl"]`) documentées comme **volontairement dormantes** dans `i18n.css` — pour qu'un audit ne les re-signale pas comme code mort et qu'un ménage ne les supprime pas. Garde-fou gratuit conservé : **propriétés logiques comme convention par défaut** (`inset-inline-end` plutôt que `right`, `border-inline-start` plutôt que `border-l`) — même effort à l'écriture, et la dette cesse de croître à chaque nouveau drawer. L'existant n'est **pas** à reprendre : aucun bug tant qu'aucune locale RTL n'est livrée. Cette décision ne débloque aucun lot, elle en **ferme** un. (Sprint 82 #589)
...
## DEC-S85-007 — `ink-muted` remplace `ink-faint` partout où la sidebar de la frise porte du TEXTE
**Contexte.** La maquette pose `ink-faint` sur les titres de bloc, les compteurs et le pied de la sidebar : 2,82:1 et 2,99:1 mesurés sur `surface`, sous le seuil AA de 4,5:1.
**Décision.** `ink-muted` pour tout texte de la sidebar #592 et pour le compteur d'en-tête #601. La pastille en contour, elle, reste sur un token de filet (tier décoratif).
**Pourquoi.** Même arbitrage qu'au S84 pour l'orchidée : l'AA prime sur la valeur exacte du handoff. `ink-faint` reste consommé ailleurs comme texte (eyebrow de `timeline/page.tsx`), à un ratio identique — c'est une décision de CHARTE, pas une correction de ce sprint.
**Portée.** Follow-up ouvert : relever le token ou le réserver au non-textuel. (Sprint 85 #592, confirmé par la revue `ui-design`)

## DEC-S85-008 — Replier une catégorie au-dessus du viewport ne compense PAS le `scrollY`
**Contexte.** Critère 4 de #601 (« la virtualisation gère le passage déployé ↔ replié sans saut de position »). Replier une catégorie située au-dessus de la zone visible fait remonter le contenu de N × 46 px (mesuré, exactement la hauteur des lanes retirées).
**Décision.** Aucune compensation de `scrollY`. Ce qui est garanti : hauteur de rangée identique pliée et dépliée, en-tête replié immobile, catégorie suivante collée, lanes aux positions du modèle.
**Pourquoi.** Chromium n'ancre pas ici (il choisit `.mt-tlv__scroll` comme ancre), et le cas n'est atteignable qu'au clavier avec `focus({preventScroll})` — l'utilisateur qui replie voit l'en-tête qu'il a cliqué. Compenser coûterait un suivi de position pour un cas qui ne se produit pas au pointeur.
**Portée.** Si un « tout plier » depuis le clavier devient un parcours courant, rouvrir. (Sprint 85 #601)

## DEC-S85-009 — Hauteurs de la barre d'outils : 26 px et 32 px, pas les 38 px de la maquette
**Contexte.** La maquette §D pose 38 px pour « Aujourd'hui » et « Nouvel événement », dans un EN-TÊTE d'écran. La prod n'a pas cet en-tête : ses contrôles vivent dans `.mt-tlv__toolbar`, alignés sur 26 px.
...
## DEC-S92-001 — Toasts : react-hot-toast reste le moteur, `ui/toast.tsx` devient le rendu unique
Arbitrage dev (#621) : « brancher » plutôt que supprimer le composant DS. `AppToaster` (`ui/toaster.tsx`) passe par la render-prop `children` de `<Toaster>` : les 15 appels `toast.success/error` existants prennent le traitement DS sans réécriture, le conteneur `#_rht_toaster` reste (masque de `sprint-77-theme-visual`). Écartés : `toast.custom` (réécrire chaque appel), `useToaster` seul (perdre le conteneur). `--z-toast` passe de 60 (mort) à 78 : au-dessus de `--z-modal` (70) et `--z-popover-over-modal` (75), sous `--z-netbanner` (80) — approuvé par le Designer ; ferme #460. Messages dans `common.toast.*`, impersonnels. (Sprint 92 #621)

## DEC-S92-002 — Toast : pause au survol ET au focus, sans bouton fermer, 72 px sous le haut
Arbitrages dev (revue Designer puis conséquence mesurée) : la durée de lecture doit être ajustable (WCAG 2.2.1) → seule la carte visible capte le pointeur et est focusable (`tabIndex=0`, jamais focalisée automatiquement), survol ou focus pausent via `useToaster().handlers` ; la pause est GLOBALE au store react-hot-toast (tous les toasts visibles). Position `top-right` décalée de `calc(--space-5 + --space-11 + --space-2 + safe-area)` = 72 px pour dégager croix des drawers, fermer tactile paysage et hamburger mobile. Non dégagés, documentés : croix du `ProductDrawer` en bottom sheet mobile, haut d'un drawer ouvert. (Sprint 92 #621, `5c64e01`, `87dca1d`)

## DEC-S92-003 — Liste produits : « Dernière activité » retirée au profit du prochain événement
Arbitrage dev (#603). Colonnes : Produit (catégorie mono sous le nom, donc plus de colonne ni de tri par catégorie) · Prochain événement (titre + date ISO, toujours visible) · mini-frise 90 j (`md:`) · nombre d'événements actifs (`sm:`) · Actions. Tri par défaut « Prochain événement », sans échéance en dernier. `nextStart` borné par `seriesHorizon` : une série non bornée démarrée il y a plus de 5 ans n'a pas d'échéance, même règle que les fantômes de la frise (DEC-S91-004). (Sprint 92 #603)
...
> **CADUQUE depuis le Sprint 101 (#735)** : divergence refermée, bloc de tests supprimé — voir [[DEC-S101-002]].
...
## DEC-S96-001 — Un critère « vérifier qu'aucune autre X n'existe » se borne dans l'issue, pas au briefing
Le 3e critère de #633 supposait exact l'énoncé « une exception » ; le balayage, borné aux réglages par le lead, l'a INFIRMÉ (`settings-back` 36 px sur le même écran, primitives shadcn à 36/32 px). #633 corrige un symptôme ; la cause systémique est la hauteur des primitives partagées et relève d'un arbitrage Designer à l'échelle du produit, pas d'une rustine par bouton. Un critère de balayage non borné est soit ingérable, soit clos à tort. (Sprint 96, #633)

## DEC-S96-002 — #656 vise WCAG AAA (2.4.12), tranché par 2.5.8 et non par 2.4.11
Le recouvrement tenait 2.4.11 (AA). Mais la cible tactile des déclencheurs vaut 44 px, 20 étaient masqués : il en restait exactement 24, le seuil AA 2.5.8 au pixel près, sans marge. Le dégagement complet coûtait un mot-clé CSS (`relative`) et zéro déplacement perçu hors bannière (mesuré) : l'arbitrage ne se posait pas. Réserve : AAA vaut au repos et à la réception du focus, pas après un défilement utilisateur de 32 px (résidu encadré par un test, follow-up). (Sprint 96, #656)

## DEC-S96-003 — Palette : agrandir la boîte en mobile plutôt qu'appliquer PAT-S24-002
`size-11 sm:size-7` au lieu d'un `::before` 44×44 hors flux, pourtant motif maison : au pas de grille d'origine (36 px) les pseudos se chevaucheraient de 8 px, et les trois surfaces sont des panneaux défilants où `overflow-y:auto` force `overflow-x:auto` — le débordement du pseudo serait rogné en silence. La charte n'exige « conserver le visuel » que pour le `✕` de fermeture. Conséquence assumée : disque visuellement plus grand en mobile ; taille desktop strictement inchangée, DISPOSITION desktop délibérément changée (11+1 → 6+6). (Sprint 96, #665)

## DEC-S96-004 — #702 corrigé côté test uniquement, et la décision reste réfutable
La sonde `focusin` établit que le `Select` rend le focus une seule fois, ~29 ms après le clic, et que le focus clavier tient ensuite 1,5 s : aucun utilisateur ne peut s'insérer dans la fenêtre, et la restitution au déclencheur est le comportement attendu du motif. Le défaut était la synchronisation du test. Pour que ce choix ne masque rien : oracle de durabilité en lecture UNIQUE non réessayée de `document.activeElement`, et contrôle négatif (ligne d'attente neutralisée → 2 échecs/6). Zéro ligne de production. (Sprint 96, #702)
...
## DEC-S98-001 — Libellés de frise : réserve ESTIMÉE sans DOM plutôt que troncature ou mesure
Contexte : #746, libellés de pin > 100/90 px et libellé extérieur de secours (0 px réservé) chevauchaient l'occurrence suivante de leur rangée (#709). ui-design recommandait la troncature à 84/74 px (`sprint-98/ui-design-746.md`). Décision du dev : réserve estimée par `applyLabelReserves` (`label-reserve.ts`), avant `layoutLane` — caractères × chasse 0,525 em (6,3 px desktop / 6,6 px mobile), ↻ compté 3 caractères ; pin `clamp(11 + texte, 100|90, 251)` ; libellé extérieur `6 + min(8 + texte, 240)` (desktop seul, le mobile n'en rend pas). Le libellé est borné à sa réserve en CSS (`--mt-label-max`, ellipse, `title`) : zéro chevauchement même si l'estimation sous-évalue. Pourquoi : 84 px ≈ 12 caractères et le doigt n'a pas de survol ; une mesure canvas/DOM casserait la pureté et le déterminisme de `layoutLane` (DEC-S97-002). Conséquence assumée : plus de rangées quand les titres sont longs ; `widthPx` des barres inchangé. (Sprint 98, #746)

## DEC-S98-002 — Zoom des frises mobiles : ancre au CENTRE de la piste visible
Contexte : #747, les frises mobiles ne ré-ancraient pas le défilement au changement de zoom. Décision (lead, au démarrage) : l'ancre est le jour au centre de la zone de piste visible (hors colonne sticky de 120 px), alors que le desktop (#449) ancre au bord gauche de piste. Pourquoi : sur 360-850 px l'objet regardé est au centre, et le pinch agit au centre ; ancrer au bord gauche ferait sortir la zone regardée au zoom avant. Un seul hook (`useTimelineMobileState`) sert portrait et paysage. (Sprint 98, #747)

## DEC-S99-001 — Cibles tactiles 44 px : champs sur la primitive, boutons au consommateur (réglages)
Contexte : #738, `Input`/`SelectTrigger` 36 px, `SelectItem` 36,28 px, `Button` 32/36 px sous 768 px. ui-design recommandait de n'agrandir que les champs (`sprint-99/ui-design-738-739.md`), ce qui laissait les 17 boutons des réglages hors critère. Décision du dev : `max-md:h-11` sur `Input`/`SelectTrigger` et `max-md:min-h-11` sur `SelectItem` (primitives, donc aussi hors réglages) ; primitive `Button` INCHANGÉE, `max-md:h-11` (+ `w-11` en icône) posé par les écrans de réglages via `settings/touchTarget.ts`. Pourquoi : un champ se touche sur toute sa surface et n'a aucun usage dense ; `Button size="sm"` porte la densité voulue du dashboard et de la frise, et `language-selector`/`theme-toggle` étendent déjà leur cible par pseudo (PAT-S24-002). `pointer: coarse` écarté (désaligné de la bascule `max-width: 767px`). Le reste de l'app mobile → issue de suivi. (Sprint 99, #738)

## DEC-S99-002 — Poignée des bottom sheets : 28 px, exemptée par WCAG 2.5.8 « Equivalent »
Contexte : #739, zone de glisser 28 px (`settings/mobile/BottomSheet.tsx` `h-7`, `.mt-sheet__grabber-zone` de la frise) alors que `a11y-audit.md` exigeait 44×44. Décision du dev (reco ui-design) : exception 2.5.8 — la croix 44×44 et Escape ferment le même panneau ; la même croix satisfait 2.5.7. `a11y-audit.md` corrigé, condition écrite à côté des deux poignées : retirer la croix 44 ou Escape ré-ouvre l'obligation d'agrandir. Pourquoi : +16 px pris au contenu d'un sheet borné, pour une action déjà offerte par un contrôle conforme. (Sprint 99, #739)

## DEC-S99-003 — Décision B #714 étendue : recouvrement partiel toast/croix accepté à 390×844
Contexte : les champs 44 px (DEC-S99-001) font passer le contenu du drawer de création produit de 702 à 726 px ; la croix remonte de 158 à 134 px et passe 3,5 px sous la carte du toast à 390×844 (0 avant). Décision du dev : accepté ; l'oracle TALL de `sprint-95-toast-overlap` passe de « nul » à « encadré ]0 ; hauteur de croix[ », JSDoc `toaster.tsx` remesurée. Pourquoi : la sortie de secours (tap sur la bande d'overlay, disjointe du toast) est intacte, et les cibles 44 px priment ; déplacer le toast rouvrirait l'arbitrage #714. Borne basse ≈ 3,5 px de marge si les métriques de police Linux diffèrent. (Sprint 99, #738) **Remplacée par DEC-S100-001** (le 3,5 px était un artefact du débordement horizontal).

## DEC-S100-001 — Décision B #714 étendue au régime TALL : croix entièrement recouverte par le toast à 390×844
Contexte : passer `DialogContent` de `grid` à `flex` (#732/#740) supprime le débordement horizontal de la sheet produit mobile (BUG-S100-001) ; le texte se replie à la bonne largeur, le contenu passe de 726 à 802 px, dépasse 92vh (776 px) dès 390×844, et la croix est alors ENTIÈREMENT sous le toast d'erreur, comme le régime MID. Décision du dev (2026-09-21) : accepté, `sprint-95-toast-overlap` TALL attend un recouvrement total, JSDoc `toaster.tsx` réécrite. Pourquoi : la position du toast (72 px) ne bouge pas, la sortie structurelle (tap sur la bande d'overlay 0–68 px) est intacte, et le recouvrement ne dure que le temps du toast. **Remplace le « 3,5 px » de DEC-S99-003**, qui était un artefact du débordement (PIT-S100-004). (Sprint 100, #732/#740)

## DEC-S101-001 — Un 403 affiche « Accès refusé » en toast, SANS redirection vers /login
Contexte : l'intercepteur traitait le 403 comme une session expirée (toast « session expirée » + `window.location.href` vers `/{locale}/login`). Le backend n'émet un 403 qu'à un utilisateur AUTHENTIFIÉ (ownership dans `ProductController`/`CategoryController`, `AccessDeniedException` dans `EventController`, `hasAuthority("ROLE_USER")`), CSRF désactivé, l'expiration produisant un 401. Décision du dev (2026-09-21) : toast seul, verrou `isRedirecting` non pris ; clé `errors.auth.forbidden` (ex-`forbiddenRedirect`), distincte du groupe `errors.forbidden` de l'écran d'erreur. (Sprint 101, #733)

## DEC-S101-002 — `PASSWORD_POLICY` réplique exactement `StrongPasswordValidator` : `\p{Uppercase}` et `\p{Nd}` bornés au BMP
Remplace [[DEC-S95-005]] (divergence ASCII/Unicode figée par des tests), désormais caduque. `/(?=[\0-\uFFFF])\p{Uppercase}/u` et `/(?=[\0-\uFFFF])\p{Nd}/u` reproduisent `Character.isUpperCase(char)`/`isDigit(char)` itérés sur les unités UTF-16 ; `\p{Lu}` prescrit par l'issue a été écarté ([[PIT-S101-002]]). L'invariant #508 (refusé serveur ⇒ jamais `strong`/`medium`) tient dans les deux sens. (Sprint 101, #735)

## DEC-S101-003 — Cibles tactiles 44 px étendues hors réglages ; `lib/touchTarget.ts` source unique
Suite de [[DEC-S99-001]] (réglages seulement). Arbitrage ui-design S101 : `TOUCH_TARGET_BUTTON` (`max-md:h-11`, croissance réelle) sur les surfaces spacieuses (pieds de drawers, dialogues de confirmation, CTA d'état vide) ; `TOUCH_TARGET_HITBOX` (pseudo `::before` 44×44, PAT-S24-002) sur les rangées denses et les contrôles de formulaire à boîte fixe (case, interrupteur) ; `size="sm"` conservé, desktop inchangé ; `settings/touchTarget.ts` ré-exporte. Croix de `DialogContent` : 44×44 sous 768 px, coin haut-droit ancré (+16/+16), fond opaque `bg-background`, encre `text-muted-foreground`, survol sur la seule surface (5,96:1 clair, 6,26:1 sombre, mesurés). Exemption : `TimelineView` `timeline-new-event` (jamais peint sous 768 px). (Sprint 101, #754/#757)

## DEC-S102-001 — Indicateur de force : minuscule `\p{Lowercase}` bornée au BMP, symbole = ni lettre, ni marque, ni chiffre
Contexte : `scorePassword` comptait toute lettre ou tout chiffre non-ASCII comme symbole (`Ωabcdefg١` → « fort »). Il n'y a pas de contrepartie serveur pour « minuscule » ni « symbole » (heuristique d'affichage). Décision : minuscule = `/(?=[\0-\uFFFF])\p{Lowercase}/u` (symétrique de `PASSWORD_POLICY.uppercase`, DEC-S101-002) ; symbole = `/[^\p{L}\p{M}\p{N}]/u` ; un diacritique combinant n'est pas un symbole (un `é` NFD ne note pas plus fort qu'un `é` NFC), une lettre hors BMP ne rapporte rien. Principe : ne jamais surévaluer. Porte #508 (`meetsPolicy`) inchangée. (Sprint 102, #762)
...
## DEC-S109-001 — En-têtes de section du tableau de bord : aucun sur-titre, compteur à droite du titre
Relevé de maquette au démarrage du S109 (`docs/memory/sprints/sprint-109/maquette-sections.md`) : aucune des 6 sections (« Cette semaine », « En bref », « Tes produits », agenda compact, carrousel, sous-frise produit) n'a de sur-titre ; la donnée est un compteur mono `ink-muted` À DROITE du `h2`. Arbitrage dev : compteurs « n événements » (semaine, agenda compact qui compte ce qu'il affiche : aujourd'hui + demain), « n produits » (liste, carrousel), plage de dates de la sous-frise ; rien sur « En bref » ni l'historique (DEC-S84-004 conservée) ; pas de compteur en état vide. Taille 13 px (`text-2xs`, plus petit palier du DS) au lieu des 11/10 px de la maquette, écart entériné en clôture. La plage de la sous-frise est l'étendue des événements filtrés, pas la fenêtre visible (suivi en issue). (Sprint 109, #664)

## DEC-S109-002 — `.mt-eyebrow` pour tous les sur-titres mono capitales ; 6 utilitaires i18n du DS documentés dormants
L'eyebrow de date du salut passe sur `.mt-eyebrow` : la maquette le pose en 10 px `ink-muted` (tranche le point laissé ouvert au S84, plus qu'une taille d'eyebrow sur le dashboard). Idem `CompactAgenda`, `MobileDrawer`, page et squelette de la Frise. `.mt-seg*`, `.mt-tabs--collapsible`, `.mt-btn--wrap`, `.mt-truncate`, `.mt-eyebrow--wrap`, `.mt-eyebrow--title` n'ont aucun cas d'usage : documentés « volontairement dormants » dans l'en-tête de `ds/components/i18n.css`, CSS conservée. Le badge « archivé » de la fiche reste un badge (bordure, pas mono), détendu par `[&:lang(de)]:tracking-[.02em]`. (Sprint 109, #632)

## DEC-S110-001 — Écrans d'erreur : écarts assumés avec la maquette `États système`
Voix : celle déjà en place par locale (vous en fr/de/en, tu en es), pas le tutoiement de la maquette. Encres de texte `ink-muted` au lieu du `ink-faint` de la maquette (DEC-S97-001). Feuillet du 404 en `bg-surface` : il repose sur la page (`bg-bg`) et non sur une carte `surface` comme sur la planche. Bouton « Retour à l'accueil » conservé tant que `/${locale}` sert la landing aux personnes connectées (suite en issue). Référence d'incident = `error.digest` seul (hash djb2 de Next, non réversible), jamais la valeur factice `MT-AAAA-NNNN` du prototype ; rien d'affiché sans digest. (Sprint 110, #627 #628)
