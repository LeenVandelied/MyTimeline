...
En S37 (#145, premier E2E métier), la DB locale `eventmanager` était bloquée à une vieille version : le boot backend échouait à V7 (`events_recurrence_unit_check`) — sans lien avec le code du sprint. **Corrigé S88 #545** — deux points étaient faux : (1) « données stale » : les données sont convertibles (`weeks`/`years`) ; la cause est une contrainte CHECK **créée hors Flyway** (nom auto Postgres `events_recurrence_unit_check`, aucune migration ne le pose) sur une base adoptée par `baseline-on-migrate`, que V7 ne retire pas avant de passer les valeurs en majuscules — détail dans [[PIT-S47-003]] ; (2) « bloquée à V3 » : l'historique Flyway lu au S88 montre V4..V6 appliquées le 2026-07-13, la base est en V6 (état au S37 non vérifiable). Solution non destructive : `CREATE DATABASE eventmanager_e2e` + `DB_URL` dessus → Flyway rebuild propre depuis zéro. **Règle : les E2E tournent sur une DB jetable fraîche, JAMAIS la DB dev polluée.** En CI le Postgres est déjà frais (service jetable). Follow-up : découpler la capture du token E2E du schéma DB (voir [[PAT-S37-002]]). (Sprint 37 #145)
...
**Corrigé S88 #545** (titre d'origine : « inmigrable : V7 casse sur des données que V9 nettoierait » — diagnostic faux). En S47, booter le backend sur la base de dev (figée à V6) échoue à `V7__design_v3_schema.sql` avec `violates check constraint "events_recurrence_unit_check"`. **Ce qui était faux** : (a) les données ne sont pas invalides — valeurs lues au S88 : `weeks`×1, `years`×2, NULL×16, toutes convertibles, et le pré-vol de V7 (qui lève `V7 abort : recurrence_unit non convertible…` sur une valeur inconvertible) passe ; (b) V9 ne « nettoierait » rien : elle ne retire elle aussi que `ck_events_recurrence_unit` et réécrirait en majuscules, donc buterait sur la même contrainte. **Cause observée** : la base (Homebrew PG14 `127.0.0.1:5432`) porte, en plus des `ck_events_*` posées par V4, les contraintes legacy à nom auto Postgres `events_recurrence_unit_check`, `events_duration_unit_check`, `events_type_check`, `users_role_check` — créées avant Flyway (schéma adopté par `baseline-on-migrate=true`, ligne `<< Flyway Baseline >>` en rang 1). V4 annonçait les réconcilier mais ne fait `drop constraint if exists` que sur les noms `ck_*`. V7 ligne 88 passe `weeks`→`WEEK` : rejet par `events_recurrence_unit_check` (minuscules), transaction annulée, base restée en V6. **Reproduit** au S88 sur Postgres 16 jetable (V1..V6 + CHECK sans nom ajouté à la main + une ligne `weeks` → même erreur, SQLSTATE 23514). Un clone neuf n'a jamais cette contrainte : `flyway-smoke` (V1..V15 sur base vierge) est vert à juste titre. **Règle : pour tout boot local, une base neuve dédiée (`createdb` + `DB_URL`) ; `eventmanager_e2e` et `eventmanager_s79` (V15) sont des contournements, pas des références. Ne pas « réparer » `eventmanager` par une migration** (V16 ne s'exécuterait jamais avant V7 ; éditer V7 casse le checksum). Corollaire de conception : une migration qui réconcilie des contraintes legacy doit les chercher dans `pg_constraint` par définition/colonne, pas par un nom supposé. (Sprint 47, corrigé Sprint 88 #545)
...
## PIT-S78-004 — Un `<argLine>` littéral sur surefire détache l'agent JaCoCo ET peut faire tomber toute la suite Testcontainers
`jacoco:prepare-agent` pose la **propriété Maven** `argLine`, que surefire consomme. Y écrire un `<argLine>` littéral la remplace : rapport de couverture à 0 %, et si l'argLine existant portait des propriétés d'environnement (ici `-Dapi.version` pour docker-java), le JVM forké les perd et Testcontainers ne trouve plus Docker — **toute la suite backend tombe**, avec un message qui ne parle ni de JaCoCo ni d'argLine. Si un `<argLine>` devient nécessaire, il DOIT contenir `@{argLine}`. (Sprint 78 #169)

## PIT-S78-005 — `spring-boot-starter-parent` ne gère PAS toutes les versions de plugins : vérifier avant d'omettre `<version>`
Le parent Boot 3.5.16 ne contient **aucune** occurrence de `jacoco-maven-plugin` : omettre la version par mimétisme avec les autres plugins du `pom.xml` casse le build. Le style local d'un fichier n'est pas une preuve de ce que le parent gère — `grep` le pom du parent (ou `help:effective-pom`) avant de supposer. (Sprint 78 #169)
...
## PIT-S81-001 — Le lien de reset de mot de passe part vers `localhost` en production
`BrevoEmailService.java:68` porte le default `http://localhost:3000/reset-password` et `application-prod.properties` **ne le surcharge pas** : sans `APP_FRONTEND_RESET_URL_BASE`, chaque email de réinitialisation part avec un lien mort — l'envoi réussit, aucun log ne signale rien. Le **préfixe de locale est obligatoire** (`/fr/reset-password`) : le front est en `localePrefix:'always'`, donc une URL non préfixée dépend d'une redirection, et le garde canonique #322 **supprime la query string** en redirigeant (`middleware.test.ts:90`) — le `?token=` serait perdu. Les E2E n'attaquent que la forme déjà préfixée : elles ne peuvent pas attraper ce défaut. (Mise en ligne #370)
...
## PIT-S87-002 — Borner un débordement par les ancêtres `overflow` doit suivre la chaîne des BLOCS CONTENEURS, pas `parentElement`
Une frise en boucle (piste 1640 px dans un panneau `overflow:hidden`) rougissait forcément `landing-mobile-overflow` (balayage `rect.right > clientWidth`, qui ignore le rognage — PIT-S77-002). Le premier correctif bornait par chaque ancêtre DOM rognant : un `position:fixed` (ou `absolute`) qui ÉCHAPPE au rognage mais déborde réellement aurait passé — faux vert sur le garde-fou assoupli par le commit même (MAJEUR de revue). Solution : remonter la chaîne des blocs conteneurs (`fixed` → 1er ancêtre `transform`/`filter`/`contain`/`will-change`/`container-type`…, `absolute` → idem ou 1er positionné) ; un rognant situé strictement entre un élément hors flux et son bloc conteneur ne le rogne pas. Prévention : toute sonde qui borne un débordement porte une sonde d'armement `fixed` ET `absolute` échappés, vue rouge par mutation. (Sprint 87 #611 + revue C1)

## PIT-S87-003 — `animations: 'disabled'` NE rend PAS déterministe une boucle CSS infinie en `transform` + `will-change`
Référence `landing-hero` régénérée dans l'image noble contre le build de production, puis recomparée une minute plus tard : **2382 px (ratio 0.01)**, UNIQUEMENT le texte de la piste animée décalé d'~1 px. La capture tombe après une avancée fractionnaire de la boucle (~16 px/s) et la couche composée rastérise ce texte différemment. Élargir la tolérance désarme la spec (PIT-S77-019). Solution : figer par feuille injectée (`animation:none; transform:none; will-change:auto` en `!important`) puis ATTENDRE `getAnimations().length === 0` et `transform === 'none'` ; même chemin pour le contrôle négatif. Prévention : après toute régénération, 3 recomparaisons consécutives sans `--update-snapshots` ; une mesure `Animation.cancel()` isolée ne prouve rien. (Sprint 87, lead + revue C2)

## PIT-S87-004 — Sous jsdom, `import.meta.url` n'est pas `file:` : `fileURLToPath(new URL(…, import.meta.url))` lève
Un test Vitest (environnement jsdom) qui relit une feuille CSS par `fileURLToPath(new URL('…', import.meta.url))` lève `TypeError: The URL must be of scheme file`. Les tests en `@vitest-environment node` (ex. `base-layer.test.ts`) n'ont pas le problème. Solution : `join(__dirname, …)`, ou copier le motif AVEC son en-tête d'environnement node. (Sprint 87 #611)
...
## PIT-S89-001 — Un test existant peut graver le bug : s'il rougit après le correctif, lire ce qu'il attendait avant de toucher au correctif
Au S89, le cas Vitest « erreur 409 : message conflict inline » (variante catégorie, sans cible) décrivait exactement le comportement défectueux de #546 et a rougi une fois le bug corrigé. Remède : déplacer l'attente sur une variante où elle reste vraie (event) et commenter pourquoi. Prévention : devant un test qui rougit après un correctif, vérifier s'il validait le défaut avant de « réparer » le correctif. (Sprint 89 #546)
...
## PIT-S93-001 — Un soft delete masqué par `@SQLRestriction` rend l'ownership du contrôleur INAPPLICABLE
Le motif habituel « charger la ressource, comparer le propriétaire, 403/404 » ne marche plus sur une entité que `@SQLRestriction("archived = false")` rend invisible : toute lecture répond 404, y compris pour le propriétaire. L'autorisation doit alors migrer DANS la requête native (`WHERE id = :id AND user_id = :uid AND archived = true`), et son absence ne se voit **à aucun endroit du contrôleur** — une revue qui lit le contrôleur conclut à tort qu'il n'y a pas de contrôle. Corollaire : 0 ligne modifiée ⇒ 404 uniforme (inconnu / déjà actif / d'autrui), le 403 restant réservé au path ≠ JWT. (Sprint 93 #711)

## PIT-S93-002 — Armer un test d'IDOR en retirant la clause SQL sans le binding produit un faux rouge
Retirer `AND user_id = :uid` en laissant le `setParameter("uid", …)` correspondant fait échouer la requête sur un **paramètre non lié** : le test rougit pour une erreur technique, pas parce que l'IDOR est devenu possible. L'armement ne prouve alors rien. Retirer **clause ET binding** ensemble, et vérifier que le rouge est bien sémantique (204 au lieu de 404). (Sprint 93 #711)

## PIT-S93-003 — `FreezingArchRule` gèle par VIOLATION, pas par classe
Ajouter à un mapper de `application/` une méthode qui prend une entité JPA casse `ArchitectureTest` **même quand la classe viole déjà la règle** : le gel enregistre chaque violation individuellement, pas la classe fautive. Une nouvelle méthode = une nouvelle violation non gelée. Mettre le mapping dans l'adaptateur `infrastructure/` plutôt que d'étendre le gel. (Sprint 93 #711)
...
## PIT-S93-005 — Sur une entité PARTAGÉE, le filtre `user_id` d'un comptage est un contrôle d'isolation, pas une optimisation
Les catégories « système » (owner NULL) sont visibles de tous. Un comptage de produits liés sans `WHERE user_id = :caller` publie donc, sur une simple carte, le nombre de produits d'autres comptes. Le filtre ressemble à une optimisation et se fait supprimer comme telle en revue de performance. Corollaire : deux comptages de la même table peuvent avoir des scopes **volontairement** différents (`countByCategoryForUser` = affichage scopé ; `countByCategoryId` = intégrité FK, tous utilisateurs) — les unifier casse soit l'isolation, soit la garde du 409. Un test qui les OPPOSE explicitement est le seul garde-fou. (Sprint 93 #695)
...
## PIT-S95-004 — Un seuil numérique recopié dans un composant de feedback dérive en silence (6 vs 8 pendant 24 sprints)
`PasswordStrength.tsx` scorait `length >= 6` alors que la politique était passée à 8 au S71 (#148) : rien ne reliait la valeur à sa source, et sa propre JSDoc affirmait encore « la contrainte réelle (>= 6) ». Toute valeur d'un composant d'UI qui reflète une règle serveur doit être IMPORTÉE de la constante partagée, jamais retapée. (Sprint 95, #508)

## PIT-S95-005 — Une clé i18n d'état vide réutilisée pour deux portées devient un mensonge
`CompactAgenda` réutilisait `compactAgenda.empty` (« ni aujourd'hui ni demain ») dans la branche du groupe « Aujourd'hui », atteinte uniquement quand `isEmpty` est FAUX par construction — donc quand des événements existent demain. Une clé par portée ; et en review i18n, relire le libellé À CÔTÉ de sa condition de rendu, pas la seule liste de clés. (Sprint 95, #701)

## PIT-S95-006 — Remplacer une chaîne en dur par une clé i18n casse les regex de test qui citaient l'ancienne chaîne
`apiClient.test.ts:64` assertait `/session expirée/i` ; la clé réutilisée dit « session **a** expiré » — l'accord grammatical diffère, et l'échec n'est pas visible à la lecture du diff. Après tout remplacement de ce type, grepper les regex de test qui citent l'ancien libellé ; le run ciblé du seul fichier modifié ne suffit pas. (Sprint 95, #713)
...
## PIT-S96-002 — Un `flex-wrap` découpe différemment selon chaque surface qui le monte
La même palette rendait 11+1, 11+1 et 10+2 selon l'écran d'accueil ; l'issue n'en avait vu qu'un. Dès qu'un retour à la ligne inesthétique est signalé sur UN écran, mesurer les AUTRES points de montage avant de corriger. Remède structurel : `grid-cols-N` + `w-fit`, le découpage devient une propriété du composant et non de son contexte. (Sprint 96, #665)

## PIT-S96-003 — `w-fit` sur une grille : des colonnes comprimées font déborder des enfants à taille figée
`w-fit` vaut `min(max-content, disponible)` : si l'intrinsèque dépasse de peu le conteneur, les colonnes se compriment (43,5 px) pendant que des enfants `size-11` débordent de leur cellule — 0,5 px, invisible à l'œil comme à un test de classes. Dimensionner l'écart pour que l'intrinsèque tienne dans la plus ÉTROITE des surfaces (294 ≤ 301 px ici), et mesurer `max(x+w) − (min(x) + largeurGroupe)` sur toutes. (Sprint 96, #665)

## PIT-S96-004 — Un `absolute` dans un conteneur défilant bouge avec le contenu : « enfant.y == parent.y + c » n'est vrai qu'à `scrollTop` 0
`sprint-95-toast-overlap` assertait la position de la croix d'un `DialogContent` relativement au sheet ; +30 px de contenu (palette 44 px) l'ont fait tomber, et le message accusait la correction d'un défaut de position qu'elle ne causait pas. Toute assertion de cette forme dans un conteneur `overflow:auto` doit neutraliser le défilement (`scrollTop = 0`) ou le mesurer, et écrire la prémisse. (Sprint 96, #665)

## PIT-S96-005 — Ce qu'une spec visuelle MASQUE est exactement ce qu'elle ne prouve pas
`sprint-77-theme-visual` pose `display:none` sur `network-banner` avant chaque capture (à raison). Conséquence non écrite : le recouvrement de #656 a vécu ≥ 13 sprints derrière ce masque, invisible dans les deux sens. Avant de conclure d'un vert de captures, lister ce que la spec neutralise ; ces éléments exigent un oracle de géométrie (`boundingBox` + `elementFromPoint`, élément FORCÉ à l'écran). (Sprint 96, #656)
...
## PIT-S96-008 — Une spec de garde peut ne garder que la moitié du défaut qu'elle documente
La JSDoc de la palette mesurait le défaut à 1280 px ; la spec livrée avec ne tournait qu'à 375 px. Le critère d'acceptation principal n'était donc pas gardé là où le défaut avait été constaté — et l'audit de Phase 6 du lead l'avait déclaré couvert. Quand une spec et le commentaire qu'elle sert citent des conditions différentes (viewport, thème, état), l'écart EST le trou : les lire côte à côte. Trouvé par la review batch. (Sprint 96, review #665)

## PIT-S96-009 — Un renvoi « déjà couvert par la spec X » se vérifie en deux temps
Le commentaire déléguait la couverture desktop à deux specs « qui tournent à 1280 px » : l'une y tourne sans aucun `boundingBox`, l'autre mesure des boîtes mais à 390 px. Vérifier que X tourne à la bonne CONDITION **et** asserte la bonne PROPRIÉTÉ — greper l'oracle (`boundingBox`, `toHaveScreenshot`…), pas seulement `viewport`/`test.use`. (Sprint 96, review #665)
...
## PIT-S111-001 — Un numéro de migration retiré de l'historique reste dans les `flyway_schema_history` locales
Un premier `V16__delete_unbounded_recurring_events.sql` (#452) a été ajouté puis retiré le 2026-09-02 (`072a40a6` → `61ca5d0f`) ; le nouveau V16 (#653) réutilise le numéro. Toute base qui a booté entre les deux commits refusera le boot (checksum/description). CI et Testcontainers sont épargnés (bases neuves), la base e2e était à V15. Remédiation (ADR-010, Risques) : supprimer la ligne 16 de `flyway_schema_history` puis redémarrer — **jamais `flyway repair`**, qui marquerait V16 appliquée sans créer la colonne. Prévention : `git log --all --name-status -- 'backend/src/main/resources/db/migration/V<N>*'` avant de prendre un numéro. (Sprint 111 #653, revue db-expert)
...
## PIT-S112-005 — Ajouter un paramètre au constructeur de `SecurityConfig` casse la COMPILATION de toute la suite de tests
`CorsAllowedOriginsConfigIntegrationTest` l'instancie directement (`new SecurityConfig(null, null, …)`) : l'erreur n'est pas un test rouge mais 0 test exécuté. `/usr/bin/grep -rn "new SecurityConfig(" backend/src/test` avant de toucher la signature. (Sprint 112 #831)
