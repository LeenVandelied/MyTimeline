...
`rangeStart` est inatteignable à la souris (en-tête de lane sticky `--lane-header-w=168px` à l'époque — 176 px depuis #674, S105 — recouvrant un event
...
## PAT-S92-001 — Fonction de contexte à options branchée en `onClick` : typer pour que `tsc` refuse, normaliser à l'exécution
`onClick={open}` passe le `MouseEvent` en premier argument : dès que `open(options?)` accepte un objet, l'évènement devient `options`. Parade double : un type d'options sans propriété commune avec `MouseEvent` (TypeScript refuse alors `onClick={open}`, ce qui force `onClick={() => open()}`) + une normalisation à l'exécution qui n'accepte qu'un objet simple portant un `productId` chaîne non vide. Garder l'identité de la fonction stable (`useCallback` à deps vides). Référence : `CreateEventContext.toCreateEventPrefill` ; armement : gardes retirées → 4 rouges. (Sprint 92 #605)
...
## PAT-S96-001 — Cible tactile d'un bouton-icône : agrandir ou `::before` se tranche par la densité de la surface
Zone d'expansion invisible `::before` 44×44 (PAT-S24-002) quand le visuel est contraint par ses voisins (barre d'outils de la frise, en-tête de drawer) ; agrandissement réel quand la surface est aérée — et à densité égale, copier les classes du bouton-icône le plus proche dans le même dossier (`BottomSheet.tsx:181` a donné `h-11 w-11` à l'identique pour #633). Anti-pattern : le `::before` partout « parce que c'est le motif maison », qui fige des visuels à 28–36 px et creuse un écart durable entre boîte peinte et boîte tappable. Voir aussi DEC-S96-003 : dans un panneau défilant, le `::before` est rogné. (Sprint 96, #633 / #665)

## PAT-S96-002 — Prouver une correction de layout par une sonde jetable qui mesure l'AVANT
Une sonde Playwright jetable imprime les boîtes en JSON sur une ligne marquée (`###PROBE###`), lancée via `rtk proxy` ; ses chiffres deviennent le contrôle négatif de la spec définitive, puis elle est supprimée. Si la correction est déjà commitée, réinjecter l'ancienne mise en page par `page.addStyleTag()` (l'ancien `className` se lit avec `git show <sha>^:<fichier>`) plutôt que de reverter la production. Au S96 elle a reproduit 402/402/377 px et 11+1/11+1/10+2 — identiques à la JSDoc, ce qui arme la garde ET confirme le commentaire d'un coup. Anti-pattern : une spec définitive vue verte sans aucune preuve qu'elle aurait été rouge avant. (Sprint 96, #665 + review)

## PAT-S96-003 — Forcer la bannière réseau de façon déterministe sur une page publique
`page.route('**/api/auth/me', r => r.fulfill({ status: 500 }))` avant `goto`. `AuthProvider` appelle `/me` au montage sur toute page, et l'intercepteur d'`apiClient` classe la santé réseau AVANT le court-circuit `INLINE_AUTH_ENDPOINTS` (`apiClient.ts:192-197`) : un ≥ 500 monte `network-banner` en `data-state="server-error"` sans redirection (seul le 401 redirige). Anti-pattern : injecter une feuille de style pour « faire apparaître » la bannière — on mesurerait une géométrie fabriquée. Corollaire : `sprint-95-toast-overlap` utilise un 400 précisément pour ÉVITER cet effet. (Sprint 96, #656)
...
## PAT-S102-001 — Faire taire un signalement global pour un seul écran : opt-out typé PAR REQUÊTE
Problème : un intercepteur (toast d'erreur) doublonne un écran qui affiche déjà l'erreur. Solution : champ ajouté par augmentation de module `declare module 'axios' { interface AxiosRequestConfig { inlineHandledStatuses?: readonly InlineHandledStatus[] } }`, le type restreignant les statuts neutralisables (`InlineHandledStatus = 403` : le 401 ne peut PAS être tu), transporté écran → hook → service en paramètre optionnel ; constante gelée `HANDLES_FORBIDDEN_INLINE`. Anti-pattern : liste d'URL (façon `INLINE_VALIDATION_ENDPOINTS`), qui tait aussi les appelants présents et futurs de la route sans gestion inline. Règle : n'opter out qu'APRÈS avoir rendu l'erreur dans l'écran. Exemple : `services/inlineErrorHandling.ts`. (Sprint 102, #761)
...
## PAT-S105-001 — Une valeur de token lue par les E2E, jamais recopiée
Un helper E2E lit le token dans la feuille (`LANE_GUTTER_PX` dans `e2e/support/timeline-lanes.ts`, même regex que le test de dérive Vitest du miroir JS `LANE_TRACK_OFFSET_PX`) ; les assertions restent comparées à une mesure de rendu. Anti-pattern : recopier la valeur en dur dans N specs (168 à 3 endroits a survécu au changement du token) ou importer le composant TSX dans Playwright pour une constante. (Sprint 105, #674)
