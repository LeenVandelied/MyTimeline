...
Au S81, trois gardes sur trois ont été vérifiées rouges avant d'être acceptées, et le contrôle négatif du correctif de sécurité a découvert un défaut **que les trois relecteurs avaient manqué** : le masquage des en-têtes fonctionnait, mais deux JWT en clair subsistaient dans la même sortie ([[PIT-S81-004]]). Deux cas où le contrôle est **indispensable**, pas optionnel : (a) ajout d'un slot de rate-limit — le filtre court-circuite avant l'authz, donc **tout est déjà non-429** et un test mal écrit est vert par construction (parade : renommer la clé du slot, exiger le rouge `429 → 401`, restaurer) ; (b) garde écrite **après** le fix et jamais vue échouer. Règle : lire la sortie du run rouge, pas seulement son code de retour. (Sprint 81 #499, #215, #500)
