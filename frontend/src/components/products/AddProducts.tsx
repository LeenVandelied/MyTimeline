...
export default function AddProduct({ onProductAdded }: AddProductProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent
        className="bg-bg border-rule max-h-[90vh] overflow-y-auto rounded-xl border p-0 shadow-xl sm:max-w-[600px]"
          ...
          <DialogHeader>
            ...
            <DialogDescription id="product-dialog-description" className="text-ink">
              {t('products.add.description') ||
                'Ajoutez un nouveau produit avec ses événements associés'}
            </DialogDescription>
          </DialogHeader>
        </div>
          ...
          <Form {...productForm}>
            <form onSubmit={productForm.handleSubmit(onSubmitProduct)} className="space-y-6">
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          ...
                          <SelectContent className="text-ink-muted bg-surface border-rule border">
                            <SelectItem
                              className="hover:text-ink hover:bg-surface-2 focus:bg-surface-2"
                              value="7446a49c-4053-4751-837a-c968a3f568ba"
                            >
                              {t('products.add.categories.vehicles')}
                            </SelectItem>
                            <SelectItem
                              className="hover:text-ink hover:bg-surface-2 focus:bg-surface-2"
                              value="dbc134fb-9558-476d-88c7-75992a249adc"
                            >
                              {t('products.add.categories.insurance')}
                            </SelectItem>
                            <SelectItem
                              className="hover:text-ink hover:bg-surface-2 focus:bg-surface-2"
                              value="9817e487-b43a-47b8-9cc7-ba5bf785bcdd"
                            >
                              {t('products.add.categories.food')}
                            </SelectItem>
                            <SelectItem
                              className="hover:text-ink hover:bg-surface-2 focus:bg-surface-2"
                              value="ec088b7c-0a78-4c41-84f4-21c7626d9707"
                            >
              ...
              >
                ...
                {events.length > 0 && (
                  <div className="mb-6">
                    <h4 className="text-ink-muted mb-3 text-sm font-medium">
                      {t('products.add.event.list')}
                    </h4>
                    <ul className="space-y-2">
                      {events.map((event, index) => (
                        <motion.li
                          ...
                          key={index}
                          className="bg-surface-2 border-rule-strong group hover:border-accent flex items-center justify-between rounded-lg border p-3 transition-all"
                        >
                          <span className="text-ink flex items-center">
                            {event.type === 'duration' ? (
                              <Timer className="text-accent mr-2 h-4 w-4" />
                            ) : (
                              <Calendar className="mr-2 h-4 w-4 text-[var(--color-success)]" />
                            )}
                            ...
                            {event.type === 'duration' &&
                              ...
                              event.durationUnit && (
                                <span className="text-ink-muted ml-2 text-sm">
                                  ({event.durationValue}{' '}
                                  {t(`products.add.event.units.${event.durationUnit}`)})
                                </span>
                              )}
                          ...
                          <Button
                            ...
                            className="text-danger hover:text-danger hover:bg-surface-2 opacity-0 transition-opacity group-hover:opacity-100"
                          >
                            <X size={16} />
                          </Button>
                        </motion.li>
                ...
                <div className="bg-surface border-rule space-y-4 rounded-lg border p-4">
                  <Form {...eventForm}>
                    <div className="space-y-4">
                          ...
                          <FormItem>
                            ...
                            <FormControl>
                              <Select onValueChange={field.onChange} defaultValue={field.value}>
                                <SelectTrigger className="bg-surface border-rule text-ink focus:border-accent focus:ring-accent">
                                  <SelectValue
                                    placeholder={t('products.add.event.form.typePlaceholder')}
                                  />
                                </SelectTrigger>
                                <SelectContent className="text-ink-muted bg-surface border-rule border">
                                  <SelectItem
                                ...
                                <FormControl>
                                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                                    <SelectTrigger className="bg-surface border-rule text-ink focus:border-accent focus:ring-accent">
                                      <SelectValue
                                        placeholder={t(
                                          'products.add.event.form.durationUnitPlaceholder',
                                        )}
                                      />
                                    </SelectTrigger>
                                    <SelectContent className="text-ink-muted bg-surface border-rule border">
                        ...
                        >
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem className="flex-1">
                                <Label className="text-ink">
                                  {t('products.add.event.form.date')}
                                </Label>
                                <FormControl>
                                  <Input
                                    type="date"
                      ...
                      <div className="mt-4 flex justify-end">
                        <Button
                          type="button"
                          className="bg-accent hover:bg-accent-hover text-accent-ink flex transform items-center gap-2 transition-all hover:translate-y-[-2px]"
              ...
              >
                <Button
                  type="submit"
                  className="bg-accent hover:bg-accent-hover text-accent-ink flex transform items-center gap-2 rounded-lg px-6 py-2 font-medium shadow-lg transition-all hover:translate-y-[-2px] hover:shadow-md"
                >
                  {t('products.add.form.submit')}
                  <ArrowRight size={16} />
                </Button>
