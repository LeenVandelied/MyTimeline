...
describe('ConflictDialog', () => {
  ...
  it('ouvert : dialog accessible (role=dialog) + titre + description + action recharger', () => {
    ...
    expect(screen.getByText('common.conflictDialog.title')).toBeInTheDocument()
    expect(screen.getByText('common.conflictDialog.description')).toBeInTheDocument()
  ...
  it('clic annuler : ferme sans recharger', async () => {
    ...
    const onOpenChange = vi.fn()
    render(<ConflictDialog open onOpenChange={onOpenChange} onReload={onReload} />)
    await userEvent.click(screen.getByText('common.conflictDialog.dismiss'))
    expect(onOpenChange).toHaveBeenLastCalledWith(false)
