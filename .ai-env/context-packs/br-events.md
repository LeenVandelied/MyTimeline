...
**Test** : `frontend/src/components/EventEditForm.debounce.test.tsx` (Sprint 82 #507) — 3 tests (titre, couleur, `startDate`) sous `vi.useFakeTimers()` : à `t + 149 ms` l'aperçu doit encore porter l'ANCIENNE valeur, la nouvelle n'arrivant qu'à `t + 150 ms`. Vu ROUGE 3/3 en rebranchant les props sur `form.watch()` brut avant merge.
