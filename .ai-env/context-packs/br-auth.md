...
**Arbitrage (exécuté côté FRONT, pas par le serveur)** : à l'établissement d'une session, si `themePreference` du compte est non nulle → elle s'applique et écrase le choix local ; si elle est `null` → le choix local courant (défaut `next-themes` : `system`) est envoyé par `PUT /api/me/preferences` et devient celui du compte. Ensuite toute bascule écrit localement ET sur le compte ; un second appareil reçoit la préférence du compte à sa connexion (`GET /api/auth/me`).
...
> ⚠ **Côté front (vague 2 S111)** : `UserSchema` Zod doit déclarer `themePreference` **nullable** (et tolérer la clé) ; l'arbitrage vit dans `theme-provider.tsx` / l'`AuthContext`. Tant que la vague 2 n'est pas livrée, le serveur stocke la préférence mais aucun client ne la lit.
