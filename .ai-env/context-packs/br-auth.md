...
| `PUT /api/me/preferences` | ❌ (401) | ✅ | ✅ | — | S111 #653 : préférence de thème du compte (BR-AUT-013) ; identité = JWT ; plafond 30/min PAR UTILISATEUR (id de compte, `UserRateLimitingFilter`, S112 #831, ADR-010 § 7 amendé) |
...
**Tests** : `PasswordPolicyTest` (29, dont l'égalité des verdicts entre les 3 endpoints et la non-application au login) ; `AuthControllerLegacyPasswordLoginTest` (3, comptes legacy : login à 6 caractères OK, register du même mot de passe refusé, mise en conformité par change-password) ; `frontend/src/lib/schemas/password-policy.test.ts` (58 depuis S101 #735 : bloc « divergence ASCII/Unicode » supprimé, cas Unicode ajoutés — la politique front réplique `StrongPasswordValidator` à l'identique, DEC-S101-002).
...
> ✅ **Étendu Sprint 111 (#653)** : la projection `UserResponse` porte `themePreference` (`"light"`|`"dark"`|`"system"`|`null`, clé TOUJOURS présente, `null` = aucun choix explicite) — sur `GET /api/me`, `GET /api/auth/me` ET `PUT /api/me/preferences`. Cf. BR-AUT-013. Aucun secret ajouté.
...
### BR-AUT-013 — Préférence de thème du compte + arbitrage à la connexion (Sprint 111 #653, ADR-010)
**Règle** : Le compte PEUT porter une préférence de thème `light` | `dark` | `system` ; `null` signifie « aucun choix explicite » et est DISTINCT de `system` (choix explicite « suivre l'OS »). `PUT /api/me/preferences {"themePreference": ...}` MUST n'accepter que ces trois valeurs exactes (minuscules, sans espace) et MUST refuser en **400** `validation_failed` toute autre valeur, `null`, la clé absente ou un corps vide ; il MUST agir sur le caller issu du JWT (401 anonyme) et renvoyer `UserResponse` à jour (200). L'API ne remet JAMAIS un compte à `null`. Les autres écritures du profil (PATCH `/api/me`, change/reset-password, avatar) MUST NOT modifier la préférence.
**Arbitrage (exécuté côté FRONT, pas par le serveur)** : à la **connexion explicite** (succès de `login`), JAMAIS à la restauration de session au montage : si `themePreference` du compte est non nulle → elle s'applique localement et écrase le choix local, SANS réécriture du compte ; si elle est `null` et qu'un choix local EXPLICITE existe (clé `localStorage['theme']` présente) → il est envoyé par `PUT /api/me/preferences` et devient celui du compte ; si elle est `null` sans choix local → rien n'est écrit (le compte reste `null`). `register` n'arbitre pas (pas de session ouverte). Ensuite toute bascule écrit localement ET, si authentifié, sur le compte (pas de PUT si valeur identique ; échec toléré, thème local conservé). Un second appareil reçoit la préférence du compte à sa connexion. **Conséquence assumée (décision S111)** : un appareil déjà connecté ne suit une préférence changée ailleurs qu'à sa reconnexion. Logout ne touche pas au thème local.
**Pourquoi** : clôt les 2 critères non tenus de #642 (DEC-S82-009 temps 2 et 3, DEC-S83-003). Sans `null` distinct de `system`, un compte neuf écraserait le choix fait avant connexion par un défaut qu'il n'a jamais exprimé.
**Implémentation** : migration `V16__user_theme_preference.sql` (colonne `users.theme_preference varchar(16)` NULLABLE sans défaut, CHECK nommé `ck_users_theme_preference`) ; enum domaine `ThemePreference` (`value()` minuscule, `fromValue` strict) ; `ThemePreferenceConverter` (JPA, PAS `@Enumerated(STRING)` qui écrirait `LIGHT`) ; hydratation en LECTURE dans l'adaptateur (`UserRepositoryJpaImpl.toDomain` → `User.withThemePreference`), PAS dans `UserMapper` (application → infra, dette gelée par `ArchitectureTest`) — le `User` propriétaire d'un `Product` porte donc `null` ; port dédié `UserRepository.updateThemePreference` — **seul chemin d'écriture** : `UserRepositoryJpaImpl.copyMutableFields` ne recopie volontairement PAS la préférence (5 chemins reconstruisent un `User` sans elle) ; `UserService.updateThemePreference` ; `UserController` `PUT /api/me/preferences` + `UpdatePreferencesRequest` (`@NotNull @Pattern("light|dark|system")`). Export RGPD : `ExportedProfile.themePreference` (JSON/CSV/Markdown). Rate-limit : ~~hors périmètre~~ → **S112 #831** : 30 `PUT`/min par id de compte via `UserRateLimitingFilter` (monté avant `AuthorizationFilter`, donc après `JwtFilter` ; un plafond par IP est exclu car la suite E2E partage 127.0.0.1), 429 identique aux autres créneaux ; principal authentifié sans id de compte → 401 fail-closed ; variantes de chemin comptées (`%70references`) ou rejetées (400/404) ; CSRF : aligné sur les autres routes mutantes `/api/me` (`csrf.disable()` + `SameSite=Lax`).
**Tests** : `ThemePreferenceIntegrationTest` (6 — Postgres réel, V16 + `ddl-auto=validate` au boot, `null` sur les deux `/me`, stockage minuscule, 2e session, PATCH n'efface pas — armé : recopier la préférence dans `copyMutableFields` le rougit —, 400/401 par la vraie chaîne, CHECK SQL), `UserControllerTest` (+13 dont 8 cas 400 paramétrés), `UserServiceImplTest` (+3), `ThemePreferenceTest` (9), `UserDataExportTest` (+1), `ExportRenderersTest` (+3).
> ✅ **Côté front (vague 2 S111)** : `UserSchema` Zod déclare `themePreference: z.enum(THEME_OPTIONS).nullable()` ; arbitrage dans `AuthContext.login` (`applyAccountThemeAtLogin` avant publication du user, `adoptLocalThemeAtLogin` après) ; écriture des bascules par `useThemeChoice.setThemeChoice` → `ThemePersistenceContext` (fourni par `AuthProvider`) → `userService.updatePreferences`. Tests : `AuthContext.theme.test.tsx` (14), `useThemeChoice.test.tsx` (+6), `types/user.test.ts` (+8) ; E2E `sprint-111-theme-account-preference.spec.ts` (compte neuf). Les specs E2E qui basculent le thème sous `SHARED` interceptent le PUT (`e2e/support/theme-preference.ts#keepThemeOffSharedAccount`) : une préférence posée ne peut plus revenir à `null`.
...
- ADR : `docs/adr/ADR-010-preference-de-theme-du-compte.md` (BR-AUT-013)
