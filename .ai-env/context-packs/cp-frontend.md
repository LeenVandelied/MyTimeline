...
- **`src/components/`** : `ui/` (shadcn : button, card, dialog, select, form, input, spinner, dropdown-menu, popover, language-selector…), `calendar/`, `pages/`, `products/`, + composants métier (`EventContent`, `EventEditForm`, `theme-provider`) ; `landing/` porte les sections de la page d'accueil (`FeaturesSection` et `Testimonial*` supprimés au S103).
...
- **Thème (#655, #653)** : `hooks/useThemeChoice.ts` est le SEUL module qui écrit le thème (garde-fou textuel dans son test). Préférence portée par le compte (ADR-010, BR-AUT-013) : arbitrée à la CONNEXION explicite (`AuthContext.login`), jamais au montage ; les bascules authentifiées sont persistées via `ThemePersistenceContext` fourni par `AuthProvider`. E2E : jamais basculer le thème sous `SHARED` sans `keepThemeOffSharedAccount` (une préférence posée ne revient pas à `null`).
...
- DTO connus : login `{username,password}`, register `{name,username,email,password}`, forgot `{email}`, reset `{token,newPassword}`, `/auth/me` → `UserSchema {id(uuid),name,username,email,role,avatarUrl|null,themePreference: 'light'|'dark'|'system'|null}` (#653 : clé toujours présente, `null` ≠ `system`) ; `PUT /api/me/preferences {themePreference}` → `UserResponse` (`userService.updatePreferences`).
